...
- AXI4 写侧当前按路径区分：
  ...
  - LLC 打开时，interconnect 侧同样最多可排队 `MAX_WRITE_OUTSTANDING`
    个上游写请求，LLC 核心自身现在也带有内部 pending write queue
  - 但 LLC 仍通过单个 active write pipeline 串行执行真正的写命中/失配/
    victim writeback；同一个 master 的后继 queued write 只有在前一个写
    响应槽被消费后才会继续提升执行
...
- 当前收尾版本把 correctness 边界定义为：
  ...
  - 写侧支持“interconnect 多 pending + LLC 内部 pending queue”，并保证
    写响应、victim writeback、maintenance 与 demand miss 的顺序正确。
  - LLC 内部暂不实现多个 active write-context 并发执行；当前使用
    “多排队 + 单 active write pipeline” 作为收尾版本。
- 不继续把 LLC 写路径做成多个 active write-context 的原因是：
  - 当前 correctness 风险已经集中在写命中/失配、victim writeback、
    stale refill、maintenance interlock 这些时序点上。
  - 单 active pipeline 已能承载多个 pending write，并且显著降低验证复杂度。
  - 后续如果要进一步追性能，再把它当成独立性能阶段处理，而不是和当前
    correctness 收尾混在一起。
- AXI3 路径进入冻结/退场路线：
