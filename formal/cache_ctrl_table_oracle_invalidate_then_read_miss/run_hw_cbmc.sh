...
build_dir="${repo_root}/local_debug/hw_cbmc_cache_ctrl_table_oracle_invalidate_then_read_miss"
...
exec timeout "${timeout_sec}" "${hw_cbmc}" \
  ...
  "${build_dir}/llc_cache_ctrl.pre.v" \
  "${script_dir}/cache_ctrl_table_oracle_invalidate_then_read_miss_formal_top.v" \
