...
build_dir="${repo_root}/local_debug/hw_cbmc_dual_bridge_prod_width_cacheline_ar_shape"
timeout_sec="${HW_CBMC_TIMEOUT_SEC:-240}"
extra_args=()
...
exec timeout "${timeout_sec}" "${hw_cbmc}" \
  ...
  "${build_dir}/axi_llc_axi_bridge_dual.pre.v" \
  "${script_dir}/axi_llc_axi_bridge_dual_prod_width_cacheline_ar_shape_formal_top.v" \
