...
build_dir="${repo_root}/local_debug/hw_cbmc_dual_bridge_prod_helper_write_issue_shape"
timeout_sec="${HW_CBMC_TIMEOUT_SEC:-600}"
extra_args=()
if [[ -n "${HW_CBMC_EXTRA_ARGS:-}" ]]; then
  # shellcheck disable=SC2206
  extra_args=(${HW_CBMC_EXTRA_ARGS})
fi
...
exec timeout "${timeout_sec}" "${hw_cbmc}" \
  ...
  "${build_dir}/axi_llc_axi_bridge_dual.pre.v" \
  "${script_dir}/axi_llc_axi_bridge_dual_prod_helper_write_issue_shape_formal_top.v" \
  --module axi_llc_axi_bridge_dual_prod_helper_write_issue_shape_formal_top \
  --bound 10 \
  "${extra_args[@]}"
