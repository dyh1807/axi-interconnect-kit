...
test_name="${HW_CBMC_TEST_NAME:-subsystem_dual_mode0_ddr_bypass_read_response}"
build_dir="${repo_root}/local_debug/hw_cbmc_${test_name}"
harness_c="${HARNESS_C:-${script_dir}/harness.c}"
...
exec timeout "${timeout_sec}" "${hw_cbmc}" \
  ...
  "${build_dir}/axi_llc_axi_bridge_dual.pre.v" \
  "${build_dir}/axi_llc_subsystem_dual.pre.v" \
  "${script_dir}/axi_llc_subsystem_dual_mode0_ddr_bypass_read_response_formal_top.v" \
  --module axi_llc_subsystem_dual_mode0_ddr_bypass_read_response_formal_top \
  --bound 48
