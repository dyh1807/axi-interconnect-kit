...
struct module_axi_llc_subsystem_dual_cache_dirty_evict_post_b_hit_formal_top
{
  bool clk;
  bool rst_n;
...
extern struct
    module_axi_llc_subsystem_dual_cache_dirty_evict_post_b_hit_formal_top
        axi_llc_subsystem_dual_cache_dirty_evict_post_b_hit_formal_top;
...
static void drive_idle(void)
{
  ...
  axi_llc_subsystem_dual_cache_dirty_evict_post_b_hit_formal_top.ddr_axi_bid =
      false;
}
...
int main(void)
{
  ...
  const uint64_t new_line_data = 0x2122232425262728ull;
  unsigned phase = 0u;
  ...
  axi_llc_subsystem_dual_cache_dirty_evict_post_b_hit_formal_top.rst_n =
      false;
  ...
  axi_llc_subsystem_dual_cache_dirty_evict_post_b_hit_formal_top.rst_n = true;
  ...
  {
    const bool issue_write = (phase < 3u) && !request_seen[phase];
    const bool issue_read = response_seen[2] && !read_request_seen;
    const bool b_should_drive = aw_accepted && w_accepted && !b_accepted;

    axi_llc_subsystem_dual_cache_dirty_evict_post_b_hit_formal_top
        .write_req_valid = issue_write;
    axi_llc_subsystem_dual_cache_dirty_evict_post_b_hit_formal_top
        .write_req_sel = (uint8_t)phase;
    axi_llc_subsystem_dual_cache_dirty_evict_post_b_hit_formal_top
        .read_req_valid = issue_read;
    axi_llc_subsystem_dual_cache_dirty_evict_post_b_hit_formal_top
    ...
    assert(!axi_llc_subsystem_dual_cache_dirty_evict_post_b_hit_formal_top
                .ddr_axi_arvalid);
    assert(!axi_llc_subsystem_dual_cache_dirty_evict_post_b_hit_formal_top
                .mmio_axi_arvalid);
    assert(!axi_llc_subsystem_dual_cache_dirty_evict_post_b_hit_formal_top
    ...
    if(axi_llc_subsystem_dual_cache_dirty_evict_post_b_hit_formal_top
           .write_req_valid &&
       axi_llc_subsystem_dual_cache_dirty_evict_post_b_hit_formal_top
           .write_req_ready)
    {
    ...
    if(axi_llc_subsystem_dual_cache_dirty_evict_post_b_hit_formal_top
           .read_resp_valid)
    {
      assert(read_request_seen || read_accepted_seen);
      read_resp_seen = true;
      assert(axi_llc_subsystem_dual_cache_dirty_evict_post_b_hit_formal_top
                 .read_resp_id == 4u);
      assert(axi_llc_subsystem_dual_cache_dirty_evict_post_b_hit_formal_top
                 .read_resp_data == new_line_data);
    }
