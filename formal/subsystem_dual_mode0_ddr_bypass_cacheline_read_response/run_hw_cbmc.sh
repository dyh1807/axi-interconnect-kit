...
build_dir="${repo_root}/local_debug/hw_cbmc_subsystem_dual_mode0_ddr_bypass_cacheline_read_response"
...
write_formal_params() {
  ...
  printf '`define AXI_LLC_LLC_SIZE_BYTES 128\n'
  printf '`undef AXI_LLC_WINDOW_BYTES\n'
  printf '`define AXI_LLC_WINDOW_BYTES 64\n'
...
exec timeout "${timeout_sec}" "${hw_cbmc}" \
  ...
  "${build_dir}/axi_llc_axi_bridge_dual.pre.v" \
  "${build_dir}/axi_llc_subsystem_dual.pre.v" \
  "${script_dir}/axi_llc_subsystem_dual_mode0_ddr_bypass_cacheline_read_response_formal_top.v" \
