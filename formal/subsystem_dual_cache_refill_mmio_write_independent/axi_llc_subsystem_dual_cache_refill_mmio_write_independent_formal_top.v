module axi_llc_subsystem_dual_cache_refill_mmio_write_independent_formal_top(
    input             clk,
    input             rst_n,
    input             ddr_read_req_valid,
    input             mmio_write_req_valid,
    output            ddr_read_req_ready,
    output            mmio_write_req_ready,
    output            ddr_read_req_accepted,
    output            mmio_write_req_accepted,
    output     [3:0]  ddr_read_req_accepted_id,
    ...
    localparam [FORMAL_ADDR_BITS-1:0] MMIO_WRITE_ADDR = 32'h1000_0008;

    wire [1:0] read_req_ready_w;
    wire [1:0] read_req_accepted_w;
    wire [2*FORMAL_ID_BITS-1:0] read_req_accepted_id_w;
    wire [2*FORMAL_READ_RESP_BITS-1:0] read_resp_data_w;
    wire [2*FORMAL_ID_BITS-1:0] read_resp_id_w;
    ...
    assign ddr_read_req_ready = read_req_ready_w[1];
    assign ddr_read_req_accepted = read_req_accepted_w[1];
    assign ddr_read_req_accepted_id =
        read_req_accepted_id_w[FORMAL_ID_BITS +: FORMAL_ID_BITS];
    ...
    axi_llc_subsystem_dual #(
        ...
        .read_req_valid({ddr_read_req_valid, 1'b0}),
        .read_req_ready(read_req_ready_w),
        .read_req_accepted(read_req_accepted_w),
        .read_req_accepted_id(read_req_accepted_id_w),
        .read_req_addr({DDR_READ_ADDR, {FORMAL_ADDR_BITS{1'b0}}}),
        .read_req_total_size({8'd7, 8'd0}),
        .read_req_id({4'h6, {FORMAL_ID_BITS{1'b0}}}),
        .read_req_bypass(2'b00),
        .read_resp_valid(),
        .read_resp_ready(2'b11),
        .read_resp_data(read_resp_data_w),
        .read_resp_id(read_resp_id_w),
        .write_req_valid(mmio_write_req_valid),
        .write_req_ready(mmio_write_req_ready),
        .write_req_accepted(mmio_write_req_accepted),
        .write_req_addr(MMIO_WRITE_ADDR),
        .write_req_wdata({32'd0, 32'hDEAD_BEEF}),
        .write_req_wstrb({4'd0, 4'hF}),
        .write_req_total_size(8'd3),
        .write_req_id(4'hA),
