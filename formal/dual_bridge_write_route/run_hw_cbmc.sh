...
build_dir="${repo_root}/local_debug/hw_cbmc_dual_bridge_write_route"
timeout_sec="${HW_CBMC_TIMEOUT_SEC:-180}"

mkdir -p "${build_dir}"

# hw-cbmc does not honor Verilog +incdir in this environment. Generate
# preprocessed RTL views from the real source files by replacing only the
# shared parameter include; the module bodies remain the production RTL.
{
...
exec timeout "${timeout_sec}" "${hw_cbmc}" \
  ...
  "${build_dir}/axi_llc_axi_bridge_dual.pre.v" \
  "${script_dir}/axi_llc_axi_bridge_dual_write_route_formal_top.v" \
  --module axi_llc_axi_bridge_dual_write_route_formal_top \
  --bound 7
