#include <assert.h>
#include <stdbool.h>
#include <stdint.h>

void next_timeframe(void);
void set_inputs(void);

struct
    module_axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
{
  bool clk;
  bool rst_n;
  bool cache_req_valid;
  bool bypass_req_valid;
  bool ddr_axi_awready;
  bool ddr_axi_wready;
  bool mmio_axi_awready;
  bool mmio_axi_wready;
  bool cache_req_ready;
  bool bypass_req_ready;
  bool ddr_axi_awvalid;
  bool ddr_axi_awid;
  uint32_t ddr_axi_awaddr;
  uint8_t ddr_axi_awlen;
  uint8_t ddr_axi_awsize;
  bool ddr_axi_wvalid;
  uint64_t ddr_axi_wdata_0;
  uint64_t ddr_axi_wdata_1;
  uint64_t ddr_axi_wdata_2;
  uint64_t ddr_axi_wdata_3;
  uint32_t ddr_axi_wstrb;
  bool ddr_axi_wlast;
  bool mmio_axi_awvalid;
  bool mmio_axi_awid;
  uint32_t mmio_axi_awaddr;
  uint8_t mmio_axi_awlen;
  uint8_t mmio_axi_awsize;
  bool mmio_axi_wvalid;
  uint32_t mmio_axi_wdata;
  uint8_t mmio_axi_wstrb;
  bool mmio_axi_wlast;
  bool ddr_axi_arvalid;
  bool mmio_axi_arvalid;
};

extern struct
    module_axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
        axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top;

static void drive_idle(void)
{
  axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
      .cache_req_valid = false;
  axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
      .bypass_req_valid = false;
}

static void hold_all_lower_ready_low(void)
{
  axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
      .ddr_axi_awready = false;
  axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
      .ddr_axi_wready = false;
  axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
      .mmio_axi_awready = false;
  axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
      .mmio_axi_wready = false;
}

static void check_no_read_escape(void)
{
  assert(!axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
              .ddr_axi_arvalid);
  assert(!axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
              .mmio_axi_arvalid);
}

static void check_aw_shape(bool ddr_awvalid, bool mmio_awvalid)
{
  if(ddr_awvalid)
  {
    assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
               .ddr_axi_awaddr == 0x40000200u);
    assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
               .ddr_axi_awlen == 1u);
    assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
               .ddr_axi_awsize == 5u);
  }
  if(mmio_awvalid)
  {
    assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
               .mmio_axi_awaddr == 0x10000024u);
    assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
               .mmio_axi_awlen == 0u);
    assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
               .mmio_axi_awsize == 2u);
  }
}

static void check_w_shape(bool ddr_wvalid, bool mmio_wvalid,
                          bool prior_ddr_w_seen)
{
  if(ddr_wvalid)
  {
    assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
               .ddr_axi_wstrb == 0xffffffffu);
    if(!prior_ddr_w_seen)
    {
      assert(!axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
                  .ddr_axi_wlast);
      assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
                 .ddr_axi_wdata_0 == 0xa5a55a5adeadbeefull);
      assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
                 .ddr_axi_wdata_1 == 0x1122334455667788ull);
      assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
                 .ddr_axi_wdata_2 == 0x8877665544332211ull);
      assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
                 .ddr_axi_wdata_3 == 0x0f1e2d3c4b5a6978ull);
    }
    else
    {
      assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
                 .ddr_axi_wlast);
      assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
                 .ddr_axi_wdata_0 == 0x89abcdef01234567ull);
      assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
                 .ddr_axi_wdata_1 == 0x0123456789abcdefull);
      assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
                 .ddr_axi_wdata_2 == 0xffeeddccbbaa9988ull);
      assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
                 .ddr_axi_wdata_3 == 0x7766554433221100ull);
    }
  }

  if(mmio_wvalid)
  {
    assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
               .mmio_axi_wdata == 0xcafe0204u);
    assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
               .mmio_axi_wstrb == 0xfu);
    assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
               .mmio_axi_wlast);
  }
}

int main(void)
{
  bool seen_aw_together = false;
  bool seen_w_together = false;
  bool prior_ddr_w_seen = false;

  axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
      .rst_n = false;
  drive_idle();
  hold_all_lower_ready_low();
  set_inputs();
  next_timeframe();

  axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
      .rst_n = true;
  drive_idle();
  hold_all_lower_ready_low();
  set_inputs();
  next_timeframe();

  axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
      .cache_req_valid = true;
  axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
      .bypass_req_valid = true;
  hold_all_lower_ready_low();
  set_inputs();

  assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
             .cache_req_ready);
  assert(axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
             .bypass_req_ready);

  for(unsigned step = 0u; step < 4u; step++)
  {
    next_timeframe();
    drive_idle();
    hold_all_lower_ready_low();
    set_inputs();

    const bool ddr_awvalid =
        axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
            .ddr_axi_awvalid;
    const bool mmio_awvalid =
        axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
            .mmio_axi_awvalid;
    check_no_read_escape();
    check_aw_shape(ddr_awvalid, mmio_awvalid);
    if(ddr_awvalid || mmio_awvalid)
    {
      assert(ddr_awvalid && mmio_awvalid);
      seen_aw_together = true;
    }
  }

  assert(seen_aw_together);

  axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
      .ddr_axi_awready = true;
  axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
      .mmio_axi_awready = true;
  axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
      .ddr_axi_wready = false;
  axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
      .mmio_axi_wready = false;
  set_inputs();
  next_timeframe();

  for(unsigned step = 0u; step < 4u; step++)
  {
    drive_idle();
    axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
        .ddr_axi_awready = true;
    axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
        .mmio_axi_awready = true;
    axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
        .ddr_axi_wready = false;
    axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
        .mmio_axi_wready = false;
    set_inputs();

    const bool ddr_wvalid =
        axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
            .ddr_axi_wvalid;
    const bool mmio_wvalid =
        axi_llc_axi_bridge_dual_prod_width_ddr_write_mmio_write_independent_formal_top
            .mmio_axi_wvalid;
    check_no_read_escape();
    if(!seen_w_together)
    {
      check_w_shape(ddr_wvalid, mmio_wvalid, prior_ddr_w_seen);
      if(ddr_wvalid || mmio_wvalid)
      {
        assert(ddr_wvalid && mmio_wvalid);
        seen_w_together = true;
        prior_ddr_w_seen = prior_ddr_w_seen || ddr_wvalid;
      }
    }
    next_timeframe();
  }

  assert(seen_w_together);
}
