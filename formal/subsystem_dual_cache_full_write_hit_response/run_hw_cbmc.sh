...
build_dir="${repo_root}/local_debug/hw_cbmc_subsystem_dual_cache_full_write_hit_response"
...
exec timeout "${timeout_sec}" "${hw_cbmc}" \
  ...
  "${build_dir}/axi_llc_axi_bridge_dual.pre.v" \
  "${build_dir}/axi_llc_subsystem_dual.pre.v" \
  "${script_dir}/axi_llc_subsystem_dual_cache_full_write_hit_response_formal_top.v" \
  --module axi_llc_subsystem_dual_cache_full_write_hit_response_formal_top \
  --bound 56
