...
build_dir="${repo_root}/local_debug/hw_cbmc_dual_bridge_multi_read_outstanding"
timeout_sec="${HW_CBMC_TIMEOUT_SEC:-180}"

mkdir -p "${build_dir}"

{
  printf '`define AXI_LLC_BRIDGE_READ_PENDING_COUNT 2\n'
...
exec timeout "${timeout_sec}" "${hw_cbmc}" \
  ...
  "${build_dir}/axi_llc_axi_bridge_dual.pre.v" \
  "${script_dir}/axi_llc_axi_bridge_dual_multi_read_outstanding_formal_top.v" \
  --module axi_llc_axi_bridge_dual_multi_read_outstanding_formal_top \
  --bound 12
