...
build_dir="${repo_root}/local_debug/hw_cbmc_bridge_prod_width_cacheline_read_response"
...
exec timeout "${timeout_sec}" "${hw_cbmc}" \
  ...
  "${build_dir}/axi_llc_axi_bridge.pre.v" \
  "${script_dir}/axi_llc_axi_bridge_prod_width_cacheline_read_response_formal_top.v" \
