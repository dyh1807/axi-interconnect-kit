...
`formal/cache_ctrl_table_oracle_dirty_evict_then_read`、
`formal/cache_ctrl_table_oracle_write_then_read`、
`formal/cache_ctrl_table_oracle_read_miss_refill_then_read` 和
`formal/cache_ctrl_table_oracle_partial_write_miss_refill_then_read` 四个 cache-control
table-oracle shadow row proof 原型，targeted logs 包括
...
- 初始 tracked set 已满，replacement way 指向 dirty victim；第一笔 full-line write
  miss 必须发出 dirty victim writeback。
- writeback response 后，proof 观测实际 data/meta/valid/repl 写表端口，检查新 dirty
  line 安装到 victim way，并更新 shadow row。
- write response 返回后，第二笔同地址 read 必须从 shadow row 命中并返回新安装 line，
  且不得再次发 lower memory request。
...
### `formal/cache_ctrl_table_oracle_partial_write_miss_refill_then_read`
...
```sh
formal/cache_ctrl_table_oracle_partial_write_miss_refill_then_read/run_hw_cbmc.sh
```
...
- 直接实例化生产 cache-control FSM，formal top 在 data/meta/valid/repl 表边界提供
  一个 tracked set 的 shadow row oracle。
- 初始 tracked set 为空；第一笔 partial write miss 必须先发出整行 lower-memory
  refill read。
- refill response 后，proof 观测实际 data/meta/valid/repl 写表端口，检查按
  request offset/`WSTRB` merge 后安装 valid+dirty line，并更新 shadow row。
- write response 返回后，第二笔同地址 read 必须从 shadow row 命中并返回 merge 后
  line，且不得再次发 lower memory request。
...
- 直接实例化生产 cache-control FSM，formal top 在 data/meta/valid/repl 表边界提供
  一个 tracked set 的 shadow row oracle。
- 初始 tracked set 中 way0 为 clean valid hit；partial write hit 被接受后，proof
  观测实际 data/meta/valid/repl 写表端口并更新 shadow row。
- 后续同地址 read 必须从 shadow row 命中并返回 merge 后的 line，且不得误发 lower
  memory request。
- 该 proof 验证 table write intent 与后续 read-after-write 可观察结果的一致性，
  不需要把整张 cache table 展开为 PI/PO。
