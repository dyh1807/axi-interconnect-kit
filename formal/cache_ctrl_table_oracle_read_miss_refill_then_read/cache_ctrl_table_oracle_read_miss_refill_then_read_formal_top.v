module cache_ctrl_table_oracle_read_miss_refill_then_read_formal_top(
    input             clk,
    input             rst_n,
    input             req_valid,
    output            req_ready,
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            data_row_r <= {FORMAL_DATA_ROW_BITS{1'b0}};
            meta_row_r <= {FORMAL_META_ROW_BITS{1'b0}};
            valid_bits_r <= {FORMAL_WAY_COUNT{1'b0}};
            repl_way_r <= {FORMAL_WAY_BITS{1'b0}};
        end else begin
            if (data_wr_en) begin
