...
build_dir="${repo_root}/local_debug/hw_cbmc_cache_ctrl_table_oracle_read_miss_refill_then_read"
...
exec timeout "${timeout_sec}" "${hw_cbmc}" \
  ...
  "${build_dir}/llc_cache_ctrl.pre.v" \
  "${script_dir}/cache_ctrl_table_oracle_read_miss_refill_then_read_formal_top.v" \
