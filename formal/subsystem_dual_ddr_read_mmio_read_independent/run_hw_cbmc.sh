...
build_dir="${repo_root}/local_debug/hw_cbmc_subsystem_dual_ddr_read_mmio_read_independent"
...
exec timeout "${timeout_sec}" "${hw_cbmc}" \
  ...
  "${build_dir}/axi_llc_axi_bridge_dual.pre.v" \
  "${build_dir}/axi_llc_subsystem_dual.pre.v" \
  "${script_dir}/axi_llc_subsystem_dual_ddr_read_mmio_read_independent_formal_top.v" \
