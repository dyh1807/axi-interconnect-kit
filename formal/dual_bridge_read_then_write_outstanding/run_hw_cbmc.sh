...
build_dir="${repo_root}/local_debug/hw_cbmc_dual_bridge_read_then_write_outstanding"
...
exec timeout "${timeout_sec}" "${hw_cbmc}" \
  ...
  "${build_dir}/axi_llc_axi_bridge_dual.pre.v" \
  "${script_dir}/axi_llc_axi_bridge_dual_read_then_write_outstanding_formal_top.v" \
  --module axi_llc_axi_bridge_dual_read_then_write_outstanding_formal_top \
  --bound 16
