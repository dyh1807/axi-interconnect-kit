...
struct module_axi_llc_axi_bridge_dual_read_then_write_outstanding_formal_top
{
  bool clk;
  bool rst_n;
...
extern struct
    module_axi_llc_axi_bridge_dual_read_then_write_outstanding_formal_top
        axi_llc_axi_bridge_dual_read_then_write_outstanding_formal_top;
...
static void drive_idle(void)
{
  ...
  axi_llc_axi_bridge_dual_read_then_write_outstanding_formal_top
      .cache_req_addr = 0u;
  axi_llc_axi_bridge_dual_read_then_write_outstanding_formal_top.cache_req_id =
      0u;
  axi_llc_axi_bridge_dual_read_then_write_outstanding_formal_top
      .cache_req_size = 7u;
  axi_llc_axi_bridge_dual_read_then_write_outstanding_formal_top
...
{
  assert(!axi_llc_axi_bridge_dual_read_then_write_outstanding_formal_top
              .mmio_axi_arvalid);
  assert(!axi_llc_axi_bridge_dual_read_then_write_outstanding_formal_top
              .cache_resp_valid);

  if(axi_llc_axi_bridge_dual_read_then_write_outstanding_formal_top
         .ddr_axi_arvalid)
  {
    assert(axi_llc_axi_bridge_dual_read_then_write_outstanding_formal_top
               .ddr_axi_araddr == addr);
...
int main(void)
{
  const uint32_t read_addr = 0x40000000u;
  const uint32_t write_addr = 0x40000008u;
  bool seen_ar = false;
  bool seen_aw = false;
  bool seen_w = false;

  axi_llc_axi_bridge_dual_read_then_write_outstanding_formal_top.rst_n = false;
  drive_idle();
  set_inputs();
  next_timeframe();

  axi_llc_axi_bridge_dual_read_then_write_outstanding_formal_top.rst_n = true;
  drive_idle();
  set_inputs();
  next_timeframe();

  drive_read(read_addr, 3u);
  set_inputs();
  assert(axi_llc_axi_bridge_dual_read_then_write_outstanding_formal_top
             .cache_req_ready);
  ...
  for(unsigned step = 0; step < 5u && !seen_ar; step++)
  {
    drive_idle();
    set_inputs();
    sample_read_issue(read_addr, &seen_ar);
  ...
  drive_write(write_addr, 9u, 0x1122334455667788ull);
  set_inputs();
  assert(axi_llc_axi_bridge_dual_read_then_write_outstanding_formal_top
             .cache_req_ready);
