...
build_dir="${repo_root}/local_debug/hw_cbmc_dual_bridge_ddr_multibeat_write"
...

{
  ...
  printf '`define AXI_LLC_LINE_BYTES 16\n'
  printf '`undef AXI_LLC_LINE_BITS\n'
  printf '`define AXI_LLC_LINE_BITS 128\n'
  printf '`undef AXI_LLC_LINE_OFFSET_BITS\n'
  printf '`define AXI_LLC_LINE_OFFSET_BITS 4\n'
  ...
  printf '`define AXI_LLC_READ_RESP_BYTES 16\n'
  printf '`undef AXI_LLC_READ_RESP_BITS\n'
  printf '`define AXI_LLC_READ_RESP_BITS 128\n'
...
exec timeout "${timeout_sec}" "${hw_cbmc}" \
  ...
  "${build_dir}/axi_llc_axi_bridge_dual.pre.v" \
  "${script_dir}/axi_llc_axi_bridge_dual_ddr_multibeat_write_formal_top.v" \
