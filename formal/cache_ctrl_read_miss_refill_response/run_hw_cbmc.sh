...
build_dir="${repo_root}/local_debug/hw_cbmc_cache_ctrl_read_miss_refill_response"
...
exec timeout "${timeout_sec}" "${hw_cbmc}" \
  ...
  "${build_dir}/llc_cache_ctrl.pre.v" \
  "${script_dir}/cache_ctrl_read_miss_refill_response_formal_top.v" \
  --module cache_ctrl_read_miss_refill_response_formal_top \
  --bound 36
