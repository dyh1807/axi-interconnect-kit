`ifndef AXI_LLC_PARAMS_VH
`define AXI_LLC_PARAMS_VH

`define AXI_LLC_ADDR_BITS         32
`define AXI_LLC_ID_BITS           4
`define AXI_LLC_SLOT_ID_BITS      6
`define AXI_LLC_MODE_BITS         2
`define AXI_LLC_LINE_BYTES        64
`define AXI_LLC_LINE_BITS         512
`define AXI_LLC_LINE_OFFSET_BITS  6
`define AXI_LLC_SET_COUNT         8192
`define AXI_LLC_SET_BITS          13
`define AXI_LLC_WAY_COUNT         16
`define AXI_LLC_WAY_BITS          4
`define AXI_LLC_META_BITS         16
`define AXI_LLC_META_SRAM_BITS    16
`define AXI_LLC_META_SRAM_DEPTH   4096
`define AXI_LLC_META_SRAM_ADDR_BITS 12
`define AXI_LLC_LLC_SIZE_BYTES    8388608
`define AXI_LLC_WINDOW_BYTES      4194304
`define AXI_LLC_WINDOW_WAYS       8
`define AXI_LLC_MMIO_BASE         32'h10000000
`define AXI_LLC_MMIO_SIZE         32'h00001000
`define AXI_LLC_AXI_ID_BITS       6
`define AXI_LLC_AXI_DATA_BYTES    32
`define AXI_LLC_AXI_DATA_BITS     256
`define AXI_LLC_AXI_STRB_BITS     32
`define AXI_LLC_TABLE_READ_LATENCY 3
`define AXI_LLC_READ_RESP_BYTES   256
`define AXI_LLC_READ_RESP_BITS    2048
`define AXI_LLC_MAX_OUTSTANDING   32
`define AXI_LLC_MAX_READ_OUTSTANDING_PER_MASTER 32
`define AXI_LLC_MAX_WRITE_OUTSTANDING 32

`ifndef AXI_LLC_BRIDGE_READ_PENDING_COUNT
`define AXI_LLC_BRIDGE_READ_PENDING_COUNT `AXI_LLC_MAX_OUTSTANDING
`endif
`ifndef AXI_LLC_BRIDGE_WRITE_PENDING_COUNT
`define AXI_LLC_BRIDGE_WRITE_PENDING_COUNT `AXI_LLC_MAX_WRITE_OUTSTANDING
`endif

`endif
