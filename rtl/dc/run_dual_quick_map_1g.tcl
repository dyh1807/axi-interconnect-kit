...
axi_llc_init_run dual_quick_map_1g
...
puts "=== DC_STAGE quick_map_start [clock format [clock seconds] -format {%Y-%m-%d %H:%M:%S}] ==="
flush stdout
compile -map_effort medium
puts "=== DC_STAGE quick_map_done [clock format [clock seconds] -format {%Y-%m-%d %H:%M:%S}] ==="
flush stdout
...
flush stdout
redirect -file [file join $rpt_root ${top_name}_quick_map_check_timing.rpt] {check_timing}
redirect -file [file join $rpt_root ${top_name}_quick_map_timing.rpt] {report_timing -delay max -max_paths 1}
redirect -file [file join $rpt_root ${top_name}_quick_map_timing_max20.rpt] {report_timing -delay max -max_paths 20}
redirect -file [file join $rpt_root ${top_name}_quick_map_qor.rpt] {report_qor}
redirect -file [file join $rpt_root ${top_name}_quick_map_area.rpt] {report_area -hierarchy}
