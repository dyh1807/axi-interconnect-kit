...
module llc_mshr_payload_mux_split_timing_probe (
    input  clk,
    output sink
);
    ...
    reg               issue_write_s1_r;
    reg               commit_is_write_s1_r;
    reg               commit_need_refill_s1_r;
    reg               state_mem_req_valid_s1_r;
    reg [LINE_BITS-1:0] state_mem_req_wdata_s1_r;
    reg [LINE_BITS-1:0] issue_victim_data_s1_r;
    reg [ADDR_BITS-1:0] commit_addr_s1_r;
    reg [LINE_BITS-1:0] commit_refill_line_s1_r;
    reg [LINE_BITS-1:0] commit_wdata_s1_r;
    reg [LINE_BYTES-1:0] commit_wstrb_s1_r;
    reg sink_r;
    ...
    wire [LINE_BITS-1:0] commit_merge_line_w =
        merge_line(commit_refill_line_s1_r,
                   commit_addr_s1_r,
                   commit_wdata_s1_r,
                   commit_wstrb_s1_r);
    wire [LINE_BITS-1:0] mem_req_wdata_w =
        state_mem_req_valid_s1_r ? state_mem_req_wdata_s1_r
                                 : issue_victim_data_s1_r;
    wire [LINE_BITS-1:0] install_line_w =
        commit_is_write_s1_r ? (commit_need_refill_s1_r ? commit_merge_line_w
                                                        : commit_wdata_s1_r)
                             : commit_refill_line_s1_r;

    always @(posedge clk) begin
        issue_victim_data_s1_r <= line_at(mshr_victim_data_r, issue_slot_safe_w);
        commit_addr_s1_r <= addr_at(mshr_addr_r, commit_slot_safe_w);
        commit_refill_line_s1_r <= line_at(mshr_refill_line_r, commit_slot_safe_w);
        commit_wdata_s1_r <= line_at(mshr_wdata_r, commit_slot_safe_w);
        commit_wstrb_s1_r <= wstrb_at(mshr_wstrb_r, commit_slot_safe_w);
        issue_write_s1_r <= issue_write_r;
        commit_is_write_s1_r <= commit_is_write_r;
        commit_need_refill_s1_r <= commit_need_refill_r;
        state_mem_req_valid_s1_r <= state_mem_req_valid_r;
        state_mem_req_wdata_s1_r <= state_mem_req_wdata_r;
        ...
        commit_is_write_r <= commit_is_write_r ^ issue_write_s1_r;
        commit_need_refill_r <= commit_need_refill_r ^ commit_is_write_s1_r;
        state_mem_req_valid_r <= state_mem_req_valid_r ^ commit_need_refill_s1_r;
        ...
        mshr_wstrb_r <= {mshr_wstrb_r[(MSHR_COUNT*LINE_BYTES)-2:0],
                         ^commit_wdata_s1_r};
        sink_r <= ^mem_req_wdata_w ^ ^install_line_w ^ issue_write_s1_r;
    end
endmodule
