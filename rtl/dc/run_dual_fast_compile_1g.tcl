...
axi_llc_init_run dual_fast_compile_1g
...
puts "=== DC_STAGE fast_reports_start [clock format [clock seconds] -format {%Y-%m-%d %H:%M:%S}] ==="
flush stdout
redirect -file [file join $rpt_root ${top_name}_fast_postcompile_check_timing.rpt] {check_timing}
redirect -file [file join $rpt_root ${top_name}_fast_postcompile_timing.rpt] {report_timing -delay max -max_paths 1}
redirect -file [file join $rpt_root ${top_name}_fast_postcompile_timing_max20.rpt] {report_timing -delay max -max_paths 20}
redirect -file [file join $rpt_root ${top_name}_fast_postcompile_qor.rpt] {report_qor}
redirect -file [file join $rpt_root ${top_name}_fast_postcompile_area.rpt] {report_area -hierarchy}
puts "=== DC_STAGE fast_reports_done [clock format [clock seconds] -format {%Y-%m-%d %H:%M:%S}] ==="
flush stdout
