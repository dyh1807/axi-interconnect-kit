...
module llc_valid_ram #(
    ...
    localparam integer USE_BANKED_VALID_RAM =
        (SET_COUNT == 8192) &&
        (SET_BITS == 13) &&
        (WAY_COUNT == 16) &&
    ...
    generate
        if (USE_BANKED_VALID_RAM) begin : gen_banked
            ...
            always @(posedge clk or negedge rst_n) begin
                if (!rst_n) begin
                    // valid_mem contents are cleared by the startup /
                    // reconfiguration invalidate sweep rather than an
                    // array-wide reset.
                    rd_valid <= 1'b0;
                    rd_bits <= {WAY_COUNT{1'b0}};
                    read_pending_r <= 1'b0;
                    ...
                    if (wr_en) begin
                        valid_mem[wr_set] <= (valid_mem[wr_set] & (~wr_mask)) |
                                             (wr_bits & wr_mask);
                    end
                end
            end
