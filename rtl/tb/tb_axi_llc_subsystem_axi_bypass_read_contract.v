...
module tb_axi_llc_subsystem_axi_bypass_read_contract;
    ...
    task fail_now;
        input [8*160-1:0] msg;
        begin
            $display("tb_axi_llc_subsystem_axi_bypass_read_contract FAIL: %0s", msg);
            $finish(1);
        end
    endtask
    ...
    task issue_bypass_read;
        integer timeout;
        begin
            read_req_addr[ADDR_BITS-1:0] = BYPASS_ADDR;
            ...
            if (!(read_req_valid[0] && read_req_ready[0])) begin
                fail_now("bypass read request handshake timeout");
            end
            #1;
    ...
    task wait_ar_contract;
        ...
        begin
            ...
            if (timeout == 0) begin
                fail_now("bypass read AR timeout");
            end
            if (axi_araddr !== BYPASS_ADDR) begin
                fail_now("bypass read AR address mismatch");
            end
            if (axi_arlen !== 8'd0) begin
                fail_now("bypass 4B read must use single-beat AR");
            end
            if (axi_arsize !== AXI_SIZE_32B) begin
                fail_now("bypass 4B read must use fixed downstream beat size");
            end
            if (axi_arburst !== AXI_BURST_INCR) begin
                fail_now("bypass read burst must be INCR");
            end
            if (axi_awvalid || axi_wvalid) begin
                fail_now("bypass read unexpectedly drove AXI write channels");
            end
            seen_arid = axi_arid;
            axi_arready = 1'b1;
            @(posedge clk);
    ...
    initial begin
        ...
        expected_word = 32'hA5A5_1C2D;
        axi_rdata[31:0] = expected_word;
        ...
        if (ar_count !== 1) begin
            fail_now("bypass read should emit exactly one AR");
        end
        if (r_count < 1) begin
            fail_now("bypass read should consume at least one R beat");
        end
        if (config_error) begin
            fail_now("config_error asserted unexpectedly");
        end

        $display("tb_axi_llc_subsystem_axi_bypass_read_contract PASS");
        $finish(0);
    end
