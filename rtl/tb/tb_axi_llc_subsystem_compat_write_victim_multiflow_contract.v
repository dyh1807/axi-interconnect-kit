...
module tb_axi_llc_subsystem_compat_write_victim_multiflow_contract;
    ...
    localparam [ADDR_BITS-1:0] OTHER_READ_ADDR = 32'h0000_0008;
    localparam [ADDR_BITS-1:0] WRITE_MISS_ADDR = 32'h0000_0010;
    localparam [ID_BITS-1:0]   FILL_ID         = 4'h1;
    localparam [ID_BITS-1:0]   DIRTY_WRITE_ID  = 4'h2;
    localparam [ID_BITS-1:0]   WRITE_MISS_ID   = 4'h3;
    localparam [ID_BITS-1:0]   OTHER_READ_ID   = 4'h4;
    ...
    reg  [LINE_BITS-1:0]                  write_miss_line;
    reg  [LINE_BITS-1:0]                  other_read_line;
    reg  [LINE_BITS-1:0]                  observed_read_line;
    reg  [ID_BITS-1:0]                    lower_wb_id;
    reg  [ID_BITS-1:0]                    lower_read_id;
    integer                               timeout;
    ...
    task fail_now;
        input [8*160-1:0] msg;
        begin
            $display("tb_axi_llc_subsystem_compat_write_victim_multiflow_contract FAIL: %0s", msg);
            $finish(1);
        end
    endtask
    ...
    task wait_write_resp;
        ...
        begin
            ...
            if (get_write_resp_id(0) !== expect_id) begin
    ...
    initial begin
        ...
        dirty_line = make_line(8'h80);
        write_miss_line = make_line(8'hC0);
        other_read_line = make_line(8'h40);
        observed_read_line = {LINE_BITS{1'b0}};
        lower_wb_id = {ID_BITS{1'b0}};
        lower_read_id = {ID_BITS{1'b0}};
        ...
        wait_cache_req(1'b0, VICTIM_ADDR, lower_read_id);
        drive_cache_resp(lower_read_id, victim_line);
        wait_read_resp(FILL_ID, victim_line);

        issue_write(VICTIM_ADDR, DIRTY_WRITE_ID, dirty_line, {LINE_BYTES{1'b1}});
        wait_write_resp(DIRTY_WRITE_ID);

        issue_write(WRITE_MISS_ADDR, WRITE_MISS_ID, write_miss_line, {LINE_BYTES{1'b1}});
        wait_cache_req(1'b1, VICTIM_ADDR, lower_wb_id);

        issue_read(OTHER_READ_ADDR, OTHER_READ_ID);
        wait_cache_req(1'b0, OTHER_READ_ADDR, lower_read_id);

        drive_cache_resp(lower_wb_id, {LINE_BITS{1'b0}});
        wait_write_resp(WRITE_MISS_ID);

        drive_cache_resp(lower_read_id, other_read_line);
        wait_read_resp(OTHER_READ_ID, other_read_line);

        if (bypass_req_valid) begin
            fail_now("unexpected bypass activity");
        end

        $display("tb_axi_llc_subsystem_compat_write_victim_multiflow_contract PASS");
        $finish(0);
    end
