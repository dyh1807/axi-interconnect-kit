...
module tb_axi_llc_subsystem_axi_cache_refill_contract;
    ...
    localparam [ID_BITS-1:0] READ_ID = 4'h6;
    ...
    reg  [AXI_DATA_BITS-1:0]        beat0_data;
    reg  [AXI_DATA_BITS-1:0]        beat1_data;
    reg  [LINE_BITS-1:0]            expected_line;
    reg  [LINE_BITS-1:0]            read_line;
    integer                         byte_idx;
    ...
    task fail_now;
        input [8*160-1:0] msg;
        begin
            $display("tb_axi_llc_subsystem_axi_cache_refill_contract FAIL: %0s", msg);
            $finish(1);
        end
    endtask
    ...
    task issue_cache_read;
        integer timeout;
        begin
            read_req_addr[ADDR_BITS-1:0] = CACHE_ADDR;
            ...
            if (!(read_req_valid[0] && read_req_ready[0])) begin
                fail_now("cache read request handshake timeout");
            end
            #1;
    ...
    task wait_ar_contract;
        ...
        begin
            ...
            if (timeout == 0) begin
                fail_now("cache refill AR timeout");
            end
            if (axi_awvalid || axi_wvalid) begin
                fail_now("cache refill unexpectedly drove AW/W");
            end
            if (axi_araddr !== CACHE_ADDR) begin
                fail_now("cache refill AR address mismatch");
            end
            if (axi_arlen !== 8'd1) begin
                fail_now("cache refill AR len should be 1 for 64B");
            end
            if (axi_arsize !== AXI_SIZE_32B) begin
                fail_now("cache refill AR size should be 32B beat");
            end
            if (axi_arburst !== AXI_BURST_INCR) begin
                fail_now("cache refill AR burst should be INCR");
            end
            seen_arid = axi_arid;
            axi_arready = 1'b1;
            @(posedge clk);
    ...
    initial begin
        ...
        beat0_data = {AXI_DATA_BITS{1'b0}};
        beat1_data = {AXI_DATA_BITS{1'b0}};
        for (byte_idx = 0; byte_idx < AXI_DATA_BYTES; byte_idx = byte_idx + 1) begin
            beat0_data[(byte_idx * 8) +: 8] = 8'h10 + byte_idx[7:0];
            beat1_data[(byte_idx * 8) +: 8] = 8'h80 + byte_idx[7:0];
        end
        expected_line = assemble_two_beats(beat0_data, beat1_data);
        ...
        issue_cache_read();
        wait_ar_contract();
        drive_r_beat(beat0_data, 1'b0);
        drive_r_beat(beat1_data, 1'b1);
        wait_read_response();
        wait_cycles(1);

        if (ar_count !== 1) begin
            fail_now("cache refill should emit exactly one AR");
        end
        if (r_count < 2) begin
            fail_now("cache refill should consume at least two R beats");
        end
        if ((aw_count !== 0) || (w_count !== 0) || (b_count !== 0)) begin
            fail_now("cache refill must not use AXI write channels");
        end
        if (config_error) begin
            fail_now("config_error asserted unexpectedly");
        end

        $display("tb_axi_llc_subsystem_axi_cache_refill_contract PASS");
        $finish(0);
    end
