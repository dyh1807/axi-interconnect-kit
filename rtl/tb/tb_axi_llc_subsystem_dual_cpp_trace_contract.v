...
`include "axi_dual_cpp_trace_vectors.vh"
    ...
    task issue_mode1_to_mode2_pending_mmio_write_and_check;
        input [31:0] req_addr;
        input [7:0] req_size;
        input [3:0] req_id;
        input [511:0] req_wdata;
        ...
        begin
            ...
            if (timeout == 0) begin
                fail_now("C++ trace pending mode transition did not issue MMIO AW");
            end
            #1;
            expect_no_ddr_activity();
            if (mmio_axi_awaddr != exp_awaddr ||
                ...
                mmio_axi_awburst != exp_awburst ||
                mmio_axi_awid != exp_awid) begin
                fail_now("C++ trace pending mode transition MMIO AW mismatch");
            end
            seen_mmio_awid = mmio_axi_awid;
            mmio_axi_awready = 1'b1;
            @(posedge clk);
            ...
            if (!accepted_seen) begin
                fail_now("C++ trace pending mode transition write accepted pulse missing");
            end
            if (active_mode == MODE_MAPPED) begin
                fail_now("mode transition completed before MMIO write pending check");
            end
            @(negedge clk);
            write_req_valid[WRITE_MASTER] = 1'b0;
            write_req_bypass[WRITE_MASTER] = 1'b0;
            ...
            if (timeout == 0) begin
                fail_now("C++ trace pending mode transition first W timeout");
            end
            #1;
            expect_no_ddr_activity();
            if (mmio_axi_wdata != wbeat0[MMIO_DATA_BITS-1:0] ||
                mmio_axi_wstrb != wstrb0[MMIO_STRB_BITS-1:0] ||
                mmio_axi_wlast != wlast0) begin
                fail_now("C++ trace pending mode transition first W mismatch");
            end
            mmio_axi_wready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            ...
            if (beat_count == 2) begin
                ...
                if (timeout == 0) begin
                    fail_now("C++ trace pending mode transition second W timeout");
                end
                #1;
                expect_no_ddr_activity();
                if (mmio_axi_wdata != wbeat1[MMIO_DATA_BITS-1:0] ||
                    mmio_axi_wstrb != wstrb1[MMIO_STRB_BITS-1:0] ||
                    mmio_axi_wlast != wlast1) begin
                    fail_now("C++ trace pending mode transition second W mismatch");
                end
                mmio_axi_wready = 1'b1;
                @(posedge clk);
                @(negedge clk);
            ...
            repeat (4) begin
                ...
                #1;
                if (active_mode == MODE_MAPPED) begin
                    fail_now("mode transition completed while MMIO write pending");
                end
            end
            ...
            #1;
            if (!mmio_axi_bready) begin
                fail_now("C++ trace pending mode transition B was backpressured");
            end
            if (active_mode == MODE_MAPPED) begin
                fail_now("mode transition completed in MMIO B handshake cycle");
            end
            @(posedge clk);
            @(negedge clk);
            mmio_axi_bvalid = 1'b0;
            ...
            while (!write_resp_valid[WRITE_MASTER] && (timeout > 0)) begin
                ...
                #1;
                if (active_mode == MODE_MAPPED) begin
                    fail_now("mode transition completed before held write response");
                end
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace pending mode transition write response timeout");
            end
            #1;
            if (write_resp_id[(WRITE_MASTER * ID_BITS) +: ID_BITS] != exp_resp_id ||
                write_resp_code[(WRITE_MASTER * 2) +: 2] != exp_resp_code) begin
                fail_now("C++ trace pending mode transition write response mismatch");
            end
            repeat (4) begin
                @(posedge clk);
                #1;
                if (active_mode == MODE_MAPPED) begin
                    fail_now("mode transition completed while write response held");
                end
            end
            ...
            if (write_resp_id[(WRITE_MASTER * ID_BITS) +: ID_BITS] != exp_resp_id ||
                write_resp_code[(WRITE_MASTER * 2) +: 2] != exp_resp_code) begin
                fail_now("C++ trace pending mode transition retire write response mismatch");
            end
            if (active_mode == MODE_MAPPED) begin
                fail_now("mode transition completed before write response retire edge");
            end
            @(posedge clk);
            @(negedge clk);
            write_resp_ready[WRITE_MASTER] = 1'b0;
            ...
            if (timeout == 0) begin
                fail_now("mode transition did not complete after MMIO write retired");
            end
            #1;
            if (active_offset != 32'h3000_0000) begin
                fail_now("mode transition active offset mismatch after pending MMIO write");
            end
        end
    endtask
    ...
    task issue_mode1_to_mode2_pending_cache_read_and_check;
        integer timeout;
        reg accepted_seen;
        begin
            reset_dut();
            ...
            read_req_addr[(CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_REQ_ADDR;
            read_req_total_size[(CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_MASTER * 8) +: 8] =
                CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_REQ_SIZE;
            read_req_id[(CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_REQ_ID;
            read_req_bypass[CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_MASTER] = 1'b0;
            read_req_valid[CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_MASTER] = 1'b1;
            ...
            while (!accepted_seen && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (read_req_accepted[CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (!accepted_seen) begin
                fail_now("C++ trace mode1-to-mode2 cache read accept timeout");
            end
            read_req_valid[CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_MASTER] = 1'b0;
            @(negedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace mode1-to-mode2 cache read AR timeout");
            end
            #1;
            if (ddr_axi_araddr != CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_ARADDR ||
                ddr_axi_arlen != CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_ARLEN ||
                ddr_axi_arsize != CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_ARSIZE ||
                ddr_axi_arburst != CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_ARBURST ||
                ddr_axi_arid != CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_ARID ||
                mmio_axi_arvalid) begin
                fail_now("C++ trace mode1-to-mode2 cache read DDR AR mismatch");
            end
            seen_ddr_arid = ddr_axi_arid;
            ddr_axi_arready = 1'b1;
            @(posedge clk);
            ...
            repeat (4) begin
                ...
                #1;
                if (active_mode == MODE_MAPPED) begin
                    fail_now("mode transition completed while cache read pending");
                end
            end

            ddr_axi_rid = seen_ddr_arid;
            ddr_axi_rdata = CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_RBEAT0;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b0;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !==
                    CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_RREADY_PENDING ||
                active_mode == MODE_MAPPED) begin
                fail_now("C++ trace mode1-to-mode2 cache read R beat0 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            ddr_axi_rid = seen_ddr_arid;
            ddr_axi_rdata = CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_RBEAT1;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b1;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !==
                    CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_RREADY_PENDING ||
                active_mode == MODE_MAPPED) begin
                fail_now("C++ trace mode1-to-mode2 cache read R beat1 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            timeout = 240;
            while (!read_resp_valid[CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_MASTER] &&
                   (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (active_mode == MODE_MAPPED) begin
                    fail_now("mode transition completed before cache response");
                end
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace mode1-to-mode2 cache read response timeout");
            end
            #1;
            if (read_resp_id[(CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_RESP_ID ||
                read_resp_data[(CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_RESP_DATA) begin
                fail_now("C++ trace mode1-to-mode2 cache read response mismatch");
            end
            ...
            repeat (4) begin
                ...
                #1;
                if (active_mode == MODE_MAPPED) begin
                    fail_now("mode transition completed while cache response held");
                end
            end

            @(negedge clk);
            read_resp_ready[CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_MASTER] = 1'b1;
            #1;
            if (read_resp_id[(CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_RESP_ID ||
                read_resp_data[(CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_RESP_DATA ||
                active_mode == MODE_MAPPED) begin
                fail_now("C++ trace mode1-to-mode2 cache read retire mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            read_resp_ready[CPP_MODE1_TO_MODE2_PENDING_CACHE_READ_MASTER] = 1'b0;
            ...
            if (timeout == 0) begin
                fail_now("mode transition did not complete after cache read retired");
            end
            #1;
            if (active_offset != 32'h3000_0000) begin
                fail_now("mode transition active offset mismatch after cache read");
            end
        end
    endtask
    ...
    task issue_mode1_invalidate_line_cache_mmio_write_and_check;
        integer timeout;
        reg accepted_seen;
        begin
            reset_dut();
            ...
            mmio_axi_wready = 1'b0;
            invalidate_line_valid = 1'b0;
            invalidate_line_addr = CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_INVALIDATE_ADDR;

            write_req_addr[(CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_REQ_ADDR;
            write_req_total_size[(CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_MASTER * 8) +: 8] =
                CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_REQ_SIZE;
            write_req_id[(CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_REQ_ID;
            write_req_wdata[(CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_MASTER * LINE_BITS) +: LINE_BITS] =
                CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_REQ_WDATA;
            write_req_wstrb[(CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_MASTER * LINE_BYTES) +: LINE_BYTES] =
                CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_REQ_WSTRB[LINE_BYTES-1:0];
            write_req_bypass[CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_MASTER] = 1'b0;
            write_req_valid[CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_MASTER] = 1'b1;
            timeout = 180;
            accepted_seen = 1'b0;
            while (!accepted_seen && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (write_req_accepted[CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (!accepted_seen) begin
                fail_now("C++ trace invline cache/MMIO write cache accept timeout");
            end
            write_req_valid[CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_MASTER] = 1'b0;
            @(negedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace invline cache/MMIO write refill AR timeout");
            end
            #1;
            if (ddr_axi_araddr != CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_REFILL_ARADDR ||
                ddr_axi_arlen != CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_REFILL_ARLEN ||
                ddr_axi_arsize != CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_REFILL_ARSIZE ||
                ddr_axi_arburst != CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_REFILL_ARBURST ||
                ddr_axi_arid != CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_REFILL_ARID ||
                mmio_axi_arvalid || ddr_axi_awvalid || ddr_axi_wvalid) begin
                fail_now("C++ trace invline cache/MMIO write refill AR mismatch");
            end
            seen_ddr_arid = ddr_axi_arid;
            ddr_axi_arready = 1'b1;
            @(posedge clk);
            ...
            write_req_addr[(CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_REQ_ADDR;
            write_req_total_size[(CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_MASTER * 8) +: 8] =
                CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_REQ_SIZE;
            write_req_id[(CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_REQ_ID;
            write_req_wdata[(CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_MASTER * LINE_BITS) +: LINE_BITS] =
                CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_REQ_WDATA;
            write_req_wstrb[(CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_MASTER * LINE_BYTES) +: LINE_BYTES] =
                CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_REQ_WSTRB[LINE_BYTES-1:0];
            write_req_bypass[CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_MASTER] = 1'b0;
            write_req_valid[CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_MASTER] = 1'b1;
            timeout = 180;
            accepted_seen = 1'b0;
            while (!accepted_seen && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (write_req_accepted[CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (!accepted_seen) begin
                fail_now("C++ trace invline cache/MMIO write MMIO accept timeout");
            end
            write_req_valid[CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_MASTER] = 1'b0;
            @(negedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace invline cache/MMIO write MMIO AW timeout");
            end
            #1;
            if (mmio_axi_awaddr != CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_AWADDR ||
                mmio_axi_awlen != CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_AWLEN ||
                mmio_axi_awsize != CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_AWSIZE ||
                mmio_axi_awburst != CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_AWBURST ||
                mmio_axi_awid != CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_AWID ||
                ddr_axi_awvalid) begin
                fail_now("C++ trace invline cache/MMIO write MMIO AW mismatch");
            end
            seen_mmio_awid = mmio_axi_awid;
            mmio_axi_awready = 1'b1;
            @(posedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace invline cache/MMIO write MMIO W timeout");
            end
            #1;
            if (mmio_axi_wdata !=
                    CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_WBEAT0[MMIO_DATA_BITS-1:0] ||
                mmio_axi_wstrb !=
                    CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_WSTRB0[MMIO_STRB_BITS-1:0] ||
                mmio_axi_wlast != CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_WLAST0 ||
                ddr_axi_wvalid) begin
                fail_now("C++ trace invline cache/MMIO write MMIO W mismatch");
            end
            mmio_axi_wready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            ...
            mmio_axi_bvalid = 1'b1;
            #1;
            if (mmio_axi_bready !== CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_BREADY_STALLED ||
                mmio_axi_bready !== CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_BREADY_PENDING ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline cache/MMIO write MMIO B mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            mmio_axi_bvalid = 1'b0;
            ...
            timeout = 140;
            while (!write_resp_valid[CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_MASTER] &&
                   (timeout > 0)) begin
                #1;
                if (invalidate_line_accepted) begin
                    fail_now("C++ trace invline cache/MMIO write accepted before MMIO response");
                end
                @(posedge clk);
                timeout = timeout - 1;
            end
            if (timeout == 0 ||
                write_resp_valid[CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_MASTER]) begin
                fail_now("C++ trace invline cache/MMIO write MMIO response owner mismatch");
            end
            #1;
            if (write_resp_id[(CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_RESP_ID ||
                write_resp_code[(CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_MASTER * 2) +: 2] !=
                    CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_RESP_CODE ||
                invalidate_line_accepted !==
                    !CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_BLOCKED_MMIO_HELD) begin
                fail_now("C++ trace invline cache/MMIO write MMIO response hold mismatch");
            end

            @(negedge clk);
            ddr_axi_rid = seen_ddr_arid;
            ddr_axi_rdata = CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_REFILL_RBEAT0;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b0;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_REFILL_RREADY_STALLED ||
                ddr_axi_rready !== CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_DDR_RREADY_MMIO_HELD ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline cache/MMIO write refill R beat0 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            ddr_axi_rid = seen_ddr_arid;
            ddr_axi_rdata = CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_REFILL_RBEAT1;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b1;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_REFILL_RREADY_STALLED ||
                ddr_axi_rready !== CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_DDR_RREADY_MMIO_HELD ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline cache/MMIO write refill R beat1 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            timeout = 260;
            while (!write_resp_valid[CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_MASTER] &&
                   (timeout > 0)) begin
                #1;
                if (!write_resp_valid[CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_MASTER] ||
                    invalidate_line_accepted) begin
                    fail_now("C++ trace invline cache/MMIO write response drain ordering mismatch");
                end
                @(posedge clk);
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace invline cache/MMIO write cache response timeout");
            end
            #1;
            if (write_resp_id[(CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_RESP_ID ||
                write_resp_code[(CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_MASTER * 2) +: 2] !=
                    CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_RESP_CODE ||
                invalidate_line_accepted !==
                    !CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_BLOCKED_CACHE_HELD) begin
                fail_now("C++ trace invline cache/MMIO write cache response hold mismatch");
            end

            write_resp_ready[CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_MASTER] = 1'b1;
            #1;
            if (invalidate_line_accepted) begin
                fail_now("C++ trace invline cache/MMIO write accepted before MMIO retire");
            end
            @(posedge clk);
            @(negedge clk);
            write_resp_ready[CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_MMIO_MASTER] = 1'b0;

            write_resp_ready[CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_MASTER] = 1'b1;
            #1;
            if (invalidate_line_accepted) begin
                fail_now("C++ trace invline cache/MMIO write accepted before cache retire");
            end
            @(posedge clk);
            @(negedge clk);
            write_resp_ready[CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_CACHE_MASTER] = 1'b0;

            if (CPP_MODE1_INVLINE_CACHE_MMIO_WRITE_ACCEPTED_AFTER_RETIRE) begin
                timeout = 10000;
                accepted_seen = 1'b0;
                while (!accepted_seen && (timeout > 0)) begin
                    #1;
                ...
                if (!accepted_seen) begin
                    fail_now("C++ trace invline cache/MMIO write final accept timeout");
                end
            end else begin
                timeout = 64;
                while (timeout > 0) begin
                    #1;
                    if (invalidate_line_accepted) begin
                        fail_now("C++ trace invline cache/MMIO write accepted despite dirty line");
                    end
                    @(posedge clk);
                    timeout = timeout - 1;
                end
    ...
    task issue_mode1_invalidate_line_cache_mmio_rw_and_check;
        integer timeout;
        reg accepted_seen;
        begin
            reset_dut();
            ...
            mmio_axi_wready = 1'b0;
            invalidate_line_valid = 1'b0;
            invalidate_line_addr = CPP_MODE1_INVLINE_CACHE_MMIO_RW_INVALIDATE_ADDR;

            read_req_addr[(CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_REQ_ADDR;
            read_req_total_size[(CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_MASTER * 8) +: 8] =
                CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_REQ_SIZE;
            read_req_id[(CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_REQ_ID;
            read_req_bypass[CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_MASTER] = 1'b0;
            read_req_valid[CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_MASTER] = 1'b1;
            timeout = 180;
            accepted_seen = 1'b0;
            while (!accepted_seen && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (read_req_accepted[CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (!accepted_seen) begin
                fail_now("C++ trace invline cache/MMIO RW cache read accept timeout");
            end
            read_req_valid[CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_MASTER] = 1'b0;
            @(negedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace invline cache/MMIO RW DDR refill AR timeout");
            end
            #1;
            if (ddr_axi_araddr != CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_ARADDR ||
                ddr_axi_arlen != CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_ARLEN ||
                ddr_axi_arsize != CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_ARSIZE ||
                ddr_axi_arburst != CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_ARBURST ||
                ddr_axi_arid != CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_ARID ||
                mmio_axi_arvalid) begin
                fail_now("C++ trace invline cache/MMIO RW DDR refill AR mismatch");
            end
            seen_ddr_arid = ddr_axi_arid;
            ddr_axi_arready = 1'b1;
            @(posedge clk);
            ...
            read_req_addr[(CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_REQ_ADDR;
            read_req_total_size[(CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_MASTER * 8) +: 8] =
                CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_REQ_SIZE;
            read_req_id[(CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_REQ_ID;
            read_req_bypass[CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_MASTER] = 1'b0;
            read_req_valid[CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_MASTER] = 1'b1;
            timeout = 180;
            accepted_seen = 1'b0;
            while (!accepted_seen && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (read_req_accepted[CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (!accepted_seen) begin
                fail_now("C++ trace invline cache/MMIO RW MMIO read accept timeout");
            end
            read_req_valid[CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_MASTER] = 1'b0;
            @(negedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace invline cache/MMIO RW MMIO AR timeout");
            end
            #1;
            if (mmio_axi_araddr != CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_ARADDR ||
                mmio_axi_arlen != CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_ARLEN ||
                mmio_axi_arsize != CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_ARSIZE ||
                mmio_axi_arburst != CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_ARBURST ||
                mmio_axi_arid != CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_ARID ||
                ddr_axi_arvalid) begin
                fail_now("C++ trace invline cache/MMIO RW MMIO AR mismatch");
            end
            seen_mmio_arid = mmio_axi_arid;
            mmio_axi_arready = 1'b1;
            @(posedge clk);
            ...
            write_req_addr[(CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_REQ_ADDR;
            write_req_total_size[(CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_MASTER * 8) +: 8] =
                CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_REQ_SIZE;
            write_req_id[(CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_REQ_ID;
            write_req_wdata[(CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_MASTER * LINE_BITS) +: LINE_BITS] =
                CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_REQ_WDATA;
            write_req_wstrb[(CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_MASTER * LINE_BYTES) +: LINE_BYTES] =
                CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_REQ_WSTRB[LINE_BYTES-1:0];
            write_req_bypass[CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_MASTER] = 1'b0;
            write_req_valid[CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_MASTER] = 1'b1;
            timeout = 180;
            accepted_seen = 1'b0;
            while (!accepted_seen && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (write_req_accepted[CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (!accepted_seen) begin
                fail_now("C++ trace invline cache/MMIO RW MMIO write accept timeout");
            end
            write_req_valid[CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_MASTER] = 1'b0;
            @(negedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace invline cache/MMIO RW MMIO AW timeout");
            end
            #1;
            if (mmio_axi_awaddr != CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_AWADDR ||
                mmio_axi_awlen != CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_AWLEN ||
                mmio_axi_awsize != CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_AWSIZE ||
                mmio_axi_awburst != CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_AWBURST ||
                mmio_axi_awid != CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_AWID ||
                ddr_axi_awvalid) begin
                fail_now("C++ trace invline cache/MMIO RW MMIO AW mismatch");
            end
            seen_mmio_awid = mmio_axi_awid;
            mmio_axi_awready = 1'b1;
            @(posedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace invline cache/MMIO RW MMIO W timeout");
            end
            #1;
            if (mmio_axi_wdata !=
                    CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_WBEAT0[MMIO_DATA_BITS-1:0] ||
                mmio_axi_wstrb !=
                    CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_WSTRB0[MMIO_STRB_BITS-1:0] ||
                mmio_axi_wlast != CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_WLAST0 ||
                ddr_axi_wvalid) begin
                fail_now("C++ trace invline cache/MMIO RW MMIO W mismatch");
            end
            mmio_axi_wready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            ...
            invalidate_line_valid = 1'b1;
            mmio_axi_rid = seen_mmio_arid;
            mmio_axi_rdata = CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_RBEAT0[MMIO_DATA_BITS-1:0];
            mmio_axi_rresp = AXI_RESP_OKAY;
            mmio_axi_rlast = 1'b1;
            mmio_axi_rvalid = 1'b1;
            mmio_axi_bid = seen_mmio_awid;
            ...
            mmio_axi_bvalid = 1'b1;
            #1;
            if (mmio_axi_rready !== CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_RREADY_STALLED ||
                mmio_axi_rready !== CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_RREADY_PENDING ||
                mmio_axi_bready !== CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_BREADY_STALLED ||
                mmio_axi_bready !== CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_BREADY_PENDING ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline cache/MMIO RW MMIO R/B mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            mmio_axi_rvalid = 1'b0;
            ...
            timeout = 180;
            while ((!read_resp_valid[CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_MASTER] ||
                    !write_resp_valid[CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_MASTER]) &&
                   (timeout > 0)) begin
                #1;
                if (invalidate_line_accepted) begin
                    fail_now("C++ trace invline cache/MMIO RW accepted before MMIO responses");
                end
                @(posedge clk);
                timeout = timeout - 1;
            end
            if (timeout == 0 ||
                read_resp_valid[CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_MASTER]) begin
                fail_now("C++ trace invline cache/MMIO RW MMIO response owner mismatch");
            end
            #1;
            if (read_resp_id[(CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_RESP_ID ||
                read_resp_data[(CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_RESP_DATA ||
                write_resp_id[(CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_RESP_ID ||
                write_resp_code[(CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_MASTER * 2) +: 2] !=
                    CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_RESP_CODE ||
                invalidate_line_accepted !==
                    !CPP_MODE1_INVLINE_CACHE_MMIO_RW_BLOCKED_MMIO_HELD) begin
                fail_now("C++ trace invline cache/MMIO RW held MMIO response mismatch");
            end

            @(negedge clk);
            ddr_axi_rid = seen_ddr_arid;
            ddr_axi_rdata = CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_RBEAT0;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b0;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_RREADY_STALLED ||
                ddr_axi_rready !== CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_RREADY_MMIO_HELD ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline cache/MMIO RW DDR R beat0 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            ddr_axi_rid = seen_ddr_arid;
            ddr_axi_rdata = CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_RBEAT1;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b1;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_RREADY_STALLED ||
                ddr_axi_rready !== CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_RREADY_MMIO_HELD ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline cache/MMIO RW DDR R beat1 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            timeout = 260;
            while (!read_resp_valid[CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_MASTER] &&
                   (timeout > 0)) begin
                #1;
                if (!read_resp_valid[CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_MASTER] ||
                    !write_resp_valid[CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_MASTER] ||
                    invalidate_line_accepted) begin
                    fail_now("C++ trace invline cache/MMIO RW response drain ordering mismatch");
                end
                @(posedge clk);
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace invline cache/MMIO RW cache response timeout");
            end
            #1;
            if (read_resp_id[(CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_RESP_ID ||
                read_resp_data[(CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_RESP_DATA ||
                invalidate_line_accepted !==
                    !CPP_MODE1_INVLINE_CACHE_MMIO_RW_BLOCKED_CACHE_HELD) begin
                fail_now("C++ trace invline cache/MMIO RW cache response hold mismatch");
            end

            read_resp_ready[CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_MASTER] = 1'b1;
            #1;
            if (invalidate_line_accepted) begin
                fail_now("C++ trace invline cache/MMIO RW accepted before MMIO read retire");
            end
            @(posedge clk);
            @(negedge clk);
            read_resp_ready[CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_READ_MASTER] = 1'b0;

            write_resp_ready[CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_MASTER] = 1'b1;
            #1;
            if (invalidate_line_accepted) begin
                fail_now("C++ trace invline cache/MMIO RW accepted before MMIO write retire");
            end
            @(posedge clk);
            @(negedge clk);
            write_resp_ready[CPP_MODE1_INVLINE_CACHE_MMIO_RW_MMIO_WRITE_MASTER] = 1'b0;

            read_resp_ready[CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_MASTER] = 1'b1;
            #1;
            if (invalidate_line_accepted) begin
                fail_now("C++ trace invline cache/MMIO RW accepted before cache retire");
            end
            @(posedge clk);
            @(negedge clk);
            read_resp_ready[CPP_MODE1_INVLINE_CACHE_MMIO_RW_DDR_MASTER] = 1'b0;
            ...
            if (accepted_seen !==
                CPP_MODE1_INVLINE_CACHE_MMIO_RW_ACCEPTED_AFTER_RETIRE) begin
                fail_now("C++ trace invline cache/MMIO RW final accept mismatch");
            end
            @(negedge clk);
            invalidate_line_valid = 1'b0;
            invalidate_line_addr = {ADDR_BITS{1'b0}};
    ...
    task issue_mode1_invalidate_line_other_cache_mmio_rw_and_check;
        integer timeout;
        reg accepted_seen;
        begin
            reset_dut();
            ...
            mmio_axi_wready = 1'b0;
            invalidate_line_valid = 1'b0;
            invalidate_line_addr = CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_INVALIDATE_ADDR;

            read_req_addr[(CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_REQ_ADDR;
            read_req_total_size[(CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_MASTER * 8) +: 8] =
                CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_REQ_SIZE;
            read_req_id[(CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_REQ_ID;
            read_req_bypass[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_MASTER] = 1'b0;
            read_req_valid[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_MASTER] = 1'b1;
            timeout = 180;
            accepted_seen = 1'b0;
            while (!accepted_seen && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (read_req_accepted[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (!accepted_seen) begin
                fail_now("C++ trace invline other cache/MMIO RW cache read accept timeout");
            end
            read_req_valid[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_MASTER] = 1'b0;
            @(negedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace invline other cache/MMIO RW DDR refill AR timeout");
            end
            #1;
            if (ddr_axi_araddr != CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_ARADDR ||
                ddr_axi_arlen != CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_ARLEN ||
                ddr_axi_arsize != CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_ARSIZE ||
                ddr_axi_arburst != CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_ARBURST ||
                ddr_axi_arid != CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_ARID ||
                mmio_axi_arvalid) begin
                fail_now("C++ trace invline other cache/MMIO RW DDR refill AR mismatch");
            end
            seen_ddr_arid = ddr_axi_arid;
            ddr_axi_arready = 1'b1;
            @(posedge clk);
            ...
            read_req_addr[(CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_REQ_ADDR;
            read_req_total_size[(CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_MASTER * 8) +: 8] =
                CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_REQ_SIZE;
            read_req_id[(CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_REQ_ID;
            read_req_bypass[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_MASTER] = 1'b0;
            read_req_valid[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_MASTER] = 1'b1;
            timeout = 180;
            accepted_seen = 1'b0;
            while (!accepted_seen && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (read_req_accepted[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (!accepted_seen) begin
                fail_now("C++ trace invline other cache/MMIO RW MMIO read accept timeout");
            end
            read_req_valid[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_MASTER] = 1'b0;
            @(negedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace invline other cache/MMIO RW MMIO AR timeout");
            end
            #1;
            if (mmio_axi_araddr != CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_ARADDR ||
                mmio_axi_arlen != CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_ARLEN ||
                mmio_axi_arsize != CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_ARSIZE ||
                mmio_axi_arburst != CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_ARBURST ||
                mmio_axi_arid != CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_ARID ||
                ddr_axi_arvalid) begin
                fail_now("C++ trace invline other cache/MMIO RW MMIO AR mismatch");
            end
            seen_mmio_arid = mmio_axi_arid;
            mmio_axi_arready = 1'b1;
            @(posedge clk);
            ...
            write_req_addr[(CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_REQ_ADDR;
            write_req_total_size[(CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_MASTER * 8) +: 8] =
                CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_REQ_SIZE;
            write_req_id[(CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_REQ_ID;
            write_req_wdata[(CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_MASTER * LINE_BITS) +: LINE_BITS] =
                CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_REQ_WDATA;
            write_req_wstrb[(CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_MASTER * LINE_BYTES) +: LINE_BYTES] =
                CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_REQ_WSTRB[LINE_BYTES-1:0];
            write_req_bypass[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_MASTER] = 1'b0;
            write_req_valid[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_MASTER] = 1'b1;
            timeout = 180;
            accepted_seen = 1'b0;
            while (!accepted_seen && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (write_req_accepted[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (!accepted_seen) begin
                fail_now("C++ trace invline other cache/MMIO RW MMIO write accept timeout");
            end
            write_req_valid[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_MASTER] = 1'b0;
            @(negedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace invline other cache/MMIO RW MMIO AW timeout");
            end
            #1;
            if (mmio_axi_awaddr != CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_AWADDR ||
                mmio_axi_awlen != CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_AWLEN ||
                mmio_axi_awsize != CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_AWSIZE ||
                mmio_axi_awburst != CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_AWBURST ||
                mmio_axi_awid != CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_AWID ||
                ddr_axi_awvalid) begin
                fail_now("C++ trace invline other cache/MMIO RW MMIO AW mismatch");
            end
            seen_mmio_awid = mmio_axi_awid;
            mmio_axi_awready = 1'b1;
            @(posedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace invline other cache/MMIO RW MMIO W timeout");
            end
            #1;
            if (mmio_axi_wdata !=
                    CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_WBEAT0[MMIO_DATA_BITS-1:0] ||
                mmio_axi_wstrb !=
                    CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_WSTRB0[MMIO_STRB_BITS-1:0] ||
                mmio_axi_wlast != CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_WLAST0 ||
                ddr_axi_wvalid) begin
                fail_now("C++ trace invline other cache/MMIO RW MMIO W mismatch");
            end
            mmio_axi_wready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            ...
            invalidate_line_valid = 1'b1;
            mmio_axi_rid = seen_mmio_arid;
            mmio_axi_rdata = CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_RBEAT0[MMIO_DATA_BITS-1:0];
            mmio_axi_rresp = AXI_RESP_OKAY;
            mmio_axi_rlast = 1'b1;
            mmio_axi_rvalid = 1'b1;
            mmio_axi_bid = seen_mmio_awid;
            ...
            mmio_axi_bvalid = 1'b1;
            #1;
            if (mmio_axi_rready !== CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_RREADY_STALLED ||
                mmio_axi_rready !== CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_RREADY_PENDING ||
                mmio_axi_bready !== CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_BREADY_STALLED ||
                mmio_axi_bready !== CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_BREADY_PENDING ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline other cache/MMIO RW MMIO R/B mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            mmio_axi_rvalid = 1'b0;
            ...
            timeout = 180;
            while ((!read_resp_valid[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_MASTER] ||
                    !write_resp_valid[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_MASTER]) &&
                   (timeout > 0)) begin
                #1;
                if (invalidate_line_accepted) begin
                    fail_now("C++ trace invline other cache/MMIO RW accepted before MMIO responses");
                end
                @(posedge clk);
                timeout = timeout - 1;
            end
            if (timeout == 0 ||
                read_resp_valid[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_MASTER]) begin
                fail_now("C++ trace invline other cache/MMIO RW MMIO response owner mismatch");
            end
            #1;
            if (read_resp_id[(CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_RESP_ID ||
                read_resp_data[(CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_RESP_DATA ||
                write_resp_id[(CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_RESP_ID ||
                write_resp_code[(CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_MASTER * 2) +: 2] !=
                    CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_RESP_CODE ||
                invalidate_line_accepted !==
                    !CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_BLOCKED_MMIO_HELD) begin
                fail_now("C++ trace invline other cache/MMIO RW held MMIO response mismatch");
            end

            @(negedge clk);
            ddr_axi_rid = seen_ddr_arid;
            ddr_axi_rdata = CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_RBEAT0;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b0;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_RREADY_STALLED ||
                ddr_axi_rready !== CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_RREADY_MMIO_HELD ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline other cache/MMIO RW DDR R beat0 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            ddr_axi_rid = seen_ddr_arid;
            ddr_axi_rdata = CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_RBEAT1;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b1;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_RREADY_STALLED ||
                ddr_axi_rready !== CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_RREADY_MMIO_HELD ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline other cache/MMIO RW DDR R beat1 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            timeout = 260;
            while (!read_resp_valid[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_MASTER] &&
                   (timeout > 0)) begin
                #1;
                if (!read_resp_valid[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_MASTER] ||
                    !write_resp_valid[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_MASTER] ||
                    invalidate_line_accepted) begin
                    fail_now("C++ trace invline other cache/MMIO RW response drain ordering mismatch");
                end
                @(posedge clk);
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace invline other cache/MMIO RW cache response timeout");
            end
            #1;
            if (read_resp_id[(CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_RESP_ID ||
                read_resp_data[(CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_RESP_DATA ||
                invalidate_line_accepted !==
                    !CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_BLOCKED_CACHE_HELD) begin
                fail_now("C++ trace invline other cache/MMIO RW cache response hold mismatch");
            end

            read_resp_ready[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_MASTER] = 1'b1;
            #1;
            if (invalidate_line_accepted) begin
                fail_now("C++ trace invline other cache/MMIO RW accepted before MMIO read retire");
            end
            @(posedge clk);
            @(negedge clk);
            read_resp_ready[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_READ_MASTER] = 1'b0;

            write_resp_ready[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_MASTER] = 1'b1;
            #1;
            if (invalidate_line_accepted) begin
                fail_now("C++ trace invline other cache/MMIO RW accepted before MMIO write retire");
            end
            @(posedge clk);
            @(negedge clk);
            write_resp_ready[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_MMIO_WRITE_MASTER] = 1'b0;

            read_resp_ready[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_MASTER] = 1'b1;
            #1;
            if (invalidate_line_accepted) begin
                fail_now("C++ trace invline other cache/MMIO RW accepted before cache retire");
            end
            @(posedge clk);
            @(negedge clk);
            read_resp_ready[CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_DDR_MASTER] = 1'b0;
            ...
            if (accepted_seen !==
                CPP_MODE1_INVLINE_OTHER_CACHE_MMIO_RW_ACCEPTED_AFTER_RETIRE) begin
                fail_now("C++ trace invline other cache/MMIO RW final accept mismatch");
            end
            @(negedge clk);
            invalidate_line_valid = 1'b0;
            invalidate_line_addr = {ADDR_BITS{1'b0}};
    ...
    task issue_mode1_invalidate_line_cache_write_mmio_rw_and_check;
        integer timeout;
        reg accepted_seen;
        begin
            reset_dut();
            ...
            invalidate_line_addr =
                CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_INVALIDATE_ADDR;

            write_req_addr[(CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_REQ_ADDR;
            write_req_total_size[(CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_MASTER * 8) +: 8] =
                CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_REQ_SIZE;
            write_req_id[(CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_REQ_ID;
            write_req_wdata[(CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_MASTER * LINE_BITS) +: LINE_BITS] =
                CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_REQ_WDATA;
            write_req_wstrb[(CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_MASTER * LINE_BYTES) +: LINE_BYTES] =
                CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_REQ_WSTRB[LINE_BYTES-1:0];
            write_req_bypass[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_MASTER] = 1'b0;
            write_req_valid[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_MASTER] = 1'b1;
            timeout = 180;
            accepted_seen = 1'b0;
            while (!accepted_seen && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (write_req_accepted[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (!accepted_seen) begin
                fail_now("C++ trace invline cache-write/MMIO RW cache write accept timeout");
            end
            write_req_valid[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_MASTER] = 1'b0;
            @(negedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace invline cache-write/MMIO RW DDR refill AR timeout");
            end
            #1;
            if (ddr_axi_araddr != CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_REFILL_ARADDR ||
                ddr_axi_arlen != CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_REFILL_ARLEN ||
                ddr_axi_arsize != CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_REFILL_ARSIZE ||
                ddr_axi_arburst != CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_REFILL_ARBURST ||
                ddr_axi_arid != CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_REFILL_ARID ||
                mmio_axi_arvalid || ddr_axi_awvalid || ddr_axi_wvalid) begin
                fail_now("C++ trace invline cache-write/MMIO RW DDR refill AR mismatch");
            end
            seen_ddr_arid = ddr_axi_arid;
            ddr_axi_arready = 1'b1;
            @(posedge clk);
            ...
            read_req_addr[(CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_REQ_ADDR;
            read_req_total_size[(CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER * 8) +: 8] =
                CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_REQ_SIZE;
            read_req_id[(CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_REQ_ID;
            read_req_bypass[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER] = 1'b0;
            read_req_valid[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER] = 1'b1;
            timeout = 180;
            accepted_seen = 1'b0;
            while (!accepted_seen && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (read_req_accepted[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (!accepted_seen) begin
                fail_now("C++ trace invline cache-write/MMIO RW MMIO read accept timeout");
            end
            read_req_valid[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER] = 1'b0;
            @(negedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace invline cache-write/MMIO RW MMIO AR timeout");
            end
            #1;
            if (mmio_axi_araddr != CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_ARADDR ||
                mmio_axi_arlen != CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_ARLEN ||
                mmio_axi_arsize != CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_ARSIZE ||
                mmio_axi_arburst != CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_ARBURST ||
                mmio_axi_arid != CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_ARID ||
                ddr_axi_arvalid) begin
                fail_now("C++ trace invline cache-write/MMIO RW MMIO AR mismatch");
            end
            seen_mmio_arid = mmio_axi_arid;
            mmio_axi_arready = 1'b1;
            @(posedge clk);
            ...
            write_req_addr[(CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_REQ_ADDR;
            write_req_total_size[(CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER * 8) +: 8] =
                CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_REQ_SIZE;
            write_req_id[(CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_REQ_ID;
            write_req_wdata[(CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER * LINE_BITS) +: LINE_BITS] =
                CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_REQ_WDATA;
            write_req_wstrb[(CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER * LINE_BYTES) +: LINE_BYTES] =
                CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_REQ_WSTRB[LINE_BYTES-1:0];
            write_req_bypass[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER] = 1'b0;
            write_req_valid[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER] = 1'b1;
            timeout = 180;
            accepted_seen = 1'b0;
            while (!accepted_seen && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (write_req_accepted[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (!accepted_seen) begin
                fail_now("C++ trace invline cache-write/MMIO RW MMIO write accept timeout");
            end
            write_req_valid[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER] = 1'b0;
            @(negedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace invline cache-write/MMIO RW MMIO AW timeout");
            end
            #1;
            if (mmio_axi_awaddr != CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_AWADDR ||
                mmio_axi_awlen != CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_AWLEN ||
                mmio_axi_awsize != CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_AWSIZE ||
                mmio_axi_awburst != CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_AWBURST ||
                mmio_axi_awid != CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_AWID ||
                ddr_axi_awvalid) begin
                fail_now("C++ trace invline cache-write/MMIO RW MMIO AW mismatch");
            end
            seen_mmio_awid = mmio_axi_awid;
            mmio_axi_awready = 1'b1;
            @(posedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace invline cache-write/MMIO RW MMIO W timeout");
            end
            #1;
            if (mmio_axi_wdata !=
                    CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_WBEAT0[MMIO_DATA_BITS-1:0] ||
                mmio_axi_wstrb !=
                    CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_WSTRB0[MMIO_STRB_BITS-1:0] ||
                mmio_axi_wlast != CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_WLAST0 ||
                ddr_axi_wvalid) begin
                fail_now("C++ trace invline cache-write/MMIO RW MMIO W mismatch");
            end
            mmio_axi_wready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            ...
            mmio_axi_rid = seen_mmio_arid;
            mmio_axi_rdata =
                CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_RBEAT0[MMIO_DATA_BITS-1:0];
            mmio_axi_rresp = AXI_RESP_OKAY;
            mmio_axi_rlast = 1'b1;
            mmio_axi_rvalid = 1'b1;
            mmio_axi_bid = seen_mmio_awid;
            ...
            #1;
            if (mmio_axi_rready !==
                    CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_RREADY_STALLED ||
                mmio_axi_rready !==
                    CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_RREADY_PENDING ||
                mmio_axi_bready !==
                    CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_BREADY_STALLED ||
                mmio_axi_bready !==
                    CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_BREADY_PENDING ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline cache-write/MMIO RW MMIO R/B mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            mmio_axi_rvalid = 1'b0;
            ...
            timeout = 180;
            while ((!read_resp_valid[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER] ||
                    !write_resp_valid[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER]) &&
                   (timeout > 0)) begin
                #1;
                if (invalidate_line_accepted ||
                    write_resp_valid[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_MASTER]) begin
                    fail_now("C++ trace invline cache-write/MMIO RW early response mismatch");
                end
                @(posedge clk);
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace invline cache-write/MMIO RW MMIO response timeout");
            end
            #1;
            if (read_resp_id[(CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_RESP_ID ||
                read_resp_data[(CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_RESP_DATA ||
                write_resp_id[(CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_RESP_ID ||
                write_resp_code[(CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER * 2) +: 2] !=
                    CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_RESP_CODE ||
                invalidate_line_accepted !==
                    !CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_BLOCKED_MMIO_HELD) begin
                fail_now("C++ trace invline cache-write/MMIO RW held MMIO response mismatch");
            end

            @(negedge clk);
            ddr_axi_rid = seen_ddr_arid;
            ddr_axi_rdata = CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_REFILL_RBEAT0;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b0;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_REFILL_RREADY_STALLED ||
                ddr_axi_rready !== CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_DDR_RREADY_MMIO_HELD ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline cache-write/MMIO RW DDR R beat0 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            ddr_axi_rid = seen_ddr_arid;
            ddr_axi_rdata = CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_REFILL_RBEAT1;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b1;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_REFILL_RREADY_STALLED ||
                ddr_axi_rready !== CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_DDR_RREADY_MMIO_HELD ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline cache-write/MMIO RW DDR R beat1 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            timeout = 260;
            while (!write_resp_valid[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_MASTER] &&
                   (timeout > 0)) begin
                #1;
                if (!read_resp_valid[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER] ||
                    !write_resp_valid[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER] ||
                    invalidate_line_accepted) begin
                    fail_now("C++ trace invline cache-write/MMIO RW response drain ordering mismatch");
                end
                @(posedge clk);
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace invline cache-write/MMIO RW cache response timeout");
            end
            #1;
            if (write_resp_id[(CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_RESP_ID ||
                write_resp_code[(CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_MASTER * 2) +: 2] !=
                    CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_RESP_CODE ||
                invalidate_line_accepted !==
                    !CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_BLOCKED_CACHE_HELD) begin
                fail_now("C++ trace invline cache-write/MMIO RW cache response hold mismatch");
            end

            read_resp_ready[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER] = 1'b1;
            #1;
            if (invalidate_line_accepted) begin
                fail_now("C++ trace invline cache-write/MMIO RW accepted before MMIO read retire");
            end
            @(posedge clk);
            @(negedge clk);
            read_resp_ready[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER] = 1'b0;

            write_resp_ready[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER] = 1'b1;
            #1;
            if (invalidate_line_accepted) begin
                fail_now("C++ trace invline cache-write/MMIO RW accepted before MMIO write retire");
            end
            @(posedge clk);
            @(negedge clk);
            write_resp_ready[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER] = 1'b0;

            write_resp_ready[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_MASTER] = 1'b1;
            #1;
            if (invalidate_line_accepted) begin
                fail_now("C++ trace invline cache-write/MMIO RW accepted before cache retire");
            end
            @(posedge clk);
            @(negedge clk);
            write_resp_ready[CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_CACHE_MASTER] = 1'b0;
            ...
            if (accepted_seen !==
                CPP_MODE1_INVLINE_CACHE_WRITE_MMIO_RW_ACCEPTED_AFTER_RETIRE) begin
                fail_now("C++ trace invline cache-write/MMIO RW final accept mismatch");
            end
            @(negedge clk);
            invalidate_line_valid = 1'b0;
            invalidate_line_addr = {ADDR_BITS{1'b0}};
    ...
    task issue_mode1_invalidate_all_cache_write_mmio_rw_and_check;
        integer timeout;
        reg accepted_seen;
        begin
            reset_dut();
            ...
            write_req_addr[(CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_REQ_ADDR;
            write_req_total_size[(CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_MASTER * 8) +: 8] =
                CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_REQ_SIZE;
            write_req_id[(CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_REQ_ID;
            write_req_wdata[(CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_MASTER * LINE_BITS) +: LINE_BITS] =
                CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_REQ_WDATA;
            write_req_wstrb[(CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_MASTER * LINE_BYTES) +: LINE_BYTES] =
                CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_REQ_WSTRB[LINE_BYTES-1:0];
            write_req_bypass[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_MASTER] = 1'b0;
            write_req_valid[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_MASTER] = 1'b1;
            timeout = 180;
            accepted_seen = 1'b0;
            while (!accepted_seen && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (write_req_accepted[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (!accepted_seen) begin
                fail_now("C++ trace invall cache-write/MMIO RW cache write accept timeout");
            end
            write_req_valid[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_MASTER] = 1'b0;
            @(negedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace invall cache-write/MMIO RW DDR refill AR timeout");
            end
            #1;
            if (ddr_axi_araddr != CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_REFILL_ARADDR ||
                ddr_axi_arlen != CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_REFILL_ARLEN ||
                ddr_axi_arsize != CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_REFILL_ARSIZE ||
                ddr_axi_arburst != CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_REFILL_ARBURST ||
                ddr_axi_arid != CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_REFILL_ARID ||
                mmio_axi_arvalid || ddr_axi_awvalid || ddr_axi_wvalid) begin
                fail_now("C++ trace invall cache-write/MMIO RW DDR refill AR mismatch");
            end
            seen_ddr_arid = ddr_axi_arid;
            ddr_axi_arready = 1'b1;
            @(posedge clk);
            ...
            read_req_addr[(CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_REQ_ADDR;
            read_req_total_size[(CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER * 8) +: 8] =
                CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_REQ_SIZE;
            read_req_id[(CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_REQ_ID;
            read_req_bypass[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER] = 1'b0;
            read_req_valid[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER] = 1'b1;
            timeout = 180;
            accepted_seen = 1'b0;
            while (!accepted_seen && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (read_req_accepted[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (!accepted_seen) begin
                fail_now("C++ trace invall cache-write/MMIO RW MMIO read accept timeout");
            end
            read_req_valid[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER] = 1'b0;
            @(negedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace invall cache-write/MMIO RW MMIO AR timeout");
            end
            #1;
            if (mmio_axi_araddr != CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_ARADDR ||
                mmio_axi_arlen != CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_ARLEN ||
                mmio_axi_arsize != CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_ARSIZE ||
                mmio_axi_arburst != CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_ARBURST ||
                mmio_axi_arid != CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_ARID ||
                ddr_axi_arvalid) begin
                fail_now("C++ trace invall cache-write/MMIO RW MMIO AR mismatch");
            end
            seen_mmio_arid = mmio_axi_arid;
            mmio_axi_arready = 1'b1;
            @(posedge clk);
            ...
            write_req_addr[(CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_REQ_ADDR;
            write_req_total_size[(CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER * 8) +: 8] =
                CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_REQ_SIZE;
            write_req_id[(CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_REQ_ID;
            write_req_wdata[(CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER * LINE_BITS) +: LINE_BITS] =
                CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_REQ_WDATA;
            write_req_wstrb[(CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER * LINE_BYTES) +: LINE_BYTES] =
                CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_REQ_WSTRB[LINE_BYTES-1:0];
            write_req_bypass[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER] = 1'b0;
            write_req_valid[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER] = 1'b1;
            timeout = 180;
            accepted_seen = 1'b0;
            while (!accepted_seen && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (write_req_accepted[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (!accepted_seen) begin
                fail_now("C++ trace invall cache-write/MMIO RW MMIO write accept timeout");
            end
            write_req_valid[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER] = 1'b0;
            @(negedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace invall cache-write/MMIO RW MMIO AW timeout");
            end
            #1;
            if (mmio_axi_awaddr != CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_AWADDR ||
                mmio_axi_awlen != CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_AWLEN ||
                mmio_axi_awsize != CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_AWSIZE ||
                mmio_axi_awburst != CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_AWBURST ||
                mmio_axi_awid != CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_AWID ||
                ddr_axi_awvalid) begin
                fail_now("C++ trace invall cache-write/MMIO RW MMIO AW mismatch");
            end
            seen_mmio_awid = mmio_axi_awid;
            mmio_axi_awready = 1'b1;
            @(posedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace invall cache-write/MMIO RW MMIO W timeout");
            end
            #1;
            if (mmio_axi_wdata !=
                    CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_WBEAT0[MMIO_DATA_BITS-1:0] ||
                mmio_axi_wstrb !=
                    CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_WSTRB0[MMIO_STRB_BITS-1:0] ||
                mmio_axi_wlast != CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_WLAST0 ||
                ddr_axi_wvalid) begin
                fail_now("C++ trace invall cache-write/MMIO RW MMIO W mismatch");
            end
            mmio_axi_wready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            ...
            mmio_axi_rid = seen_mmio_arid;
            mmio_axi_rdata =
                CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_RBEAT0[MMIO_DATA_BITS-1:0];
            mmio_axi_rresp = AXI_RESP_OKAY;
            mmio_axi_rlast = 1'b1;
            mmio_axi_rvalid = 1'b1;
            mmio_axi_bid = seen_mmio_awid;
            ...
            #1;
            if (mmio_axi_rready !==
                    CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_RREADY_STALLED ||
                mmio_axi_rready !==
                    CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_RREADY_PENDING ||
                mmio_axi_bready !==
                    CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_BREADY_STALLED ||
                mmio_axi_bready !==
                    CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_BREADY_PENDING ||
                invalidate_all_accepted) begin
                fail_now("C++ trace invall cache-write/MMIO RW MMIO R/B mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            mmio_axi_rvalid = 1'b0;
            ...
            timeout = 180;
            while ((!read_resp_valid[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER] ||
                    !write_resp_valid[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER]) &&
                   (timeout > 0)) begin
                #1;
                if (invalidate_all_accepted ||
                    write_resp_valid[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_MASTER]) begin
                    fail_now("C++ trace invall cache-write/MMIO RW early response mismatch");
                end
                @(posedge clk);
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace invall cache-write/MMIO RW MMIO response timeout");
            end
            #1;
            if (read_resp_id[(CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_RESP_ID ||
                read_resp_data[(CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_RESP_DATA ||
                write_resp_id[(CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_RESP_ID ||
                write_resp_code[(CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER * 2) +: 2] !=
                    CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_RESP_CODE ||
                invalidate_all_accepted !==
                    !CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_BLOCKED_MMIO_HELD) begin
                fail_now("C++ trace invall cache-write/MMIO RW held MMIO response mismatch");
            end

            @(negedge clk);
            ddr_axi_rid = seen_ddr_arid;
            ddr_axi_rdata = CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_REFILL_RBEAT0;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b0;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_REFILL_RREADY_STALLED ||
                ddr_axi_rready !== CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_DDR_RREADY_MMIO_HELD ||
                invalidate_all_accepted) begin
                fail_now("C++ trace invall cache-write/MMIO RW DDR R beat0 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            ddr_axi_rid = seen_ddr_arid;
            ddr_axi_rdata = CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_REFILL_RBEAT1;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b1;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_REFILL_RREADY_STALLED ||
                ddr_axi_rready !== CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_DDR_RREADY_MMIO_HELD ||
                invalidate_all_accepted) begin
                fail_now("C++ trace invall cache-write/MMIO RW DDR R beat1 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            timeout = 260;
            while (!write_resp_valid[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_MASTER] &&
                   (timeout > 0)) begin
                #1;
                if (!read_resp_valid[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER] ||
                    !write_resp_valid[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER] ||
                    invalidate_all_accepted) begin
                    fail_now("C++ trace invall cache-write/MMIO RW response drain ordering mismatch");
                end
                @(posedge clk);
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace invall cache-write/MMIO RW cache response timeout");
            end
            #1;
            if (write_resp_id[(CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_RESP_ID ||
                write_resp_code[(CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_MASTER * 2) +: 2] !=
                    CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_RESP_CODE ||
                invalidate_all_accepted !==
                    !CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_BLOCKED_CACHE_HELD) begin
                fail_now("C++ trace invall cache-write/MMIO RW cache response hold mismatch");
            end

            read_resp_ready[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER] = 1'b1;
            #1;
            if (invalidate_all_accepted) begin
                fail_now("C++ trace invall cache-write/MMIO RW accepted before MMIO read retire");
            end
            @(posedge clk);
            @(negedge clk);
            read_resp_ready[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_READ_MASTER] = 1'b0;

            write_resp_ready[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER] = 1'b1;
            #1;
            if (invalidate_all_accepted) begin
                fail_now("C++ trace invall cache-write/MMIO RW accepted before MMIO write retire");
            end
            @(posedge clk);
            @(negedge clk);
            write_resp_ready[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_MMIO_WRITE_MASTER] = 1'b0;

            write_resp_ready[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_MASTER] = 1'b1;
            #1;
            if (invalidate_all_accepted) begin
                fail_now("C++ trace invall cache-write/MMIO RW accepted before cache retire");
            end
            @(posedge clk);
            @(negedge clk);
            write_resp_ready[CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_CACHE_MASTER] = 1'b0;

            timeout = 64;
            while (timeout > 0) begin
                #1;
                if (invalidate_all_accepted) begin
                    fail_now("C++ trace invall cache-write/MMIO RW accepted despite dirty line");
                end
                @(posedge clk);
                timeout = timeout - 1;
            end
            if (CPP_MODE1_INVALL_CACHE_WRITE_MMIO_RW_ACCEPTED_AFTER_RETIRE) begin
                fail_now("C++ trace invall cache-write/MMIO RW expected blocked final state");
            end
            @(negedge clk);
            invalidate_all_valid = 1'b0;
        end
    ...
    task issue_mode1_invalidate_line_multi_master_recovery_read_and_check;
        integer timeout;
        reg accepted_seen;
        begin
            reset_dut();
            ...
            issue_mode1_invalidate_line_recovery_cache_read(
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_FIRST_MASTER,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_FIRST_FILL_REQ_ADDR,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_FIRST_FILL_REQ_SIZE,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_FIRST_FILL_REQ_ID,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_FIRST_FILL_ARADDR,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_FIRST_FILL_ARLEN,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_FIRST_FILL_ARSIZE,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_FIRST_FILL_ARBURST,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_FIRST_FILL_ARID,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_FIRST_FILL_BEATS,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_FIRST_FILL_RBEAT0,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_FIRST_FILL_RBEAT1,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_FIRST_FILL_RESP_ID,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_FIRST_FILL_RESP_DATA,
                "C++ trace invline multi-master recovery first fill mismatch");

            issue_mode1_invalidate_line_recovery_cache_read(
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SECOND_MASTER,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SECOND_FILL_REQ_ADDR,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SECOND_FILL_REQ_SIZE,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SECOND_FILL_REQ_ID,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SECOND_FILL_ARADDR,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SECOND_FILL_ARLEN,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SECOND_FILL_ARSIZE,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SECOND_FILL_ARBURST,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SECOND_FILL_ARID,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SECOND_FILL_BEATS,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SECOND_FILL_RBEAT0,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SECOND_FILL_RBEAT1,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SECOND_FILL_RESP_ID,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SECOND_FILL_RESP_DATA,
                "C++ trace invline multi-master recovery second fill mismatch");

            invalidate_line_addr =
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_INVALIDATE_ADDR;
            invalidate_line_valid = 1'b1;
            timeout = 240;
            accepted_seen = 1'b0;
            while (!accepted_seen && (timeout > 0)) begin
            ...
            if (accepted_seen !== CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_INVALIDATE_ACCEPTED) begin
                fail_now("C++ trace invline multi-master recovery accept mismatch");
            end
            @(negedge clk);
            invalidate_line_valid = 1'b0;
            invalidate_line_addr = {ADDR_BITS{1'b0}};
            ...
            issue_mode1_invalidate_line_recovery_cache_read(
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SECOND_MASTER,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_TARGET_AFTER_REQ_ADDR,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_TARGET_AFTER_REQ_SIZE,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_TARGET_AFTER_REQ_ID,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_TARGET_AFTER_ARADDR,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_TARGET_AFTER_ARLEN,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_TARGET_AFTER_ARSIZE,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_TARGET_AFTER_ARBURST,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_TARGET_AFTER_ARID,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_TARGET_AFTER_BEATS,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_TARGET_AFTER_RBEAT0,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_TARGET_AFTER_RBEAT1,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_TARGET_AFTER_RESP_ID,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_TARGET_AFTER_RESP_DATA,
                "C++ trace invline multi-master recovery target after mismatch");

            issue_mode1_invalidate_line_scope_cache_hit(
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_FIRST_MASTER,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SURVIVOR_AFTER_REQ_ADDR,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SURVIVOR_AFTER_REQ_SIZE,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SURVIVOR_AFTER_REQ_ID,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SURVIVOR_AFTER_RESP_ID,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SURVIVOR_AFTER_RESP_DATA,
                CPP_MODE1_INVLINE_MULTI_MASTER_RECOVERY_SURVIVOR_HIT_NO_EXTERNAL,
                "C++ trace invline multi-master recovery survivor hit mismatch");
        end
    endtask

    task issue_mode1_seeded_maintenance_recovery_and_check;
        integer idx;
        integer timeout;
        reg accepted_seen;
        begin
            for (idx = 0; idx < CPP_SEEDED_MAINT_COUNT; idx = idx + 1) begin
                reset_dut();
                enter_mode(MODE_CACHE);
                @(negedge clk);
                read_resp_ready = {NUM_READ_MASTERS{1'b0}};
                ...
                issue_mode1_invalidate_line_recovery_cache_read(
                    CPP_SEEDED_MAINT_FIRST_MASTER[idx],
                    CPP_SEEDED_MAINT_FIRST_FILL_REQ_ADDR[idx],
                    CPP_SEEDED_MAINT_FIRST_FILL_REQ_SIZE[idx],
                    CPP_SEEDED_MAINT_FIRST_FILL_REQ_ID[idx],
                    CPP_SEEDED_MAINT_FIRST_FILL_ARADDR[idx],
                    CPP_SEEDED_MAINT_FIRST_FILL_ARLEN[idx],
                    CPP_SEEDED_MAINT_FIRST_FILL_ARSIZE[idx],
                    CPP_SEEDED_MAINT_FIRST_FILL_ARBURST[idx],
                    CPP_SEEDED_MAINT_FIRST_FILL_ARID[idx],
                    CPP_SEEDED_MAINT_FIRST_FILL_BEATS[idx],
                    CPP_SEEDED_MAINT_FIRST_FILL_RBEAT0[idx],
                    CPP_SEEDED_MAINT_FIRST_FILL_RBEAT1[idx],
                    CPP_SEEDED_MAINT_FIRST_FILL_RESP_ID[idx],
                    CPP_SEEDED_MAINT_FIRST_FILL_RESP_DATA[idx],
                    "seeded maint first fill mismatch");

                issue_mode1_invalidate_line_recovery_cache_read(
                    CPP_SEEDED_MAINT_SECOND_MASTER[idx],
                    CPP_SEEDED_MAINT_SECOND_FILL_REQ_ADDR[idx],
                    CPP_SEEDED_MAINT_SECOND_FILL_REQ_SIZE[idx],
                    CPP_SEEDED_MAINT_SECOND_FILL_REQ_ID[idx],
                    CPP_SEEDED_MAINT_SECOND_FILL_ARADDR[idx],
                    CPP_SEEDED_MAINT_SECOND_FILL_ARLEN[idx],
                    CPP_SEEDED_MAINT_SECOND_FILL_ARSIZE[idx],
                    CPP_SEEDED_MAINT_SECOND_FILL_ARBURST[idx],
                    CPP_SEEDED_MAINT_SECOND_FILL_ARID[idx],
                    CPP_SEEDED_MAINT_SECOND_FILL_BEATS[idx],
                    CPP_SEEDED_MAINT_SECOND_FILL_RBEAT0[idx],
                    CPP_SEEDED_MAINT_SECOND_FILL_RBEAT1[idx],
                    CPP_SEEDED_MAINT_SECOND_FILL_RESP_ID[idx],
                    CPP_SEEDED_MAINT_SECOND_FILL_RESP_DATA[idx],
                    "seeded maint second fill mismatch");

                if (CPP_SEEDED_MAINT_INVALIDATE_ALL[idx]) begin
                    invalidate_all_valid = 1'b1;
                end else begin
                    invalidate_line_addr = CPP_SEEDED_MAINT_INVALIDATE_ADDR[idx];
                    invalidate_line_valid = 1'b1;
                end
                timeout = 10000;
                accepted_seen = 1'b0;
                ...
                if (accepted_seen !== CPP_SEEDED_MAINT_ACCEPTED[idx]) begin
                    fail_now("seeded maint accept mismatch");
                end
                @(negedge clk);
                invalidate_all_valid = 1'b0;
                invalidate_line_valid = 1'b0;
                ...
                if (CPP_SEEDED_MAINT_FIRST_AFTER_MISS[idx]) begin
                    issue_mode1_invalidate_line_recovery_cache_read(
                        CPP_SEEDED_MAINT_FIRST_MASTER[idx],
                        CPP_SEEDED_MAINT_FIRST_AFTER_REQ_ADDR[idx],
                        CPP_SEEDED_MAINT_FIRST_AFTER_REQ_SIZE[idx],
                        CPP_SEEDED_MAINT_FIRST_AFTER_REQ_ID[idx],
                        CPP_SEEDED_MAINT_FIRST_AFTER_ARADDR[idx],
                        CPP_SEEDED_MAINT_FIRST_AFTER_ARLEN[idx],
                        CPP_SEEDED_MAINT_FIRST_AFTER_ARSIZE[idx],
                        CPP_SEEDED_MAINT_FIRST_AFTER_ARBURST[idx],
                        CPP_SEEDED_MAINT_FIRST_AFTER_ARID[idx],
                        CPP_SEEDED_MAINT_FIRST_AFTER_BEATS[idx],
                        CPP_SEEDED_MAINT_FIRST_AFTER_RBEAT0[idx],
                        CPP_SEEDED_MAINT_FIRST_AFTER_RBEAT1[idx],
                        CPP_SEEDED_MAINT_FIRST_AFTER_RESP_ID[idx],
                        CPP_SEEDED_MAINT_FIRST_AFTER_RESP_DATA[idx],
                        "seeded maint first after miss mismatch");
                end else begin
                    issue_mode1_invalidate_line_scope_cache_hit(
                        CPP_SEEDED_MAINT_FIRST_MASTER[idx],
                        CPP_SEEDED_MAINT_FIRST_AFTER_REQ_ADDR[idx],
                        ...
                        CPP_SEEDED_MAINT_FIRST_AFTER_RESP_ID[idx],
                        CPP_SEEDED_MAINT_FIRST_AFTER_RESP_DATA[idx],
                        CPP_SEEDED_MAINT_FIRST_HIT_NO_EXTERNAL[idx],
                        "seeded maint first after hit mismatch");
                end

                if (CPP_SEEDED_MAINT_SECOND_AFTER_MISS[idx]) begin
                    issue_mode1_invalidate_line_recovery_cache_read(
                        CPP_SEEDED_MAINT_SECOND_MASTER[idx],
                        CPP_SEEDED_MAINT_SECOND_AFTER_REQ_ADDR[idx],
                        CPP_SEEDED_MAINT_SECOND_AFTER_REQ_SIZE[idx],
                        CPP_SEEDED_MAINT_SECOND_AFTER_REQ_ID[idx],
                        CPP_SEEDED_MAINT_SECOND_AFTER_ARADDR[idx],
                        CPP_SEEDED_MAINT_SECOND_AFTER_ARLEN[idx],
                        CPP_SEEDED_MAINT_SECOND_AFTER_ARSIZE[idx],
                        CPP_SEEDED_MAINT_SECOND_AFTER_ARBURST[idx],
                        CPP_SEEDED_MAINT_SECOND_AFTER_ARID[idx],
                        CPP_SEEDED_MAINT_SECOND_AFTER_BEATS[idx],
                        CPP_SEEDED_MAINT_SECOND_AFTER_RBEAT0[idx],
                        CPP_SEEDED_MAINT_SECOND_AFTER_RBEAT1[idx],
                        CPP_SEEDED_MAINT_SECOND_AFTER_RESP_ID[idx],
                        CPP_SEEDED_MAINT_SECOND_AFTER_RESP_DATA[idx],
                        "seeded maint second after miss mismatch");
                end else begin
                    issue_mode1_invalidate_line_scope_cache_hit(
                        CPP_SEEDED_MAINT_SECOND_MASTER[idx],
                        CPP_SEEDED_MAINT_SECOND_AFTER_REQ_ADDR[idx],
                        ...
                        CPP_SEEDED_MAINT_SECOND_AFTER_RESP_ID[idx],
                        CPP_SEEDED_MAINT_SECOND_AFTER_RESP_DATA[idx],
                        CPP_SEEDED_MAINT_SECOND_HIT_NO_EXTERNAL[idx],
                        "seeded maint second after hit mismatch");
                end
            end
        end
    endtask

    task issue_mode1_invalidate_all_recovery_cache_read_and_check;
        integer timeout;
        reg accepted_seen;
        begin
            reset_dut();
            ...
            issue_mode1_invalidate_line_recovery_cache_read(
                CPP_MODE1_INVALL_RECOVERY_CACHE_MASTER,
                CPP_MODE1_INVALL_RECOVERY_FIRST_FILL_REQ_ADDR,
                CPP_MODE1_INVALL_RECOVERY_FIRST_FILL_REQ_SIZE,
                CPP_MODE1_INVALL_RECOVERY_FIRST_FILL_REQ_ID,
                CPP_MODE1_INVALL_RECOVERY_FIRST_FILL_ARADDR,
                CPP_MODE1_INVALL_RECOVERY_FIRST_FILL_ARLEN,
                CPP_MODE1_INVALL_RECOVERY_FIRST_FILL_ARSIZE,
                CPP_MODE1_INVALL_RECOVERY_FIRST_FILL_ARBURST,
                CPP_MODE1_INVALL_RECOVERY_FIRST_FILL_ARID,
                CPP_MODE1_INVALL_RECOVERY_FIRST_FILL_BEATS,
                CPP_MODE1_INVALL_RECOVERY_FIRST_FILL_RBEAT0,
                CPP_MODE1_INVALL_RECOVERY_FIRST_FILL_RBEAT1,
                CPP_MODE1_INVALL_RECOVERY_FIRST_FILL_RESP_ID,
                CPP_MODE1_INVALL_RECOVERY_FIRST_FILL_RESP_DATA,
                "C++ trace invall cache recovery first fill mismatch");

            issue_mode1_invalidate_line_recovery_cache_read(
                CPP_MODE1_INVALL_RECOVERY_CACHE_MASTER,
                CPP_MODE1_INVALL_RECOVERY_SECOND_FILL_REQ_ADDR,
                CPP_MODE1_INVALL_RECOVERY_SECOND_FILL_REQ_SIZE,
                CPP_MODE1_INVALL_RECOVERY_SECOND_FILL_REQ_ID,
                CPP_MODE1_INVALL_RECOVERY_SECOND_FILL_ARADDR,
                CPP_MODE1_INVALL_RECOVERY_SECOND_FILL_ARLEN,
                CPP_MODE1_INVALL_RECOVERY_SECOND_FILL_ARSIZE,
                CPP_MODE1_INVALL_RECOVERY_SECOND_FILL_ARBURST,
                CPP_MODE1_INVALL_RECOVERY_SECOND_FILL_ARID,
                CPP_MODE1_INVALL_RECOVERY_SECOND_FILL_BEATS,
                CPP_MODE1_INVALL_RECOVERY_SECOND_FILL_RBEAT0,
                CPP_MODE1_INVALL_RECOVERY_SECOND_FILL_RBEAT1,
                CPP_MODE1_INVALL_RECOVERY_SECOND_FILL_RESP_ID,
                CPP_MODE1_INVALL_RECOVERY_SECOND_FILL_RESP_DATA,
                "C++ trace invall cache recovery second fill mismatch");
            ...
            if (accepted_seen !== CPP_MODE1_INVALL_RECOVERY_CACHE_INVALIDATE_ACCEPTED) begin
                fail_now("C++ trace invall cache recovery accept mismatch");
            end
            @(negedge clk);
            invalidate_all_valid = 1'b0;
            ...
            issue_mode1_invalidate_line_recovery_cache_read(
                CPP_MODE1_INVALL_RECOVERY_CACHE_MASTER,
                CPP_MODE1_INVALL_RECOVERY_FIRST_AFTER_REQ_ADDR,
                CPP_MODE1_INVALL_RECOVERY_FIRST_AFTER_REQ_SIZE,
                CPP_MODE1_INVALL_RECOVERY_FIRST_AFTER_REQ_ID,
                CPP_MODE1_INVALL_RECOVERY_FIRST_AFTER_ARADDR,
                CPP_MODE1_INVALL_RECOVERY_FIRST_AFTER_ARLEN,
                CPP_MODE1_INVALL_RECOVERY_FIRST_AFTER_ARSIZE,
                CPP_MODE1_INVALL_RECOVERY_FIRST_AFTER_ARBURST,
                CPP_MODE1_INVALL_RECOVERY_FIRST_AFTER_ARID,
                CPP_MODE1_INVALL_RECOVERY_FIRST_AFTER_BEATS,
                CPP_MODE1_INVALL_RECOVERY_FIRST_AFTER_RBEAT0,
                CPP_MODE1_INVALL_RECOVERY_FIRST_AFTER_RBEAT1,
                CPP_MODE1_INVALL_RECOVERY_FIRST_AFTER_RESP_ID,
                CPP_MODE1_INVALL_RECOVERY_FIRST_AFTER_RESP_DATA,
                "C++ trace invall cache recovery first after mismatch");

            issue_mode1_invalidate_line_recovery_cache_read(
                CPP_MODE1_INVALL_RECOVERY_CACHE_MASTER,
                CPP_MODE1_INVALL_RECOVERY_SECOND_AFTER_REQ_ADDR,
                CPP_MODE1_INVALL_RECOVERY_SECOND_AFTER_REQ_SIZE,
                CPP_MODE1_INVALL_RECOVERY_SECOND_AFTER_REQ_ID,
                CPP_MODE1_INVALL_RECOVERY_SECOND_AFTER_ARADDR,
                CPP_MODE1_INVALL_RECOVERY_SECOND_AFTER_ARLEN,
                CPP_MODE1_INVALL_RECOVERY_SECOND_AFTER_ARSIZE,
                CPP_MODE1_INVALL_RECOVERY_SECOND_AFTER_ARBURST,
                CPP_MODE1_INVALL_RECOVERY_SECOND_AFTER_ARID,
                CPP_MODE1_INVALL_RECOVERY_SECOND_AFTER_BEATS,
                CPP_MODE1_INVALL_RECOVERY_SECOND_AFTER_RBEAT0,
                CPP_MODE1_INVALL_RECOVERY_SECOND_AFTER_RBEAT1,
                CPP_MODE1_INVALL_RECOVERY_SECOND_AFTER_RESP_ID,
                CPP_MODE1_INVALL_RECOVERY_SECOND_AFTER_RESP_DATA,
                "C++ trace invall cache recovery second after mismatch");
        end
    endtask
    ...
    task issue_mode1_invalidate_all_multi_read_and_check;
        integer timeout;
        reg accepted_seen;
        ...
        begin
            ...
            read_req_addr[(CPP_MODE1_INVALL_MULTI_READ_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVALL_MULTI_READ_FIRST_REQ_ADDR;
            read_req_total_size[(CPP_MODE1_INVALL_MULTI_READ_MASTER * 8) +: 8] =
                CPP_MODE1_INVALL_MULTI_READ_FIRST_REQ_SIZE;
            read_req_id[(CPP_MODE1_INVALL_MULTI_READ_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVALL_MULTI_READ_FIRST_REQ_ID;
            read_req_bypass[CPP_MODE1_INVALL_MULTI_READ_MASTER] = 1'b0;
            read_req_valid[CPP_MODE1_INVALL_MULTI_READ_MASTER] = 1'b1;
            timeout = 240;
            accepted_seen = 1'b0;
            while (!ddr_axi_arvalid && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (read_req_accepted[CPP_MODE1_INVALL_MULTI_READ_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (timeout == 0 || !accepted_seen) begin
                fail_now("C++ trace invall multi-read first AR timeout");
            end
            #1;
            if (ddr_axi_araddr != CPP_MODE1_INVALL_MULTI_READ_FIRST_ARADDR ||
                ddr_axi_arlen != CPP_MODE1_INVALL_MULTI_READ_FIRST_ARLEN ||
                ddr_axi_arsize != CPP_MODE1_INVALL_MULTI_READ_FIRST_ARSIZE ||
                ddr_axi_arburst != CPP_MODE1_INVALL_MULTI_READ_FIRST_ARBURST ||
                ddr_axi_arid != CPP_MODE1_INVALL_MULTI_READ_FIRST_ARID ||
                mmio_axi_arvalid || ddr_axi_awvalid || ddr_axi_wvalid) begin
                fail_now("C++ trace invall multi-read first AR mismatch");
            end
            ...
            @(negedge clk);
            ddr_axi_arready = 1'b0;
            read_req_valid[CPP_MODE1_INVALL_MULTI_READ_MASTER] = 1'b0;

            read_req_addr[(CPP_MODE1_INVALL_MULTI_READ_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVALL_MULTI_READ_SECOND_REQ_ADDR;
            read_req_total_size[(CPP_MODE1_INVALL_MULTI_READ_MASTER * 8) +: 8] =
                CPP_MODE1_INVALL_MULTI_READ_SECOND_REQ_SIZE;
            read_req_id[(CPP_MODE1_INVALL_MULTI_READ_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVALL_MULTI_READ_SECOND_REQ_ID;
            read_req_bypass[CPP_MODE1_INVALL_MULTI_READ_MASTER] = 1'b0;
            read_req_valid[CPP_MODE1_INVALL_MULTI_READ_MASTER] = 1'b1;
            timeout = 240;
            accepted_seen = 1'b0;
            ...
            if (timeout == 0 || !accepted_seen) begin
                fail_now("C++ trace invall multi-read second AR timeout");
            end
            #1;
            if (ddr_axi_araddr != CPP_MODE1_INVALL_MULTI_READ_SECOND_ARADDR ||
                ddr_axi_arlen != CPP_MODE1_INVALL_MULTI_READ_SECOND_ARLEN ||
                ddr_axi_arsize != CPP_MODE1_INVALL_MULTI_READ_SECOND_ARSIZE ||
                ddr_axi_arburst != CPP_MODE1_INVALL_MULTI_READ_SECOND_ARBURST ||
                ddr_axi_arid != CPP_MODE1_INVALL_MULTI_READ_SECOND_ARID ||
                mmio_axi_arvalid || ddr_axi_awvalid || ddr_axi_wvalid) begin
                fail_now("C++ trace invall multi-read second AR mismatch");
            end
            ...
            ddr_axi_rdata = CPP_MODE1_INVALL_MULTI_READ_FIRST_RBEAT0;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b0;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVALL_MULTI_READ_FIRST_RREADY_PENDING ||
                invalidate_all_accepted) begin
                fail_now("C++ trace invall multi-read first R beat0 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            ddr_axi_rid = first_arid;
            ddr_axi_rdata = CPP_MODE1_INVALL_MULTI_READ_FIRST_RBEAT1;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b1;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVALL_MULTI_READ_FIRST_RREADY_PENDING ||
                invalidate_all_accepted) begin
                fail_now("C++ trace invall multi-read first R beat1 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            timeout = 260;
            while (!read_resp_valid[CPP_MODE1_INVALL_MULTI_READ_MASTER] &&
                   (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (invalidate_all_accepted) begin
                    fail_now("C++ trace invall multi-read accepted before first response");
                end
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace invall multi-read first response timeout");
            end
            #1;
            if (read_resp_id[(CPP_MODE1_INVALL_MULTI_READ_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVALL_MULTI_READ_FIRST_RESP_ID ||
                read_resp_data[(CPP_MODE1_INVALL_MULTI_READ_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_INVALL_MULTI_READ_FIRST_RESP_DATA ||
                (!invalidate_all_accepted) !== CPP_MODE1_INVALL_MULTI_READ_BLOCKED_FIRST_HELD) begin
                fail_now("C++ trace invall multi-read first response mismatch");
            end

            ddr_axi_rid = second_arid;
            ddr_axi_rdata = CPP_MODE1_INVALL_MULTI_READ_SECOND_RBEAT0;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b0;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVALL_MULTI_READ_SECOND_RREADY_FIRST_HELD ||
                invalidate_all_accepted) begin
                fail_now("C++ trace invall multi-read second R beat0 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            ddr_axi_rid = second_arid;
            ddr_axi_rdata = CPP_MODE1_INVALL_MULTI_READ_SECOND_RBEAT1;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b1;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVALL_MULTI_READ_SECOND_RREADY_FIRST_HELD ||
                invalidate_all_accepted) begin
                fail_now("C++ trace invall multi-read second R beat1 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            read_resp_ready[CPP_MODE1_INVALL_MULTI_READ_MASTER] = 1'b1;
            #1;
            if (read_resp_id[(CPP_MODE1_INVALL_MULTI_READ_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVALL_MULTI_READ_FIRST_RESP_ID ||
                read_resp_data[(CPP_MODE1_INVALL_MULTI_READ_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_INVALL_MULTI_READ_FIRST_RESP_DATA ||
                invalidate_all_accepted) begin
                fail_now("C++ trace invall multi-read first retire mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            read_resp_ready[CPP_MODE1_INVALL_MULTI_READ_MASTER] = 1'b0;
            ...
            while (!read_resp_valid[CPP_MODE1_INVALL_MULTI_READ_MASTER] &&
                ...
                #1;
                if (invalidate_all_accepted) begin
                    fail_now("C++ trace invall multi-read accepted before second response");
                end
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace invall multi-read second response timeout");
            end
            #1;
            if (read_resp_id[(CPP_MODE1_INVALL_MULTI_READ_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVALL_MULTI_READ_SECOND_RESP_ID ||
                read_resp_data[(CPP_MODE1_INVALL_MULTI_READ_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_INVALL_MULTI_READ_SECOND_RESP_DATA ||
                (!invalidate_all_accepted) !== CPP_MODE1_INVALL_MULTI_READ_BLOCKED_SECOND_HELD) begin
                fail_now("C++ trace invall multi-read second response mismatch");
            end
            ...
            if (read_resp_id[(CPP_MODE1_INVALL_MULTI_READ_MASTER * ID_BITS) +: ID_BITS] !=
                ...
                read_resp_data[(CPP_MODE1_INVALL_MULTI_READ_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_INVALL_MULTI_READ_SECOND_RESP_DATA ||
                invalidate_all_accepted) begin
                fail_now("C++ trace invall multi-read second retire mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            read_resp_ready[CPP_MODE1_INVALL_MULTI_READ_MASTER] = 1'b0;
            ...
            if (accepted_seen !== CPP_MODE1_INVALL_MULTI_READ_ACCEPTED_AFTER_RETIRE) begin
                fail_now("C++ trace invall multi-read final accept mismatch");
            end
            @(negedge clk);
            invalidate_all_valid = 1'b0;
        end
    ...
    task issue_mode1_invalidate_all_multi_master_read_and_check;
        integer timeout;
        reg accepted_seen;
        reg [AXI_ID_BITS-1:0] first_arid;
        reg [AXI_ID_BITS-1:0] second_arid;
        begin
            ...
            read_req_addr[(CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_REQ_ADDR;
            read_req_total_size[(CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_MASTER * 8) +: 8] =
                CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_REQ_SIZE;
            read_req_id[(CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_REQ_ID;
            read_req_bypass[CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_MASTER] = 1'b0;
            read_req_valid[CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_MASTER] = 1'b1;
            timeout = 240;
            accepted_seen = 1'b0;
            while (!ddr_axi_arvalid && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (read_req_accepted[CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (timeout == 0 || !accepted_seen) begin
                fail_now("C++ trace invall multi-master first AR timeout");
            end
            #1;
            if (ddr_axi_araddr != CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_ARADDR ||
                ddr_axi_arlen != CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_ARLEN ||
                ddr_axi_arsize != CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_ARSIZE ||
                ddr_axi_arburst != CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_ARBURST ||
                ddr_axi_arid != CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_ARID ||
                mmio_axi_arvalid || ddr_axi_awvalid || ddr_axi_wvalid) begin
                fail_now("C++ trace invall multi-master first AR mismatch");
            end
            first_arid = ddr_axi_arid;
            ddr_axi_arready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            ddr_axi_arready = 1'b0;
            read_req_valid[CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_MASTER] = 1'b0;

            read_req_addr[(CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_REQ_ADDR;
            read_req_total_size[(CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_MASTER * 8) +: 8] =
                CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_REQ_SIZE;
            read_req_id[(CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_REQ_ID;
            read_req_bypass[CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_MASTER] = 1'b0;
            read_req_valid[CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_MASTER] = 1'b1;
            timeout = 240;
            accepted_seen = 1'b0;
            while (!ddr_axi_arvalid && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (read_req_accepted[CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (timeout == 0 || !accepted_seen) begin
                fail_now("C++ trace invall multi-master second AR timeout");
            end
            #1;
            if (ddr_axi_araddr != CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_ARADDR ||
                ddr_axi_arlen != CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_ARLEN ||
                ddr_axi_arsize != CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_ARSIZE ||
                ddr_axi_arburst != CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_ARBURST ||
                ddr_axi_arid != CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_ARID ||
                mmio_axi_arvalid || ddr_axi_awvalid || ddr_axi_wvalid) begin
                fail_now("C++ trace invall multi-master second AR mismatch");
            end
            second_arid = ddr_axi_arid;
            ddr_axi_arready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            ddr_axi_arready = 1'b0;
            read_req_valid[CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_MASTER] = 1'b0;

            invalidate_all_valid = 1'b1;
            ddr_axi_rid = first_arid;
            ddr_axi_rdata = CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_RBEAT0;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b0;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_RREADY_PENDING ||
                invalidate_all_accepted) begin
                fail_now("C++ trace invall multi-master first R beat0 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            ddr_axi_rid = first_arid;
            ddr_axi_rdata = CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_RBEAT1;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b1;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_RREADY_PENDING ||
                invalidate_all_accepted) begin
                fail_now("C++ trace invall multi-master first R beat1 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            timeout = 260;
            while (!read_resp_valid[CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_MASTER] &&
                   (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (invalidate_all_accepted) begin
                    fail_now("C++ trace invall multi-master accepted before first response");
                end
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace invall multi-master first response timeout");
            end
            #1;
            if (read_resp_id[(CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_RESP_ID ||
                read_resp_data[(CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_RESP_DATA ||
                (!invalidate_all_accepted) !== CPP_MODE1_INVALL_MULTI_MASTER_READ_BLOCKED_FIRST_HELD) begin
                fail_now("C++ trace invall multi-master first response mismatch");
            end

            ddr_axi_rid = second_arid;
            ddr_axi_rdata = CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_RBEAT0;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b0;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (!read_resp_valid[CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_MASTER] ||
                ddr_axi_rready !== CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_RREADY_FIRST_HELD ||
                invalidate_all_accepted) begin
                fail_now("C++ trace invall multi-master second R beat0 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            ddr_axi_rid = second_arid;
            ddr_axi_rdata = CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_RBEAT1;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b1;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (!read_resp_valid[CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_MASTER] ||
                ddr_axi_rready !== CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_RREADY_FIRST_HELD ||
                invalidate_all_accepted) begin
                fail_now("C++ trace invall multi-master second R beat1 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            read_resp_ready[CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_MASTER] = 1'b1;
            #1;
            if (read_resp_id[(CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_RESP_ID ||
                read_resp_data[(CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_RESP_DATA ||
                invalidate_all_accepted) begin
                fail_now("C++ trace invall multi-master first retire mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            read_resp_ready[CPP_MODE1_INVALL_MULTI_MASTER_READ_FIRST_MASTER] = 1'b0;

            timeout = 260;
            while (!read_resp_valid[CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_MASTER] &&
                   (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (invalidate_all_accepted) begin
                    fail_now("C++ trace invall multi-master accepted before second response");
                end
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace invall multi-master second response timeout");
            end
            #1;
            if (read_resp_id[(CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_RESP_ID ||
                read_resp_data[(CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_RESP_DATA ||
                (!invalidate_all_accepted) !== CPP_MODE1_INVALL_MULTI_MASTER_READ_BLOCKED_SECOND_HELD) begin
                fail_now("C++ trace invall multi-master second response mismatch");
            end

            read_resp_ready[CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_MASTER] = 1'b1;
            #1;
            if (read_resp_id[(CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_RESP_ID ||
                read_resp_data[(CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_RESP_DATA ||
                invalidate_all_accepted) begin
                fail_now("C++ trace invall multi-master second retire mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            read_resp_ready[CPP_MODE1_INVALL_MULTI_MASTER_READ_SECOND_MASTER] = 1'b0;
            ...
            if (accepted_seen !== CPP_MODE1_INVALL_MULTI_MASTER_READ_ACCEPTED_AFTER_RETIRE) begin
                fail_now("C++ trace invall multi-master final accept mismatch");
            end
            @(negedge clk);
            invalidate_all_valid = 1'b0;
        end
    ...
    task issue_mode1_invalidate_line_multi_master_read_and_check;
        integer timeout;
        reg accepted_seen;
        reg [AXI_ID_BITS-1:0] first_arid;
        reg [AXI_ID_BITS-1:0] second_arid;
        begin
            ...
            ddr_axi_arready = 1'b0;
            invalidate_line_valid = 1'b0;
            invalidate_line_addr = CPP_MODE1_INVLINE_MULTI_MASTER_READ_INVALIDATE_ADDR;

            read_req_addr[(CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_REQ_ADDR;
            read_req_total_size[(CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_MASTER * 8) +: 8] =
                CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_REQ_SIZE;
            read_req_id[(CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_REQ_ID;
            read_req_bypass[CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_MASTER] = 1'b0;
            read_req_valid[CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_MASTER] = 1'b1;
            timeout = 240;
            accepted_seen = 1'b0;
            while (!ddr_axi_arvalid && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (read_req_accepted[CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (timeout == 0 || !accepted_seen) begin
                fail_now("C++ trace invline multi-master first AR timeout");
            end
            #1;
            if (ddr_axi_araddr != CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_ARADDR ||
                ddr_axi_arlen != CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_ARLEN ||
                ddr_axi_arsize != CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_ARSIZE ||
                ddr_axi_arburst != CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_ARBURST ||
                ddr_axi_arid != CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_ARID ||
                mmio_axi_arvalid || ddr_axi_awvalid || ddr_axi_wvalid) begin
                fail_now("C++ trace invline multi-master first AR mismatch");
            end
            first_arid = ddr_axi_arid;
            ddr_axi_arready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            ddr_axi_arready = 1'b0;
            read_req_valid[CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_MASTER] = 1'b0;

            read_req_addr[(CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_REQ_ADDR;
            read_req_total_size[(CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_MASTER * 8) +: 8] =
                CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_REQ_SIZE;
            read_req_id[(CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_REQ_ID;
            read_req_bypass[CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_MASTER] = 1'b0;
            read_req_valid[CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_MASTER] = 1'b1;
            timeout = 240;
            accepted_seen = 1'b0;
            while (!ddr_axi_arvalid && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (read_req_accepted[CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (timeout == 0 || !accepted_seen) begin
                fail_now("C++ trace invline multi-master second AR timeout");
            end
            #1;
            if (ddr_axi_araddr != CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_ARADDR ||
                ddr_axi_arlen != CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_ARLEN ||
                ddr_axi_arsize != CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_ARSIZE ||
                ddr_axi_arburst != CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_ARBURST ||
                ddr_axi_arid != CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_ARID ||
                mmio_axi_arvalid || ddr_axi_awvalid || ddr_axi_wvalid) begin
                fail_now("C++ trace invline multi-master second AR mismatch");
            end
            second_arid = ddr_axi_arid;
            ddr_axi_arready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            ddr_axi_arready = 1'b0;
            read_req_valid[CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_MASTER] = 1'b0;

            invalidate_line_valid = 1'b1;
            ddr_axi_rid = first_arid;
            ddr_axi_rdata = CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_RBEAT0;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b0;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_RREADY_PENDING ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline multi-master first R beat0 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            ddr_axi_rid = first_arid;
            ddr_axi_rdata = CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_RBEAT1;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b1;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_RREADY_PENDING ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline multi-master first R beat1 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            timeout = 260;
            while (!read_resp_valid[CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_MASTER] &&
                   (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (invalidate_line_accepted) begin
                    fail_now("C++ trace invline multi-master accepted before first response");
                end
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace invline multi-master first response timeout");
            end
            #1;
            if (read_resp_id[(CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_RESP_ID ||
                read_resp_data[(CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_RESP_DATA ||
                (!invalidate_line_accepted) !== CPP_MODE1_INVLINE_MULTI_MASTER_READ_BLOCKED_FIRST_HELD) begin
                fail_now("C++ trace invline multi-master first response mismatch");
            end

            ddr_axi_rid = second_arid;
            ddr_axi_rdata = CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_RBEAT0;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b0;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (!read_resp_valid[CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_MASTER] ||
                ddr_axi_rready !== CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_RREADY_FIRST_HELD ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline multi-master second R beat0 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            ddr_axi_rid = second_arid;
            ddr_axi_rdata = CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_RBEAT1;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b1;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (!read_resp_valid[CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_MASTER] ||
                ddr_axi_rready !== CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_RREADY_FIRST_HELD ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline multi-master second R beat1 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            read_resp_ready[CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_MASTER] = 1'b1;
            #1;
            if (read_resp_id[(CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_RESP_ID ||
                read_resp_data[(CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_RESP_DATA ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline multi-master first retire mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            read_resp_ready[CPP_MODE1_INVLINE_MULTI_MASTER_READ_FIRST_MASTER] = 1'b0;

            timeout = 260;
            while (!read_resp_valid[CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_MASTER] &&
                   (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (invalidate_line_accepted) begin
                    fail_now("C++ trace invline multi-master accepted before second response");
                end
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace invline multi-master second response timeout");
            end
            #1;
            if (read_resp_id[(CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_RESP_ID ||
                read_resp_data[(CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_RESP_DATA ||
                (!invalidate_line_accepted) !== CPP_MODE1_INVLINE_MULTI_MASTER_READ_BLOCKED_SECOND_HELD) begin
                fail_now("C++ trace invline multi-master second response mismatch");
            end

            read_resp_ready[CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_MASTER] = 1'b1;
            #1;
            if (read_resp_id[(CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_RESP_ID ||
                read_resp_data[(CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_RESP_DATA ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline multi-master second retire mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            read_resp_ready[CPP_MODE1_INVLINE_MULTI_MASTER_READ_SECOND_MASTER] = 1'b0;
            ...
            if (accepted_seen !== CPP_MODE1_INVLINE_MULTI_MASTER_READ_ACCEPTED_AFTER_RETIRE) begin
                fail_now("C++ trace invline multi-master final accept mismatch");
            end
            @(negedge clk);
            invalidate_line_valid = 1'b0;
            invalidate_line_addr = {ADDR_BITS{1'b0}};
    ...
    task issue_mode1_invalidate_line_cache_read_write_and_check;
        integer timeout;
        reg accepted_seen;
        reg [AXI_ID_BITS-1:0] read_arid;
        reg [AXI_ID_BITS-1:0] write_arid;
        begin
            reset_dut();
            enter_mode(MODE_CACHE);
            @(negedge clk);
            ...
            ddr_axi_arready = 1'b0;
            invalidate_line_valid = 1'b0;
            invalidate_line_addr = CPP_MODE1_INVLINE_CACHE_RW_INVALIDATE_ADDR;

            read_req_addr[(CPP_MODE1_INVLINE_CACHE_RW_READ_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVLINE_CACHE_RW_READ_REQ_ADDR;
            read_req_total_size[(CPP_MODE1_INVLINE_CACHE_RW_READ_MASTER * 8) +: 8] =
                CPP_MODE1_INVLINE_CACHE_RW_READ_REQ_SIZE;
            read_req_id[(CPP_MODE1_INVLINE_CACHE_RW_READ_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVLINE_CACHE_RW_READ_REQ_ID;
            read_req_bypass[CPP_MODE1_INVLINE_CACHE_RW_READ_MASTER] = 1'b0;
            read_req_valid[CPP_MODE1_INVLINE_CACHE_RW_READ_MASTER] = 1'b1;
            timeout = 240;
            accepted_seen = 1'b0;
            while (!ddr_axi_arvalid && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (read_req_accepted[CPP_MODE1_INVLINE_CACHE_RW_READ_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (timeout == 0 || !accepted_seen) begin
                fail_now("C++ trace invline cache RW read AR timeout");
            end
            #1;
            if (ddr_axi_araddr != CPP_MODE1_INVLINE_CACHE_RW_READ_ARADDR ||
                ddr_axi_arlen != CPP_MODE1_INVLINE_CACHE_RW_READ_ARLEN ||
                ddr_axi_arsize != CPP_MODE1_INVLINE_CACHE_RW_READ_ARSIZE ||
                ddr_axi_arburst != CPP_MODE1_INVLINE_CACHE_RW_READ_ARBURST ||
                ddr_axi_arid != CPP_MODE1_INVLINE_CACHE_RW_READ_ARID ||
                mmio_axi_arvalid || ddr_axi_awvalid || ddr_axi_wvalid) begin
                fail_now("C++ trace invline cache RW read AR mismatch");
            end
            read_arid = ddr_axi_arid;
            ddr_axi_arready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            ddr_axi_arready = 1'b0;
            read_req_valid[CPP_MODE1_INVLINE_CACHE_RW_READ_MASTER] = 1'b0;

            write_req_addr[(CPP_MODE1_INVLINE_CACHE_RW_WRITE_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVLINE_CACHE_RW_WRITE_REQ_ADDR;
            write_req_total_size[(CPP_MODE1_INVLINE_CACHE_RW_WRITE_MASTER * 8) +: 8] =
                CPP_MODE1_INVLINE_CACHE_RW_WRITE_REQ_SIZE;
            write_req_id[(CPP_MODE1_INVLINE_CACHE_RW_WRITE_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVLINE_CACHE_RW_WRITE_REQ_ID;
            write_req_wdata[(CPP_MODE1_INVLINE_CACHE_RW_WRITE_MASTER * LINE_BITS) +: LINE_BITS] =
                CPP_MODE1_INVLINE_CACHE_RW_WRITE_REQ_WDATA;
            write_req_wstrb[(CPP_MODE1_INVLINE_CACHE_RW_WRITE_MASTER * LINE_BYTES) +: LINE_BYTES] =
                CPP_MODE1_INVLINE_CACHE_RW_WRITE_REQ_WSTRB[LINE_BYTES-1:0];
            write_req_bypass[CPP_MODE1_INVLINE_CACHE_RW_WRITE_MASTER] = 1'b0;
            write_req_valid[CPP_MODE1_INVLINE_CACHE_RW_WRITE_MASTER] = 1'b1;
            timeout = 240;
            accepted_seen = 1'b0;
            while (!ddr_axi_arvalid && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (write_req_accepted[CPP_MODE1_INVLINE_CACHE_RW_WRITE_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (timeout == 0 || !accepted_seen) begin
                fail_now("C++ trace invline cache RW write AR timeout");
            end
            #1;
            if (ddr_axi_araddr != CPP_MODE1_INVLINE_CACHE_RW_WRITE_REFILL_ARADDR ||
                ddr_axi_arlen != CPP_MODE1_INVLINE_CACHE_RW_WRITE_REFILL_ARLEN ||
                ddr_axi_arsize != CPP_MODE1_INVLINE_CACHE_RW_WRITE_REFILL_ARSIZE ||
                ddr_axi_arburst != CPP_MODE1_INVLINE_CACHE_RW_WRITE_REFILL_ARBURST ||
                ddr_axi_arid != CPP_MODE1_INVLINE_CACHE_RW_WRITE_REFILL_ARID ||
                mmio_axi_arvalid || ddr_axi_awvalid || ddr_axi_wvalid) begin
                fail_now("C++ trace invline cache RW write refill AR mismatch");
            end
            write_arid = ddr_axi_arid;
            if (write_arid == read_arid) begin
                fail_now("C++ trace invline cache RW reused ARID");
            end
            ddr_axi_arready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            ddr_axi_arready = 1'b0;
            write_req_valid[CPP_MODE1_INVLINE_CACHE_RW_WRITE_MASTER] = 1'b0;

            invalidate_line_valid = 1'b1;
            ddr_axi_rid = read_arid;
            ddr_axi_rdata = CPP_MODE1_INVLINE_CACHE_RW_READ_RBEAT0;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b0;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVLINE_CACHE_RW_READ_RREADY_PENDING ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline cache RW read R beat0 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            ddr_axi_rid = read_arid;
            ddr_axi_rdata = CPP_MODE1_INVLINE_CACHE_RW_READ_RBEAT1;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b1;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVLINE_CACHE_RW_READ_RREADY_PENDING ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline cache RW read R beat1 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            timeout = 260;
            while (!read_resp_valid[CPP_MODE1_INVLINE_CACHE_RW_READ_MASTER] &&
                   (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (invalidate_line_accepted) begin
                    fail_now("C++ trace invline cache RW accepted before read response");
                end
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace invline cache RW read response timeout");
            end
            #1;
            if (read_resp_id[(CPP_MODE1_INVLINE_CACHE_RW_READ_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVLINE_CACHE_RW_READ_RESP_ID ||
                read_resp_data[(CPP_MODE1_INVLINE_CACHE_RW_READ_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_INVLINE_CACHE_RW_READ_RESP_DATA ||
                (!invalidate_line_accepted) !== CPP_MODE1_INVLINE_CACHE_RW_BLOCKED_READ_HELD) begin
                fail_now("C++ trace invline cache RW read response mismatch");
            end

            ddr_axi_rid = write_arid;
            ddr_axi_rdata = CPP_MODE1_INVLINE_CACHE_RW_WRITE_REFILL_RBEAT0;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b0;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVLINE_CACHE_RW_WRITE_RREADY_READ_HELD ||
                !read_resp_valid[CPP_MODE1_INVLINE_CACHE_RW_READ_MASTER] ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline cache RW write refill R beat0 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            ddr_axi_rid = write_arid;
            ddr_axi_rdata = CPP_MODE1_INVLINE_CACHE_RW_WRITE_REFILL_RBEAT1;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b1;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVLINE_CACHE_RW_WRITE_RREADY_READ_HELD ||
                !read_resp_valid[CPP_MODE1_INVLINE_CACHE_RW_READ_MASTER] ||
                invalidate_line_accepted) begin
                fail_now("C++ trace invline cache RW write refill R beat1 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            timeout = 260;
            while (!write_resp_valid[CPP_MODE1_INVLINE_CACHE_RW_WRITE_MASTER] &&
                   (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (!read_resp_valid[CPP_MODE1_INVLINE_CACHE_RW_READ_MASTER] ||
                    invalidate_line_accepted) begin
                    fail_now("C++ trace invline cache RW response drain ordering mismatch");
                end
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace invline cache RW write response timeout");
            end
            #1;
            if (write_resp_id[(CPP_MODE1_INVLINE_CACHE_RW_WRITE_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVLINE_CACHE_RW_WRITE_RESP_ID ||
                write_resp_code[(CPP_MODE1_INVLINE_CACHE_RW_WRITE_MASTER * 2) +: 2] !=
                    CPP_MODE1_INVLINE_CACHE_RW_WRITE_RESP_CODE ||
                (!invalidate_line_accepted) !== CPP_MODE1_INVLINE_CACHE_RW_BLOCKED_WRITE_HELD) begin
                fail_now("C++ trace invline cache RW write response mismatch");
            end

            read_resp_ready[CPP_MODE1_INVLINE_CACHE_RW_READ_MASTER] = 1'b1;
            #1;
            if (invalidate_line_accepted) begin
                fail_now("C++ trace invline cache RW accepted before read retire");
            end
            @(posedge clk);
            @(negedge clk);
            read_resp_ready[CPP_MODE1_INVLINE_CACHE_RW_READ_MASTER] = 1'b0;

            write_resp_ready[CPP_MODE1_INVLINE_CACHE_RW_WRITE_MASTER] = 1'b1;
            #1;
            if (invalidate_line_accepted) begin
                fail_now("C++ trace invline cache RW accepted before write retire");
            end
            @(posedge clk);
            @(negedge clk);
            write_resp_ready[CPP_MODE1_INVLINE_CACHE_RW_WRITE_MASTER] = 1'b0;
            ...
            if (accepted_seen !== CPP_MODE1_INVLINE_CACHE_RW_ACCEPTED_AFTER_RETIRE) begin
                fail_now("C++ trace invline cache RW final accept mismatch");
            end
            @(negedge clk);
            invalidate_line_valid = 1'b0;
            invalidate_line_addr = {ADDR_BITS{1'b0}};
    ...
    task issue_mode1_invalidate_all_cache_read_write_and_check;
        integer timeout;
        reg accepted_seen;
        reg [AXI_ID_BITS-1:0] read_arid;
        reg [AXI_ID_BITS-1:0] write_arid;
        begin
            ...
            read_req_addr[(CPP_MODE1_INVALL_CACHE_RW_READ_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVALL_CACHE_RW_READ_REQ_ADDR;
            read_req_total_size[(CPP_MODE1_INVALL_CACHE_RW_READ_MASTER * 8) +: 8] =
                CPP_MODE1_INVALL_CACHE_RW_READ_REQ_SIZE;
            read_req_id[(CPP_MODE1_INVALL_CACHE_RW_READ_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVALL_CACHE_RW_READ_REQ_ID;
            read_req_bypass[CPP_MODE1_INVALL_CACHE_RW_READ_MASTER] = 1'b0;
            read_req_valid[CPP_MODE1_INVALL_CACHE_RW_READ_MASTER] = 1'b1;
            timeout = 240;
            accepted_seen = 1'b0;
            while (!ddr_axi_arvalid && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (read_req_accepted[CPP_MODE1_INVALL_CACHE_RW_READ_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (timeout == 0 || !accepted_seen) begin
                fail_now("C++ trace invall cache RW read AR timeout");
            end
            #1;
            if (ddr_axi_araddr != CPP_MODE1_INVALL_CACHE_RW_READ_ARADDR ||
                ddr_axi_arlen != CPP_MODE1_INVALL_CACHE_RW_READ_ARLEN ||
                ddr_axi_arsize != CPP_MODE1_INVALL_CACHE_RW_READ_ARSIZE ||
                ddr_axi_arburst != CPP_MODE1_INVALL_CACHE_RW_READ_ARBURST ||
                ddr_axi_arid != CPP_MODE1_INVALL_CACHE_RW_READ_ARID ||
                mmio_axi_arvalid || ddr_axi_awvalid || ddr_axi_wvalid) begin
                fail_now("C++ trace invall cache RW read AR mismatch");
            end
            read_arid = ddr_axi_arid;
            ddr_axi_arready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            ddr_axi_arready = 1'b0;
            read_req_valid[CPP_MODE1_INVALL_CACHE_RW_READ_MASTER] = 1'b0;

            write_req_addr[(CPP_MODE1_INVALL_CACHE_RW_WRITE_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_INVALL_CACHE_RW_WRITE_REQ_ADDR;
            write_req_total_size[(CPP_MODE1_INVALL_CACHE_RW_WRITE_MASTER * 8) +: 8] =
                CPP_MODE1_INVALL_CACHE_RW_WRITE_REQ_SIZE;
            write_req_id[(CPP_MODE1_INVALL_CACHE_RW_WRITE_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_INVALL_CACHE_RW_WRITE_REQ_ID;
            write_req_wdata[(CPP_MODE1_INVALL_CACHE_RW_WRITE_MASTER * LINE_BITS) +: LINE_BITS] =
                CPP_MODE1_INVALL_CACHE_RW_WRITE_REQ_WDATA;
            write_req_wstrb[(CPP_MODE1_INVALL_CACHE_RW_WRITE_MASTER * LINE_BYTES) +: LINE_BYTES] =
                CPP_MODE1_INVALL_CACHE_RW_WRITE_REQ_WSTRB[LINE_BYTES-1:0];
            write_req_bypass[CPP_MODE1_INVALL_CACHE_RW_WRITE_MASTER] = 1'b0;
            write_req_valid[CPP_MODE1_INVALL_CACHE_RW_WRITE_MASTER] = 1'b1;
            timeout = 240;
            accepted_seen = 1'b0;
            while (!ddr_axi_arvalid && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (write_req_accepted[CPP_MODE1_INVALL_CACHE_RW_WRITE_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (timeout == 0 || !accepted_seen) begin
                fail_now("C++ trace invall cache RW write AR timeout");
            end
            #1;
            if (ddr_axi_araddr != CPP_MODE1_INVALL_CACHE_RW_WRITE_REFILL_ARADDR ||
                ddr_axi_arlen != CPP_MODE1_INVALL_CACHE_RW_WRITE_REFILL_ARLEN ||
                ddr_axi_arsize != CPP_MODE1_INVALL_CACHE_RW_WRITE_REFILL_ARSIZE ||
                ddr_axi_arburst != CPP_MODE1_INVALL_CACHE_RW_WRITE_REFILL_ARBURST ||
                ddr_axi_arid != CPP_MODE1_INVALL_CACHE_RW_WRITE_REFILL_ARID ||
                mmio_axi_arvalid || ddr_axi_awvalid || ddr_axi_wvalid) begin
                fail_now("C++ trace invall cache RW write refill AR mismatch");
            end
            write_arid = ddr_axi_arid;
            if (write_arid == read_arid) begin
                fail_now("C++ trace invall cache RW reused ARID");
            end
            ddr_axi_arready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            ddr_axi_arready = 1'b0;
            write_req_valid[CPP_MODE1_INVALL_CACHE_RW_WRITE_MASTER] = 1'b0;

            invalidate_all_valid = 1'b1;
            ddr_axi_rid = read_arid;
            ddr_axi_rdata = CPP_MODE1_INVALL_CACHE_RW_READ_RBEAT0;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b0;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVALL_CACHE_RW_READ_RREADY_PENDING ||
                invalidate_all_accepted) begin
                fail_now("C++ trace invall cache RW read R beat0 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            ddr_axi_rid = read_arid;
            ddr_axi_rdata = CPP_MODE1_INVALL_CACHE_RW_READ_RBEAT1;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b1;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVALL_CACHE_RW_READ_RREADY_PENDING ||
                invalidate_all_accepted) begin
                fail_now("C++ trace invall cache RW read R beat1 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            timeout = 260;
            while (!read_resp_valid[CPP_MODE1_INVALL_CACHE_RW_READ_MASTER] &&
                   (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (invalidate_all_accepted) begin
                    fail_now("C++ trace invall cache RW accepted before read response");
                end
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace invall cache RW read response timeout");
            end
            #1;
            if (read_resp_id[(CPP_MODE1_INVALL_CACHE_RW_READ_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVALL_CACHE_RW_READ_RESP_ID ||
                read_resp_data[(CPP_MODE1_INVALL_CACHE_RW_READ_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_INVALL_CACHE_RW_READ_RESP_DATA ||
                (!invalidate_all_accepted) !== CPP_MODE1_INVALL_CACHE_RW_BLOCKED_READ_HELD) begin
                fail_now("C++ trace invall cache RW read response mismatch");
            end

            ddr_axi_rid = write_arid;
            ddr_axi_rdata = CPP_MODE1_INVALL_CACHE_RW_WRITE_REFILL_RBEAT0;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b0;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVALL_CACHE_RW_WRITE_RREADY_READ_HELD ||
                !read_resp_valid[CPP_MODE1_INVALL_CACHE_RW_READ_MASTER] ||
                invalidate_all_accepted) begin
                fail_now("C++ trace invall cache RW write refill R beat0 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            ddr_axi_rid = write_arid;
            ddr_axi_rdata = CPP_MODE1_INVALL_CACHE_RW_WRITE_REFILL_RBEAT1;
            ddr_axi_rresp = AXI_RESP_OKAY;
            ddr_axi_rlast = 1'b1;
            ddr_axi_rvalid = 1'b1;
            #1;
            if (ddr_axi_rready !== CPP_MODE1_INVALL_CACHE_RW_WRITE_RREADY_READ_HELD ||
                !read_resp_valid[CPP_MODE1_INVALL_CACHE_RW_READ_MASTER] ||
                invalidate_all_accepted) begin
                fail_now("C++ trace invall cache RW write refill R beat1 mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_rvalid = 1'b0;
            ...
            timeout = 260;
            while (!write_resp_valid[CPP_MODE1_INVALL_CACHE_RW_WRITE_MASTER] &&
                   (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (!read_resp_valid[CPP_MODE1_INVALL_CACHE_RW_READ_MASTER] ||
                    invalidate_all_accepted) begin
                    fail_now("C++ trace invall cache RW response drain ordering mismatch");
                end
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace invall cache RW write response timeout");
            end
            #1;
            if (write_resp_id[(CPP_MODE1_INVALL_CACHE_RW_WRITE_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_INVALL_CACHE_RW_WRITE_RESP_ID ||
                write_resp_code[(CPP_MODE1_INVALL_CACHE_RW_WRITE_MASTER * 2) +: 2] !=
                    CPP_MODE1_INVALL_CACHE_RW_WRITE_RESP_CODE ||
                (!invalidate_all_accepted) !== CPP_MODE1_INVALL_CACHE_RW_BLOCKED_WRITE_HELD) begin
                fail_now("C++ trace invall cache RW write response mismatch");
            end

            read_resp_ready[CPP_MODE1_INVALL_CACHE_RW_READ_MASTER] = 1'b1;
            #1;
            if (invalidate_all_accepted) begin
                fail_now("C++ trace invall cache RW accepted before read retire");
            end
            @(posedge clk);
            @(negedge clk);
            read_resp_ready[CPP_MODE1_INVALL_CACHE_RW_READ_MASTER] = 1'b0;

            write_resp_ready[CPP_MODE1_INVALL_CACHE_RW_WRITE_MASTER] = 1'b1;
            #1;
            if (invalidate_all_accepted) begin
                fail_now("C++ trace invall cache RW accepted before write retire");
            end
            @(posedge clk);
            @(negedge clk);
            write_resp_ready[CPP_MODE1_INVALL_CACHE_RW_WRITE_MASTER] = 1'b0;

            if (CPP_MODE1_INVALL_CACHE_RW_ACCEPTED_AFTER_RETIRE) begin
                timeout = 10000;
                accepted_seen = 1'b0;
                while (!accepted_seen && (timeout > 0)) begin
                    #1;
                ...
                if (!accepted_seen) begin
                    fail_now("C++ trace invall cache RW final accept timeout");
                end
            end else begin
                timeout = 64;
                while (timeout > 0) begin
                    #1;
                    if (invalidate_all_accepted) begin
                        fail_now("C++ trace invall cache RW accepted despite dirty line");
                    end
                    @(posedge clk);
                    timeout = timeout - 1;
                end
    ...
    task issue_mode1_dirty_victim_mmio_read_and_check;
        integer timeout;
        reg accepted_seen;
        begin
            reset_dut();
            enter_mode(MODE_CACHE);
            issue_mode1_dirty_victim_setup_write_and_check(
                CPP_MODE1_DIRTY_VICTIM_READ_SETUP0_REQ_ADDR,
                CPP_MODE1_DIRTY_VICTIM_READ_SETUP0_REQ_SIZE,
                CPP_MODE1_DIRTY_VICTIM_READ_SETUP0_REQ_ID,
                CPP_MODE1_DIRTY_VICTIM_READ_SETUP0_REQ_WDATA,
                CPP_MODE1_DIRTY_VICTIM_READ_SETUP0_REQ_WSTRB,
                CPP_MODE1_DIRTY_VICTIM_READ_SETUP0_RESP_ID,
                CPP_MODE1_DIRTY_VICTIM_READ_SETUP0_RESP_CODE);
            issue_mode1_dirty_victim_setup_write_and_check(
                CPP_MODE1_DIRTY_VICTIM_READ_SETUP1_REQ_ADDR,
                CPP_MODE1_DIRTY_VICTIM_READ_SETUP1_REQ_SIZE,
                CPP_MODE1_DIRTY_VICTIM_READ_SETUP1_REQ_ID,
                CPP_MODE1_DIRTY_VICTIM_READ_SETUP1_REQ_WDATA,
                CPP_MODE1_DIRTY_VICTIM_READ_SETUP1_REQ_WSTRB,
                CPP_MODE1_DIRTY_VICTIM_READ_SETUP1_RESP_ID,
                CPP_MODE1_DIRTY_VICTIM_READ_SETUP1_RESP_CODE);
            ...
            write_req_addr[(CPP_MODE1_DIRTY_VICTIM_READ_CACHE_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_DIRTY_VICTIM_READ_CACHE_WRITE_REQ_ADDR;
            write_req_total_size[(CPP_MODE1_DIRTY_VICTIM_READ_CACHE_MASTER * 8) +: 8] =
                CPP_MODE1_DIRTY_VICTIM_READ_CACHE_WRITE_REQ_SIZE;
            write_req_id[(CPP_MODE1_DIRTY_VICTIM_READ_CACHE_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_DIRTY_VICTIM_READ_CACHE_WRITE_REQ_ID;
            write_req_wdata[(CPP_MODE1_DIRTY_VICTIM_READ_CACHE_MASTER * LINE_BITS) +: LINE_BITS] =
                CPP_MODE1_DIRTY_VICTIM_READ_CACHE_WRITE_REQ_WDATA;
            write_req_wstrb[(CPP_MODE1_DIRTY_VICTIM_READ_CACHE_MASTER * LINE_BYTES) +: LINE_BYTES] =
                CPP_MODE1_DIRTY_VICTIM_READ_CACHE_WRITE_REQ_WSTRB[LINE_BYTES-1:0];
            write_req_bypass[CPP_MODE1_DIRTY_VICTIM_READ_CACHE_MASTER] = 1'b0;
            write_req_valid[CPP_MODE1_DIRTY_VICTIM_READ_CACHE_MASTER] = 1'b1;
            ...
            while (!ddr_axi_awvalid && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (write_req_accepted[CPP_MODE1_DIRTY_VICTIM_READ_CACHE_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace dirty-victim/read writeback AW timeout");
            end
            if (!accepted_seen && write_req_accepted[CPP_MODE1_DIRTY_VICTIM_READ_CACHE_MASTER]) begin
                accepted_seen = 1'b1;
            end
            if (!accepted_seen) begin
                fail_now("C++ trace dirty-victim/read cache write accept missing");
            end
            #1;
            if (ddr_axi_awaddr != CPP_MODE1_DIRTY_VICTIM_READ_WB_AWADDR ||
                ddr_axi_awlen != CPP_MODE1_DIRTY_VICTIM_READ_WB_AWLEN ||
                ddr_axi_awsize != CPP_MODE1_DIRTY_VICTIM_READ_WB_AWSIZE ||
                ddr_axi_awburst != CPP_MODE1_DIRTY_VICTIM_READ_WB_AWBURST ||
                ddr_axi_awid != CPP_MODE1_DIRTY_VICTIM_READ_WB_AWID ||
                mmio_axi_awvalid) begin
                fail_now("C++ trace dirty-victim/read writeback AW mismatch");
            end
            seen_ddr_awid = ddr_axi_awid;
            ddr_axi_awready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            write_req_valid[CPP_MODE1_DIRTY_VICTIM_READ_CACHE_MASTER] = 1'b0;
            ddr_axi_awready = 1'b0;
            ...
            if (timeout == 0) begin
                fail_now("C++ trace dirty-victim/read writeback W0 timeout");
            end
            #1;
            if (ddr_axi_wdata != CPP_MODE1_DIRTY_VICTIM_READ_WB_WBEAT0 ||
                ddr_axi_wstrb != CPP_MODE1_DIRTY_VICTIM_READ_WB_WSTRB0 ||
                ddr_axi_wlast != CPP_MODE1_DIRTY_VICTIM_READ_WB_WLAST0) begin
                fail_now("C++ trace dirty-victim/read writeback W0 mismatch");
            end
            ddr_axi_wready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace dirty-victim/read writeback W1 timeout");
            end
            #1;
            if (ddr_axi_wdata != CPP_MODE1_DIRTY_VICTIM_READ_WB_WBEAT1 ||
                ddr_axi_wstrb != CPP_MODE1_DIRTY_VICTIM_READ_WB_WSTRB1 ||
                ddr_axi_wlast != CPP_MODE1_DIRTY_VICTIM_READ_WB_WLAST1) begin
                fail_now("C++ trace dirty-victim/read writeback W1 mismatch");
            end
            ddr_axi_wready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            ...
            read_req_addr[(CPP_MODE1_DIRTY_VICTIM_READ_MMIO_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_DIRTY_VICTIM_MMIO_READ_REQ_ADDR;
            read_req_total_size[(CPP_MODE1_DIRTY_VICTIM_READ_MMIO_MASTER * 8) +: 8] =
                CPP_MODE1_DIRTY_VICTIM_MMIO_READ_REQ_SIZE;
            read_req_id[(CPP_MODE1_DIRTY_VICTIM_READ_MMIO_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_DIRTY_VICTIM_MMIO_READ_REQ_ID;
            read_req_bypass[CPP_MODE1_DIRTY_VICTIM_READ_MMIO_MASTER] = 1'b0;
            read_req_valid[CPP_MODE1_DIRTY_VICTIM_READ_MMIO_MASTER] = 1'b1;
            ...
            while (!mmio_axi_arvalid && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (read_req_accepted[CPP_MODE1_DIRTY_VICTIM_READ_MMIO_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace dirty-victim/read MMIO AR timeout");
            end
            if (!accepted_seen && read_req_accepted[CPP_MODE1_DIRTY_VICTIM_READ_MMIO_MASTER]) begin
                accepted_seen = 1'b1;
            end
            if (!accepted_seen) begin
                fail_now("C++ trace dirty-victim/read MMIO read accept missing");
            end
            #1;
            if (mmio_axi_araddr != CPP_MODE1_DIRTY_VICTIM_MMIO_READ_ARADDR ||
                mmio_axi_arlen != CPP_MODE1_DIRTY_VICTIM_MMIO_READ_ARLEN ||
                mmio_axi_arsize != CPP_MODE1_DIRTY_VICTIM_MMIO_READ_ARSIZE ||
                mmio_axi_arburst != CPP_MODE1_DIRTY_VICTIM_MMIO_READ_ARBURST ||
                mmio_axi_arid != CPP_MODE1_DIRTY_VICTIM_MMIO_READ_ARID ||
                ddr_axi_arvalid) begin
                fail_now("C++ trace dirty-victim/read MMIO AR mismatch");
            end
            seen_mmio_arid = mmio_axi_arid;
            mmio_axi_arready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            read_req_valid[CPP_MODE1_DIRTY_VICTIM_READ_MMIO_MASTER] = 1'b0;
            mmio_axi_arready = 1'b0;

            invalidate_all_valid = 1'b1;
            mmio_axi_rid = seen_mmio_arid;
            mmio_axi_rdata = CPP_MODE1_DIRTY_VICTIM_MMIO_READ_RBEAT0[MMIO_DATA_BITS-1:0];
            mmio_axi_rresp = AXI_RESP_OKAY;
            mmio_axi_rlast = 1'b1;
            mmio_axi_rvalid = 1'b1;
            #1;
            if (mmio_axi_rready !== CPP_MODE1_DIRTY_VICTIM_MMIO_READ_RREADY_STALLED ||
                invalidate_all_accepted) begin
                fail_now("C++ trace dirty-victim/read MMIO RREADY mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            mmio_axi_rvalid = 1'b0;
            ...
            timeout = 160;
            while (!read_resp_valid[CPP_MODE1_DIRTY_VICTIM_READ_MMIO_MASTER] &&
                   (timeout > 0)) begin
                #1;
                if (invalidate_all_accepted) begin
                    fail_now("C++ trace dirty-victim/read accepted before MMIO response");
                end
                @(posedge clk);
                timeout = timeout - 1;
            end
            if (timeout == 0 ||
                write_resp_valid[CPP_MODE1_DIRTY_VICTIM_READ_CACHE_MASTER]) begin
                fail_now("C++ trace dirty-victim/read MMIO/cache response owner mismatch");
            end
            #1;
            if (invalidate_all_accepted !==
                    !CPP_MODE1_DIRTY_VICTIM_READ_INVALL_BLOCKED_MMIO_HELD) begin
                fail_now("C++ trace dirty-victim/read MMIO held invalidate mismatch");
            end
            ...
            ddr_axi_bvalid = 1'b1;
            #1;
            if (ddr_axi_bready !== CPP_MODE1_DIRTY_VICTIM_READ_WB_BREADY_STALLED ||
                invalidate_all_accepted) begin
                fail_now("C++ trace dirty-victim/read DDR BREADY mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_bvalid = 1'b0;
            ...
            #1;
            if (read_resp_id[(CPP_MODE1_DIRTY_VICTIM_READ_MMIO_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_DIRTY_VICTIM_MMIO_READ_RESP_ID ||
                read_resp_data[(CPP_MODE1_DIRTY_VICTIM_READ_MMIO_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_DIRTY_VICTIM_MMIO_READ_RESP_DATA ||
                invalidate_all_accepted) begin
                fail_now("C++ trace dirty-victim/read MMIO response payload mismatch");
            end
            read_resp_ready[CPP_MODE1_DIRTY_VICTIM_READ_MMIO_MASTER] = 1'b1;
            #1;
            if (invalidate_all_accepted) begin
                fail_now("C++ trace dirty-victim/read accepted before MMIO retire");
            end
            @(posedge clk);
            @(negedge clk);
            read_resp_ready[CPP_MODE1_DIRTY_VICTIM_READ_MMIO_MASTER] = 1'b0;

            timeout = 260;
            while (!write_resp_valid[CPP_MODE1_DIRTY_VICTIM_READ_CACHE_MASTER] &&
                   (timeout > 0)) begin
                #1;
                if (invalidate_all_accepted) begin
                    fail_now("C++ trace dirty-victim/read accepted before cache response");
                end
                @(posedge clk);
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace dirty-victim/read cache response timeout");
            end
            #1;
            if (write_resp_id[(CPP_MODE1_DIRTY_VICTIM_READ_CACHE_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_DIRTY_VICTIM_READ_CACHE_WRITE_RESP_ID ||
                write_resp_code[(CPP_MODE1_DIRTY_VICTIM_READ_CACHE_MASTER * 2) +: 2] !=
                    CPP_MODE1_DIRTY_VICTIM_READ_CACHE_WRITE_RESP_CODE ||
                invalidate_all_accepted !==
                    !CPP_MODE1_DIRTY_VICTIM_READ_INVALL_BLOCKED_CACHE_HELD) begin
                fail_now("C++ trace dirty-victim/read cache response payload mismatch");
            end
            write_resp_ready[CPP_MODE1_DIRTY_VICTIM_READ_CACHE_MASTER] = 1'b1;
            #1;
            if (invalidate_all_accepted) begin
                fail_now("C++ trace dirty-victim/read accepted before cache retire");
            end
            @(posedge clk);
            @(negedge clk);
            write_resp_ready[CPP_MODE1_DIRTY_VICTIM_READ_CACHE_MASTER] = 1'b0;

            timeout = 32;
            while (timeout > 0) begin
                #1;
                if (invalidate_all_accepted !==
                    CPP_MODE1_DIRTY_VICTIM_READ_INVALL_ACCEPTED_AFTER_RETIRE) begin
                    fail_now("C++ trace dirty-victim/read final invalidate mismatch");
                end
                @(posedge clk);
                timeout = timeout - 1;
            end
            ...
            invalidate_line_addr = CPP_MODE1_DIRTY_VICTIM_READ_INVLINE_ADDR;
            invalidate_line_valid = 1'b1;
            timeout = 260;
            accepted_seen = 1'b0;
            while (!accepted_seen && (timeout > 0)) begin
            ...
            if (accepted_seen !==
                CPP_MODE1_DIRTY_VICTIM_READ_INVLINE_ACCEPTED_AFTER_RETIRE) begin
                fail_now("C++ trace dirty-victim/read invalidate_line final mismatch");
            end
            invalidate_line_valid = 1'b0;
            invalidate_line_addr = {ADDR_BITS{1'b0}};
        end
    ...
    task issue_mode1_dirty_victim_mmio_rw_and_check;
        integer timeout;
        reg accepted_seen;
        begin
            reset_dut();
            enter_mode(MODE_CACHE);
            issue_mode1_dirty_victim_setup_write_and_check(
                CPP_MODE1_DIRTY_VICTIM_RW_SETUP0_REQ_ADDR,
                CPP_MODE1_DIRTY_VICTIM_RW_SETUP0_REQ_SIZE,
                CPP_MODE1_DIRTY_VICTIM_RW_SETUP0_REQ_ID,
                CPP_MODE1_DIRTY_VICTIM_RW_SETUP0_REQ_WDATA,
                CPP_MODE1_DIRTY_VICTIM_RW_SETUP0_REQ_WSTRB,
                CPP_MODE1_DIRTY_VICTIM_RW_SETUP0_RESP_ID,
                CPP_MODE1_DIRTY_VICTIM_RW_SETUP0_RESP_CODE);
            issue_mode1_dirty_victim_setup_write_and_check(
                CPP_MODE1_DIRTY_VICTIM_RW_SETUP1_REQ_ADDR,
                CPP_MODE1_DIRTY_VICTIM_RW_SETUP1_REQ_SIZE,
                CPP_MODE1_DIRTY_VICTIM_RW_SETUP1_REQ_ID,
                CPP_MODE1_DIRTY_VICTIM_RW_SETUP1_REQ_WDATA,
                CPP_MODE1_DIRTY_VICTIM_RW_SETUP1_REQ_WSTRB,
                CPP_MODE1_DIRTY_VICTIM_RW_SETUP1_RESP_ID,
                CPP_MODE1_DIRTY_VICTIM_RW_SETUP1_RESP_CODE);
            ...
            write_req_addr[(CPP_MODE1_DIRTY_VICTIM_RW_CACHE_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_DIRTY_VICTIM_RW_CACHE_WRITE_REQ_ADDR;
            write_req_total_size[(CPP_MODE1_DIRTY_VICTIM_RW_CACHE_MASTER * 8) +: 8] =
                CPP_MODE1_DIRTY_VICTIM_RW_CACHE_WRITE_REQ_SIZE;
            write_req_id[(CPP_MODE1_DIRTY_VICTIM_RW_CACHE_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_DIRTY_VICTIM_RW_CACHE_WRITE_REQ_ID;
            write_req_wdata[(CPP_MODE1_DIRTY_VICTIM_RW_CACHE_MASTER * LINE_BITS) +: LINE_BITS] =
                CPP_MODE1_DIRTY_VICTIM_RW_CACHE_WRITE_REQ_WDATA;
            write_req_wstrb[(CPP_MODE1_DIRTY_VICTIM_RW_CACHE_MASTER * LINE_BYTES) +: LINE_BYTES] =
                CPP_MODE1_DIRTY_VICTIM_RW_CACHE_WRITE_REQ_WSTRB[LINE_BYTES-1:0];
            write_req_bypass[CPP_MODE1_DIRTY_VICTIM_RW_CACHE_MASTER] = 1'b0;
            write_req_valid[CPP_MODE1_DIRTY_VICTIM_RW_CACHE_MASTER] = 1'b1;
            ...
            while (!ddr_axi_awvalid && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (write_req_accepted[CPP_MODE1_DIRTY_VICTIM_RW_CACHE_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace dirty-victim/RW writeback AW timeout");
            end
            if (!accepted_seen && write_req_accepted[CPP_MODE1_DIRTY_VICTIM_RW_CACHE_MASTER]) begin
                accepted_seen = 1'b1;
            end
            if (!accepted_seen) begin
                fail_now("C++ trace dirty-victim/RW cache write accept missing");
            end
            #1;
            if (ddr_axi_awaddr != CPP_MODE1_DIRTY_VICTIM_RW_WB_AWADDR ||
                ddr_axi_awlen != CPP_MODE1_DIRTY_VICTIM_RW_WB_AWLEN ||
                ddr_axi_awsize != CPP_MODE1_DIRTY_VICTIM_RW_WB_AWSIZE ||
                ddr_axi_awburst != CPP_MODE1_DIRTY_VICTIM_RW_WB_AWBURST ||
                ddr_axi_awid != CPP_MODE1_DIRTY_VICTIM_RW_WB_AWID ||
                mmio_axi_awvalid) begin
                fail_now("C++ trace dirty-victim/RW writeback AW mismatch");
            end
            seen_ddr_awid = ddr_axi_awid;
            ddr_axi_awready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            write_req_valid[CPP_MODE1_DIRTY_VICTIM_RW_CACHE_MASTER] = 1'b0;
            ddr_axi_awready = 1'b0;
            ...
            if (timeout == 0) begin
                fail_now("C++ trace dirty-victim/RW writeback W0 timeout");
            end
            #1;
            if (ddr_axi_wdata != CPP_MODE1_DIRTY_VICTIM_RW_WB_WBEAT0 ||
                ddr_axi_wstrb != CPP_MODE1_DIRTY_VICTIM_RW_WB_WSTRB0 ||
                ddr_axi_wlast != CPP_MODE1_DIRTY_VICTIM_RW_WB_WLAST0) begin
                fail_now("C++ trace dirty-victim/RW writeback W0 mismatch");
            end
            ddr_axi_wready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace dirty-victim/RW writeback W1 timeout");
            end
            #1;
            if (ddr_axi_wdata != CPP_MODE1_DIRTY_VICTIM_RW_WB_WBEAT1 ||
                ddr_axi_wstrb != CPP_MODE1_DIRTY_VICTIM_RW_WB_WSTRB1 ||
                ddr_axi_wlast != CPP_MODE1_DIRTY_VICTIM_RW_WB_WLAST1) begin
                fail_now("C++ trace dirty-victim/RW writeback W1 mismatch");
            end
            ddr_axi_wready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            ...
            read_req_addr[(CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_REQ_ADDR;
            read_req_total_size[(CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_MASTER * 8) +: 8] =
                CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_REQ_SIZE;
            read_req_id[(CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_REQ_ID;
            read_req_bypass[CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_MASTER] = 1'b0;
            read_req_valid[CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_MASTER] = 1'b1;
            ...
            while (!mmio_axi_arvalid && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (read_req_accepted[CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace dirty-victim/RW MMIO AR timeout");
            end
            if (!accepted_seen && read_req_accepted[CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_MASTER]) begin
                accepted_seen = 1'b1;
            end
            if (!accepted_seen) begin
                fail_now("C++ trace dirty-victim/RW MMIO read accept missing");
            end
            #1;
            if (mmio_axi_araddr != CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_ARADDR ||
                mmio_axi_arlen != CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_ARLEN ||
                mmio_axi_arsize != CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_ARSIZE ||
                mmio_axi_arburst != CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_ARBURST ||
                mmio_axi_arid != CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_ARID ||
                ddr_axi_arvalid) begin
                fail_now("C++ trace dirty-victim/RW MMIO AR mismatch");
            end
            seen_mmio_arid = mmio_axi_arid;
            mmio_axi_arready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            read_req_valid[CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_MASTER] = 1'b0;
            mmio_axi_arready = 1'b0;

            write_req_addr[(CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_MASTER * ADDR_BITS) +: ADDR_BITS] =
                CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_REQ_ADDR;
            write_req_total_size[(CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_MASTER * 8) +: 8] =
                CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_REQ_SIZE;
            write_req_id[(CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_MASTER * ID_BITS) +: ID_BITS] =
                CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_REQ_ID;
            write_req_wdata[(CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_MASTER * LINE_BITS) +: LINE_BITS] =
                CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_REQ_WDATA;
            write_req_wstrb[(CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_MASTER * LINE_BYTES) +: LINE_BYTES] =
                CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_REQ_WSTRB[LINE_BYTES-1:0];
            write_req_bypass[CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_MASTER] = 1'b0;
            write_req_valid[CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_MASTER] = 1'b1;
            ...
            while (!accepted_seen && (timeout > 0)) begin
                @(posedge clk);
                #1;
                if (write_req_accepted[CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_MASTER]) begin
                    accepted_seen = 1'b1;
                end
                timeout = timeout - 1;
            end
            if (!accepted_seen) begin
                fail_now("C++ trace dirty-victim/RW MMIO write accept timeout");
            end
            write_req_valid[CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_MASTER] = 1'b0;
            @(negedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace dirty-victim/RW MMIO AW timeout");
            end
            #1;
            if (mmio_axi_awaddr != CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_AWADDR ||
                mmio_axi_awlen != CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_AWLEN ||
                mmio_axi_awsize != CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_AWSIZE ||
                mmio_axi_awburst != CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_AWBURST ||
                mmio_axi_awid != CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_AWID ||
                ddr_axi_awvalid) begin
                fail_now("C++ trace dirty-victim/RW MMIO AW mismatch");
            end
            seen_mmio_awid = mmio_axi_awid;
            mmio_axi_awready = 1'b1;
            @(posedge clk);
            ...
            if (timeout == 0) begin
                fail_now("C++ trace dirty-victim/RW MMIO W timeout");
            end
            #1;
            if (mmio_axi_wdata != CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_WBEAT0[MMIO_DATA_BITS-1:0] ||
                mmio_axi_wstrb != CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_WSTRB0[MMIO_STRB_BITS-1:0] ||
                mmio_axi_wlast != CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_WLAST0 ||
                ddr_axi_wvalid) begin
                fail_now("C++ trace dirty-victim/RW MMIO W mismatch");
            end
            mmio_axi_wready = 1'b1;
            @(posedge clk);
            @(negedge clk);
            ...
            invalidate_all_valid = 1'b1;
            mmio_axi_rid = seen_mmio_arid;
            mmio_axi_rdata = CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_RBEAT0[MMIO_DATA_BITS-1:0];
            mmio_axi_rresp = AXI_RESP_OKAY;
            mmio_axi_rlast = 1'b1;
            mmio_axi_rvalid = 1'b1;
            mmio_axi_bid = seen_mmio_awid;
            ...
            mmio_axi_bvalid = 1'b1;
            #1;
            if (mmio_axi_rready !== CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_RREADY_STALLED ||
                mmio_axi_bready !== CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_BREADY_STALLED ||
                invalidate_all_accepted) begin
                fail_now("C++ trace dirty-victim/RW MMIO R/B ready mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            mmio_axi_rvalid = 1'b0;
            ...
            timeout = 160;
            while (!(read_resp_valid[CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_MASTER] &&
                     write_resp_valid[CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_MASTER]) &&
                   (timeout > 0)) begin
                #1;
                if (invalidate_all_accepted) begin
                    fail_now("C++ trace dirty-victim/RW accepted before MMIO responses");
                end
                @(posedge clk);
                timeout = timeout - 1;
            end
            if (timeout == 0 ||
                write_resp_valid[CPP_MODE1_DIRTY_VICTIM_RW_CACHE_MASTER]) begin
                fail_now("C++ trace dirty-victim/RW MMIO/cache response owner mismatch");
            end
            #1;
            if (invalidate_all_accepted !==
                    !CPP_MODE1_DIRTY_VICTIM_RW_INVALL_BLOCKED_MMIO_HELD) begin
                fail_now("C++ trace dirty-victim/RW MMIO held invalidate mismatch");
            end
            ...
            ddr_axi_bvalid = 1'b1;
            #1;
            if (ddr_axi_bready !== CPP_MODE1_DIRTY_VICTIM_RW_WB_BREADY_STALLED ||
                invalidate_all_accepted) begin
                fail_now("C++ trace dirty-victim/RW DDR BREADY mismatch");
            end
            @(posedge clk);
            @(negedge clk);
            ddr_axi_bvalid = 1'b0;
            ...
            #1;
            if (read_resp_id[(CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_RESP_ID ||
                read_resp_data[(CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_MASTER * READ_RESP_BITS) +: READ_RESP_BITS] !=
                    CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_RESP_DATA ||
                invalidate_all_accepted) begin
                fail_now("C++ trace dirty-victim/RW MMIO read payload mismatch");
            end
            read_resp_ready[CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_MASTER] = 1'b1;
            #1;
            if (invalidate_all_accepted) begin
                fail_now("C++ trace dirty-victim/RW accepted before MMIO read retire");
            end
            @(posedge clk);
            @(negedge clk);
            read_resp_ready[CPP_MODE1_DIRTY_VICTIM_RW_MMIO_READ_MASTER] = 1'b0;

            #1;
            if (write_resp_id[(CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_RESP_ID ||
                write_resp_code[(CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_MASTER * 2) +: 2] !=
                    CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_RESP_CODE ||
                invalidate_all_accepted) begin
                fail_now("C++ trace dirty-victim/RW MMIO write payload mismatch");
            end
            write_resp_ready[CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_MASTER] = 1'b1;
            #1;
            if (invalidate_all_accepted) begin
                fail_now("C++ trace dirty-victim/RW accepted before MMIO write retire");
            end
            @(posedge clk);
            @(negedge clk);
            write_resp_ready[CPP_MODE1_DIRTY_VICTIM_RW_MMIO_WRITE_MASTER] = 1'b0;

            timeout = 260;
            while (!write_resp_valid[CPP_MODE1_DIRTY_VICTIM_RW_CACHE_MASTER] &&
                   (timeout > 0)) begin
                #1;
                if (invalidate_all_accepted) begin
                    fail_now("C++ trace dirty-victim/RW accepted before cache response");
                end
                @(posedge clk);
                timeout = timeout - 1;
            end
            if (timeout == 0) begin
                fail_now("C++ trace dirty-victim/RW cache response timeout");
            end
            #1;
            if (write_resp_id[(CPP_MODE1_DIRTY_VICTIM_RW_CACHE_MASTER * ID_BITS) +: ID_BITS] !=
                    CPP_MODE1_DIRTY_VICTIM_RW_CACHE_WRITE_RESP_ID ||
                write_resp_code[(CPP_MODE1_DIRTY_VICTIM_RW_CACHE_MASTER * 2) +: 2] !=
                    CPP_MODE1_DIRTY_VICTIM_RW_CACHE_WRITE_RESP_CODE ||
                invalidate_all_accepted !==
                    !CPP_MODE1_DIRTY_VICTIM_RW_INVALL_BLOCKED_CACHE_HELD) begin
                fail_now("C++ trace dirty-victim/RW cache response payload mismatch");
            end
            write_resp_ready[CPP_MODE1_DIRTY_VICTIM_RW_CACHE_MASTER] = 1'b1;
            #1;
            if (invalidate_all_accepted) begin
                fail_now("C++ trace dirty-victim/RW accepted before cache retire");
            end
            @(posedge clk);
            @(negedge clk);
            write_resp_ready[CPP_MODE1_DIRTY_VICTIM_RW_CACHE_MASTER] = 1'b0;

            timeout = 32;
            while (timeout > 0) begin
                #1;
                if (invalidate_all_accepted !==
                    CPP_MODE1_DIRTY_VICTIM_RW_INVALL_ACCEPTED_AFTER_RETIRE) begin
                    fail_now("C++ trace dirty-victim/RW final invalidate mismatch");
                end
                @(posedge clk);
                timeout = timeout - 1;
            end
            ...
            invalidate_line_addr = CPP_MODE1_DIRTY_VICTIM_RW_INVLINE_ADDR;
            invalidate_line_valid = 1'b1;
            timeout = 260;
            accepted_seen = 1'b0;
            while (!accepted_seen && (timeout > 0)) begin
            ...
            if (accepted_seen !==
                CPP_MODE1_DIRTY_VICTIM_RW_INVLINE_ACCEPTED_AFTER_RETIRE) begin
                fail_now("C++ trace dirty-victim/RW invalidate_line final mismatch");
            end
            invalidate_line_valid = 1'b0;
            invalidate_line_addr = {ADDR_BITS{1'b0}};
        end
    ...
    initial begin
        ...
        issue_mode1_invalidate_line_multi_master_recovery_read_and_check();
        issue_mode1_seeded_maintenance_recovery_and_check();
        ...
        issue_mode1_invalidate_line_multi_master_read_and_check();
        issue_mode1_invalidate_line_multi_read_and_check();
        issue_mode1_invalidate_line_cache_read_write_and_check();
        issue_mode1_invalidate_all_cache_read_write_and_check();
        ...
        issue_mode1_invalidate_line_cache_mmio_write_and_check();
        issue_mode1_invalidate_line_cache_mmio_rw_and_check();
        issue_mode1_invalidate_line_other_cache_mmio_rw_and_check();
        issue_mode1_invalidate_line_cache_write_mmio_rw_and_check();
        issue_mode1_invalidate_all_cache_write_mmio_rw_and_check();
        issue_mode1_invalidate_all_cache_mmio_rw_and_check();
        issue_mode1_dirty_victim_mmio_write_and_check();
        issue_mode1_dirty_victim_mmio_read_and_check();
        issue_mode1_dirty_victim_mmio_rw_and_check();
        ...
        issue_mode1_to_mode2_pending_mmio_write_and_check(
            CPP_MODE1_TO_MODE2_PENDING_MMIO_WRITE_REQ_ADDR,
            CPP_MODE1_TO_MODE2_PENDING_MMIO_WRITE_REQ_SIZE,
            CPP_MODE1_TO_MODE2_PENDING_MMIO_WRITE_REQ_ID,
            CPP_MODE1_TO_MODE2_PENDING_MMIO_WRITE_REQ_WDATA,
            CPP_MODE1_TO_MODE2_PENDING_MMIO_WRITE_REQ_WSTRB,
            CPP_MODE1_TO_MODE2_PENDING_MMIO_WRITE_AWADDR,
            CPP_MODE1_TO_MODE2_PENDING_MMIO_WRITE_AWLEN,
            CPP_MODE1_TO_MODE2_PENDING_MMIO_WRITE_AWSIZE,
            CPP_MODE1_TO_MODE2_PENDING_MMIO_WRITE_AWBURST,
            CPP_MODE1_TO_MODE2_PENDING_MMIO_WRITE_AWID,
            CPP_MODE1_TO_MODE2_PENDING_MMIO_WRITE_BEATS,
            CPP_MODE1_TO_MODE2_PENDING_MMIO_WRITE_WBEAT0,
            CPP_MODE1_TO_MODE2_PENDING_MMIO_WRITE_WSTRB0,
            CPP_MODE1_TO_MODE2_PENDING_MMIO_WRITE_WLAST0,
            {DDR_DATA_BITS{1'b0}},
            {DDR_STRB_BITS{1'b0}},
            1'b0,
