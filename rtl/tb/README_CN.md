...
- 连续 direct-bypass write 必须按入队顺序带出正确的 `addr/wdata/wstrb/req_id`；
  其中第二轮写 burst 在第一轮 drain 后发起，用于覆盖 write queue head/tail 已经非零时的
  payload 顺序，防止后续 payload store 从压缩队列改为 circular slot store 时破坏数据对应关系
...
- 覆盖 MODE_CACHE 下 `ICACHE` + `DCACHE_R` multi-master cache read miss 与
  `invalidate_all` 同时存在的 drain：`ICACHE` 按 ready-first 两拍握手进入实际
...
- 覆盖 MODE_CACHE 下 `ICACHE` + `DCACHE_R` multi-master `invalidate_all` recovery：
