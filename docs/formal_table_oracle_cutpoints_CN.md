...
- `formal/cache_ctrl_table_oracle_write_then_read` 已通过 targeted hw-cbmc，并已纳入
  stable manifest。它直接实例化实际 `rtl/src/llc_cache_ctrl.v`，formal top 只在
  data/meta/valid/repl 表边界维护一个 tracked set 的 shadow row，证明 partial write
  hit 的写表意图会被后续同地址 read 观察到，且不误发 lower memory request。
- `formal/cache_ctrl_table_oracle_read_miss_refill_then_read` 已通过 targeted hw-cbmc，
  并已纳入 stable manifest。它直接实例化同一个实际生产模块，证明 read miss
  refill/install 的写表意图会被后续同地址 read 观察到，且第二笔 read 不再误发 lower
  memory request。
...
- `formal/cache_ctrl_table_oracle_invalidate_then_read_miss` 已通过 targeted hw-cbmc，
  并已纳入 stable manifest。它证明 invalidate valid clear 会被后续同地址 read 观察到，
  使该 read 不再命中旧 line，而是发起 refill。
- 这五个 proof 不改变生产 RTL/C++，也不替代 store primitive latency/mask proof；它们
...
1. 基于现有五个 table-oracle 原型，继续补 flush、dirty victim 多阶段拆分
