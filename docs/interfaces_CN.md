...
2. `mode=2`
   ...
   - 对窗口外的 DDR 访问：
     - 非 MMIO 且请求规模 `<= 256-bit` 时，interconnect 会强制发
       `256-bit` 对齐的单 beat AXI 访问
     - MMIO 排除当前按“请求起始地址是否命中 MMIO 区间”判断
     - 读响应在 interconnect 处重新按原始地址低位截断/对齐后，再返回上游
     - 写请求则在对齐后的单 beat 内按原始地址低位平移 `wdata/wstrb`
     - 请求规模暂时大于 `256-bit` 时，仍按原有 `64B/2 beat` 路径处理
3. `mode=0/3`
   - LLC_OFF 模式
...
- `ID`：4 位
- 当前集成配置下，下游 AXI4 通道数据位宽：`256-bit (32B beat)`
