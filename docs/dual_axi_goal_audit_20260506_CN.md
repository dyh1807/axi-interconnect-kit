...
审计时间：2026-05-06 20:20 CST，主机 `eda-10`。
...
| RTL contract suite | 全量 contract 通过 | 已通过 53/53；compat read-response shared payload pool 后已复跑 | `rtl/local_debug/vcs_all_contracts_resp_pool_20260506_201648_eda10/driver.log` |
...
`rtl/local_debug/vcs_all_contracts_resp_pool_20260506_201648_eda10`。
...
1. 提交并推送当前 response-pool RTL 修改，作为后续 DC 前备份。
2. 基于提交后的 HEAD 启动新的 `axi_llc_subsystem_compat` link sanity，覆盖
   compat+bridge+MSHR hygiene 以及 response-pool 结构优化后的 RTL；若仍 timeout，
   再继续评估剩余扫描结构和接口宽度。
