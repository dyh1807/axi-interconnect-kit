...
| DC/timing | full/top 1GHz DC 使用 9T20 + SMIC12 SRAM，完成后检查 timing/QoR/netlist | 当前 `c98464a` clean full DC 已在 `eda-07` 重新启动；尚未完成，不能 signoff | `rtl/dc/runs/full_compile_1g_strict_template_9t20_c98464a_20260506_2147_eda07/full_compile_1g.console.log` |
...
已在 `eda-07` 基于当前 `c98464a` 启动新的 clean full DC：
...
- 当前阶段尚无 compile/QoR/timing/netlist 结果，因此仍不能 signoff
...
1. 不再因 20:45 的 OOM 单独修改 valid/repl 或 SRAM 映射；该事件按共享服务器
   `global_oom` 处理。
2. 定期检查 `eda-07` 新 full DC 的 log mtime、阶段、RSS、是否产生
