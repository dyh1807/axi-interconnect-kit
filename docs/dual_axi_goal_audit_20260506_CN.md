...
审计时间：2026-05-06 21:41 CST，主机 `eda-10`。
...
| DC/timing | full/top 1GHz DC 使用 9T20 + SMIC12 SRAM，完成后检查 timing/QoR/netlist | 未完成，不能 signoff | 旧 full DC 启动早于后续 RTL 修改且无完整 post-compile 结果 |
...
当前无我方 `dc_shell` / `common_shell` full DC 后台在跑。`eda-10` 当前全机内存压力较高，
`qimeng1` 用户的 `simulator --mode ckpt` 进程组约占 `801.7GiB RSS`，因此不建议在未
确认资源窗口前贸然启动新的 full DC。
...
1. 不再因 20:45 的 OOM 单独修改 valid/repl 或 SRAM 映射；该事件按共享服务器
   `global_oom` 处理。
2. 资源允许时再启动新的 clean full/top 1GHz DC。启动前先检查目标 eda 服务器内存、
   负载、license 和其它用户大内存任务；DC 必须继续使用 9T20 标准单元和 SMIC12
   data/meta SRAM `.db`，并保留 QoR/timing/area/netlist/ddc/db/svf/sdc/sdf/spf。
