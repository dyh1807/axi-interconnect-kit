...
| RTL/C++ performance diagnostic | 已建立，当前失败 | direct DDR/MMIO microbench 发现 RTL 比 C++ 多 1-5 cycle；见 `docs/rtl_cpp_performance_contract_CN.md` |
...
C++ 预测。2026-05-07 新增的 testbench-only performance diagnostic 已经发现 direct
DDR/MMIO 路径上 RTL 比 C++ 多 1-5 cycle。该检查不修改 production RTL，也不进入默认
VCS contract 回归；详情见 `docs/rtl_cpp_performance_contract_CN.md`。
...
- RTL/C++ cycle-level performance closure：当前 direct path diagnostic 已发现 mismatch；
  DC 完成后需要决定是对齐 C++ cycle model，还是优化 RTL latency。
