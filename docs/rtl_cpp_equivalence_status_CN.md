...
`merge/main-cb56e2b-into-review-20260416`，当前工作树基于 HEAD `19f2640` 并包含
尚未提交的 RTL/C++ perf diagnostic 与 DC cleanup 变更。
...
| RTL VCS contract suite | 通过 53/53 | `rtl/local_debug/vcs_all_contracts_single4_generate_20260508_100218_eda-05` |
| hw-cbmc stable manifest | stable 87 项 | `formal/run_passed_hw_cbmc.sh` 当前纳入 87 个已通过入口；`formal/*/run_hw_cbmc.sh` 总数 89 |
...
| RTL/C++ performance diagnostic | LLC hit 精确通过；非 hit bounded 通过 | LLC hit `ready=0/resp=7/no external` 精确对齐；MODE_CACHE clean read miss/refill 当前最大 +2 cycle、上限 8 cycle；direct DDR/MMIO 当前最大 +5 cycle、上限 6 cycle；见 `docs/rtl_cpp_performance_contract_CN.md` |
...
的 directed 行为，并在近期 RTL-only DC hygiene 与 MMIO bridge 32-bit payload 裁剪后复跑过。
...
注意：Linux gate 当前只比较 C++ 模拟器不同版本之间的性能，不证明 RTL cycle 行为达到
C++ 逐事件预测。2026-05-07 新增的 testbench-only performance diagnostic 已确认
LLC hit 关键路径精确对齐；MODE_CACHE clean read miss/refill 路径上 RTL 比 C++ 最多
多 2 cycle，按 8-cycle 上限做 bounded gate；direct DDR/MMIO 路径上 RTL 比 C++ 多
1-5 cycle，按 6-cycle 上限做 bounded gate。该检查不修改 production RTL，也不进入
默认 VCS contract 回归；详情见 `docs/rtl_cpp_performance_contract_CN.md`。
...
- RTL/C++ cycle-level performance closure：LLC hit 已精确闭合；MODE_CACHE clean
  read miss/refill 和非 hit direct path 目前按 bounded gate 管控。DC 完成后仍可决定
  是否进一步对齐 C++ cycle model 或优化 RTL latency。
...
- `docs/rtl_cpp_performance_contract_CN.md`：RTL/C++ cycle-level performance diagnostic。
