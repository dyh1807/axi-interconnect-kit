...
都 retire 后才允许 target-line maintenance accepted。本轮此前新增 MODE_CACHE
two-outstanding cache read + target-line `invalidate_line` drain 检查：同一 DCache read master 连续发两笔 cache miss，
...
本轮此前新增 MODE_CACHE `invalidate_all` write-side
...
先填充同 set 不同 tag 的 victim/survivor 两条 clean cache line，只 invalidate victim；
随后 victim 必须重新 miss/refill 走 DDR，而 survivor 必须保持 hit 且不出现任何外部
AXI `AR/AW/W`。本轮此前新增 MODE_CACHE cacheable read miss/refill 完成并被
...
- [x] 2026-05-06 bridge invalid-payload no-clear RTL hygiene 复核：`git diff --check`
...
- [x] 全量 RTL contract：`rtl/run_all_contracts.sh` 当前通过 53/53，最新目录
  ...
  上一轮新增 `tb_axi_llc_subsystem_dual_cpp_trace_contract` 中 MODE_CACHE
  `ICACHE` + `DCACHE_R` 两个 read master 各自 cache miss 在途时的 target-line
  `invalidate_line` drain trace：target 为 second-master line；first-master response held
  时 second DDR `RREADY` 仍必须 ready，两个 master 的 response 都 retire 后才允许
  `invalidate_line` accepted。targeted 目录为
  `rtl/local_debug/vcs_dual_cpp_trace_multimaster_invline_20260506_183210_eda-10`。
  上一轮新增 `tb_axi_llc_subsystem_dual_cpp_trace_contract` 中 MODE_CACHE
  `ICACHE` + `DCACHE_R` 两个 read master 各自 cache miss 在途时的 `invalidate_all`
  drain trace：`ICACHE` 仍按 ready-first 两拍握手保持请求，两个 master 分配独立 DDR
  `ARID`；first-master response held 时 second DDR `RREADY` 仍必须 ready，两个 master 的
  response 都 retire 后才允许 `invalidate_all` accepted。targeted 目录为
  `rtl/local_debug/vcs_dual_cpp_trace_multimaster_inval_all_20260506_182525_eda-10`。
  ...
  此前本项新增 `tb_axi_llc_subsystem_dual_cpp_trace_contract` 中 MODE_CACHE
  `invalidate_line` scope trace：同 set 不同 tag 的 victim/survivor clean line
  均已 resident 后，只 invalidate victim；victim 后续重新走 DDR miss/refill，
  survivor 后续保持 hit 且无外部 AXI `AR/AW/W`。targeted 目录为
  `rtl/local_debug/vcs_dual_cpp_trace_invline_scope_read_20260506_151734_eda-10`。
  此前本项新增 `tb_axi_llc_subsystem_dual_cpp_trace_contract` 中 MODE_CACHE
...
- [x] actual cache-control table-oracle dirty-evict then read bounded formal：
  `formal/cache_ctrl_table_oracle_dirty_evict_then_read/run_hw_cbmc.sh` 当前通过；直接实例化
  实际 `llc_cache_ctrl.v`，在 data/meta/valid/repl 表边界使用 tracked set shadow row，
  覆盖 dirty victim writeback response 后安装新 dirty line，后续同地址 read 必须命中
  新安装 line 并返回写入数据，且不得再次发 lower memory request。targeted log 为
...
- [x] actual cache-control table-oracle read-miss refill then read bounded formal：
  `formal/cache_ctrl_table_oracle_read_miss_refill_then_read/run_hw_cbmc.sh` 当前通过；
...
- [x] actual cache-control table-oracle write-then-read bounded formal：
  `formal/cache_ctrl_table_oracle_write_then_read/run_hw_cbmc.sh` 当前通过；直接实例化实际
  `llc_cache_ctrl.v`，在 data/meta/valid/repl 表边界使用 tracked set shadow row，
  覆盖 partial write hit 写表意图会被后续同地址 read 命中并读出 merge 后的 line，
  且不误发 lower memory request。targeted log 为
...
- [x] `formal/cache_ctrl_table_oracle_dirty_evict_then_read`：actual `llc_cache_ctrl.v`
  table-oracle/state-IO bounded smoke，覆盖 tracked set shadow row 下 dirty victim
  writeback 后 install/post-B hit。
...
- [x] 实际 C++ `AXI_Interconnect` trace-based EC 的剩余功能场景已按短期 freeze policy
  ...
  本轮继续补齐 target-line `invalidate_line` 作用域检查：同 set 不同 tag 的
  victim/survivor 两条 clean line 填充后只 invalidate victim，要求 victim 后续
  重新 miss/refill 走 DDR，而 survivor 后续保持 hit 且无外部 AXI 逃逸；最新
  targeted VCS 目录：
  `rtl/local_debug/vcs_dual_cpp_trace_invline_scope_read_20260506_151734_eda-10`，
  全量 RTL contract 目录：
  `rtl/local_debug/vcs_all_contracts_invline_scope_read_20260506_151748_eda-10`；
  本轮继续补齐 MODE_CACHE `invalidate_all` 对 clean resident cache line 的全局恢复
  检查：两条 clean line 填充后 `invalidate_all` accepted，之后两条 line 都必须重新
  miss/refill 走 DDR；最新 targeted VCS 目录：
  `rtl/local_debug/vcs_dual_cpp_trace_inval_all_cache_recovery_20260506_152623_eda-10`，
  全量 RTL contract 目录：
  ...
  本轮继续补齐 two-outstanding cache read + `invalidate_all` drain：同一 DCache
  read master 连续发两笔 miss，第二笔分配独立 DDR `ARID=1`；first response held
  时 second DDR `RREADY` 仍为 1，两个 response 均 retire 后全局 invalidate 才 accepted。
  最新 targeted VCS 目录：
  `rtl/local_debug/vcs_dual_cpp_trace_inval_all_multi_read_20260506_154612_eda-10`，
  全量 RTL contract 目录：
  ...
  本轮继续补齐 cache read miss + cache write miss 混合并发下的 target-line
  `invalidate_line` drain：DCache read/write 各自发出不同 cache line 的 DDR refill
  `ARID=0/1`；read response held 时 write refill `RREADY` 仍为 1，read/write response
  均 retire 后目标 line maintenance 才 accepted。最新 targeted VCS 目录：
  `rtl/local_debug/vcs_dual_cpp_trace_invline_cache_rw_20260506_161212_eda-10`，
  全量 RTL contract 目录：
  ...
  本轮继续补齐 `ICACHE` + `DCACHE_R` multi-master cache read miss + `invalidate_all`
  drain：两个 read master 各自发出不同 cache line 的 DDR refill，`ICACHE` 保持
  ready-first 两拍握手；first-master response held 时 second DDR `RREADY` 仍为 1，
  两个 master 的 response 均 retire 后全局 invalidate 才 accepted。最新 targeted
  VCS 目录：
  `rtl/local_debug/vcs_dual_cpp_trace_multimaster_inval_all_20260506_182525_eda-10`，
  全量 RTL contract 目录：
  `rtl/local_debug/vcs_all_contracts_multimaster_inval_all_20260506_182538_eda-10`；
  本轮继续补齐 `ICACHE` + `DCACHE_R` multi-master cache read miss + target-line
  `invalidate_line` drain：target 为 second-master line，first-master response held 时
  second DDR `RREADY` 仍为 1，两个 master 的 response 均 retire 后 target-line
  invalidate 才 accepted。最新 targeted VCS 目录：
  `rtl/local_debug/vcs_dual_cpp_trace_multimaster_invline_20260506_183210_eda-10`，
  全量 RTL contract 目录：
  `rtl/local_debug/vcs_all_contracts_multimaster_invline_20260506_183225_eda-10`；
  本轮继续补齐 `ICACHE` + `DCACHE_R` multi-master `invalidate_all` recovery：
  两个 read master 先各自填充 clean cache line，`invalidate_all` accepted 后，同两个
  master 再读相同行都必须重新走 DDR miss/refill。最新 targeted VCS 目录：
  `rtl/local_debug/vcs_dual_cpp_trace_multimaster_recovery_20260506_184413_eda-10`，
  全量 RTL contract 目录：
...
- [x] Native dual top 在更完整生产参数下的 smoke：已新增
  ...
  RTL contract 53/53 通过，out_dir 为
  `rtl/local_debug/vcs_all_contracts_20260506_130824_invline_cache_mmio_write`。
  随后新增 clean resident line `invalidate_line` recovery/scope 与 `invalidate_all`
  cache resident recovery trace 后，C++ regression 24/24 通过；latest targeted VCS
  ...
  latest 全量 RTL contract 53/53 目录为
  `rtl/local_debug/vcs_all_contracts_inval_all_write_recovery_20260506_153923_eda-10`。
  继续新增 two-outstanding cache read + `invalidate_all` drain trace 后，C++
  regression 24/24 通过；latest targeted VCS 为
  `rtl/local_debug/vcs_dual_cpp_trace_inval_all_multi_read_20260506_154612_eda-10`，
  latest 全量 RTL contract 53/53 目录为
  `rtl/local_debug/vcs_all_contracts_inval_all_multi_read_20260506_154625_eda-10`。
  ...
  latest 全量 RTL contract 53/53 目录为
  `rtl/local_debug/vcs_all_contracts_invline_multi_read_20260506_155608_eda-10`。
  继续新增 cache read miss + cache write miss 混合并发下的 target-line
  `invalidate_line` drain trace 后，C++ regression 24/24 通过；latest targeted VCS 为
  `rtl/local_debug/vcs_dual_cpp_trace_invline_cache_rw_20260506_161212_eda-10`，
  latest 全量 RTL contract 53/53 目录为
  `rtl/local_debug/vcs_all_contracts_invline_cache_rw_20260506_161247_eda-10`。
  ...
  latest 全量 RTL contract 53/53 目录为
  `rtl/local_debug/vcs_all_contracts_invall_cache_write_mmio_rw_20260506_170725_eda-10`。
  继续新增 `ICACHE` + `DCACHE_R` multi-master cache read miss + `invalidate_all`
  drain trace 后，C++ regression 24/24 通过；latest targeted VCS 为
  `rtl/local_debug/vcs_dual_cpp_trace_multimaster_inval_all_20260506_182525_eda-10`，
  latest 全量 RTL contract 53/53 目录为
  `rtl/local_debug/vcs_all_contracts_multimaster_inval_all_20260506_182538_eda-10`。
  继续新增 `ICACHE` + `DCACHE_R` multi-master cache read miss + target-line
  `invalidate_line` drain trace 后，C++ regression 24/24 通过；latest targeted VCS 为
  `rtl/local_debug/vcs_dual_cpp_trace_multimaster_invline_20260506_183210_eda-10`，
  latest 全量 RTL contract 53/53 目录为
  `rtl/local_debug/vcs_all_contracts_multimaster_invline_20260506_183225_eda-10`。
  继续新增 `ICACHE` + `DCACHE_R` multi-master `invalidate_all` recovery trace 后，
  C++ regression 24/24 通过；latest targeted VCS 为
  `rtl/local_debug/vcs_dual_cpp_trace_multimaster_recovery_20260506_184413_eda-10`，
  latest 全量 RTL contract 53/53 目录为
  `rtl/local_debug/vcs_all_contracts_multimaster_recovery_20260506_184426_eda-10`。
...
- [x] RTL contract 回归：实际 RTL 改动后已重跑 `rtl/run_all_contracts.sh` 和
  ...
  `rtl/local_debug/vcs_dual_cpp_trace_reconfig_pending_mmio_rw_20260506_141306_eda10`；
	  ...
	  继续补齐 pending cacheable read miss/refill 期间请求 MODE_MAPPED 的 targeted VCS，
	  目录为
...
- [x] Linux/image 级 300k/5M 功能与性能 sanity：父仓库临时适配 cacheability/MMIO
  ...
  影响性能，任何非零 cycle/IPC 差异都需要先解释来源，再决定是否接受。可用
  `tools/compare_linux_boot_perf.py --require-exact <baseline.log> <current.log>`
  固化该检查；非 deterministic 或允许微小抖动的 gate 可改用默认阈值
  `--max-cycle-delta-pct 1.0 --max-ipc-drop-pct 1.0`。
  2026-05-06 用户再次明确：Linux boot 5M 测试结论不能只关注是否出现 error，
  必须报告 IPC、cycle 数量、相对 baseline 的差异是否显著，以及该差异是否可接受。
  ...
  本轮新增 multi-master `invalidate_all` trace 只修改 trace generator、generated
  include、RTL contract TB 和文档，不改变 production simulator/RTL 路径，因此不重跑
  Linux，继续沿用上述 5M cycle delta=0、IPC delta=0 的结论。
  本轮继续新增 multi-master target-line `invalidate_line` trace，仍只修改测试生成器、
  ...
  cycle delta=0、IPC delta=0 的结论。本轮继续新增固定 32 seed maintenance/recovery
  suite，仍只修改测试生成器、generated include、RTL contract TB 和文档，不改变
  production simulator/RTL 路径；Linux 5M 继续沿用上述 cycle delta=0、IPC delta=0
  的结论。下一次 production C++/RTL 路径改动后必须重新跑 300k/5M，并报告
  error/difftest、cycles、IPC、delta 和差异是否可接受。
...
- [x] invalidate_line / invalidate_all 相关 hazard、drain 与 recovery 合同；
  ...
  contract 覆盖；target-line `invalidate_line` 只清 victim、不误清同 set 不同 tag
